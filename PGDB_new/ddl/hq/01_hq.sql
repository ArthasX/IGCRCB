...
WHERE = B.RESOURCE_ID AND A.PLATFORM_ID = C.PLID AND C.GROUPTYPE = 13; 

--4 虚机属性信息视图
CREATE OR REPLACE VIEW vm_property_data AS 
select p.fqdn,a.propvalue,b.propkey,b.description from eam_platform p,eam_cprop a  , eam_cprop_key b				
where p.platform_type_id  in (10018 ,10019 ,10010)
and p.id = a.appdef_id and a.keyid=b.id  order by p.id;
ALTER TABLE vm_property_data
  OWNER TO postgres; 
