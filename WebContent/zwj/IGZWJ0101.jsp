...
<html>
<bean:define id="id" value="IGZWJ0101" toScope="request" />
...
function initAttFormFile(){
	...
	//如果是告警页面发起的流程
	var alarmFlag = '<%=request.getParameter("alarmFlag")%>';
	var result;
	var prdescEdit = document.getElementById("prdescEdit");
	if(alarmFlag == 'true' || alarmFlag == 'syslog'){
		var alarmid = "${IGPRR0101Form.parameters}";
		if(alarmid != null && alarmid != ""){
			if(alarmFlag == 'true'){
				var objAjax = new Ajax.Request(getAppRootUrl() + ajaxAction,
						{asynchronous:false,parameters:"type=getAlarmByID&alarmid="+alarmid,method:"post",
				...
				if(result != null){
					...
					if(results[0] == '1') level = "低";
					else if(results[0] == '2') level = "中";
					else if(results[0] == '3') level = "高";
					else level = '无';
	
					var showStr = "告警级别："+ level + "，告警时间：" + results[1] + "，告警IP：" + results[2] + "，业务系统：" + results[3]
					              + "，告警原因：" + results[4] + "，触发值：" + results[5] + "。";
					
					prdescEdit.innerHTML = showStr;
					document.getElementById("prdesc").value = showStr;
				}
				getFormObj("告警ID").value = alarmid;
			}else if(alarmFlag == 'syslog'){
				var objAjax = new Ajax.Request(getAppRootUrl() + ajaxAction,
						{asynchronous:false,parameters:"type=getSyslogAlarmByID&alarmid="+alarmid,method:"post",
							 onSuccess:  function(req, json){
							 	 result = req.responseText;
							 }
				});
				if(result != null){
					...
					var showStr = "级别："+ results[0] + "，触发时间：" + results[1] + "，设备IP：" + results[2] + "，设备名：" + results[3]
					              + "，日志内容：" + results[4] + "。";
					
					prdescEdit.innerHTML = showStr;
					document.getElementById("prdesc").value = showStr;
				}
				getFormObj("告警ID").value = alarmid+'#syslog';
			}
			getFormObj("发生时间").value = results[1];
			getFormObj("故障类型").selectedIndex = 9;
...
function setIGZWJ0101Params(username,orgname,orgid,useremail,userphone,usermobile){
	var index=0;
	var orgidtree="";
	if(orgid!=""){
	...
	<logic:iterate id="map" name="IGPRR01011VO" property="processInfoDefmap" >
		...
		<logic:present name="psid">
			...
			$("pivarvalue[" + index + "]").value =username;
					...
					<logic:equal value="报告人科室" name="psid" property="pidlabel">
						$("pivarvalue[" + index + "]").value = orgid + "_OrgTree_" + orgname;
						$("pivartreename[" + index + "]").value = orgname;
							</logic:equal>
							<logic:equal value="报告人联系方式" name="psid" property="pidlabel">
							$("pivarvalue[" + index + "]").value =usermobile;
									</logic:equal>
									<logic:equal value="报告人邮件" name="psid" property="pidlabel">
...
function searchuser(){	
	openSubWindow('/IGZWJ0101_Disp.do?', '_blank','1000','600','center:1');
}
</script>
...
<%-- 				<html:textarea property="prdesc" cols="60" rows="6"  errorStyleClass="inputError imeActive"></html:textarea></td> --%>
				<div class="editDiv" style="width:440px;min-height:85px;" id="prdescEdit" contenteditable="true" onkeydown="editDivContent(this);" onblur="getProcessDescValue(this);">
					${IGPRR0101Form.prdesc }
				</div>
				<html:hidden property="prdesc" styleId="prdesc" name="IGPRR0101Form"/>
			</tr>
			</logic:equal>
			...
			if("2".equalsIgnoreCase(psid.getPidaccess()) || "3".equalsIgnoreCase(psid.getPidaccess()) || "4".equalsIgnoreCase(psid.getPidaccess())){
				...
				<logic:equal value="报告人姓名" name="psid" property="pidlabel">
					<ig:formToTag 
						...
						/>
						<img src="images/seek.gif" border="0" style="cursor: pointer;" onclick="searchuser();"/>
						</logic:equal>
					<logic:notEqual value="报告人姓名" name="psid" property="pidlabel">
					<ig:formToTag 
						name="IGPRR0102Form" 
						pidid="${pidid }" 
						property="pivarvalue[${index}]" 
