...
<html:html>
<bean:define id="id" value="IGPRR0114" toScope="request" />
	...
	function initSelectRole() {
		...
		for(var i = 0; i < arr_role.length; i++){
			...
			if(flag) {
				...
				//str_role += "<option value='" + 999 + "'>" + 000 + "</option>";
				//str_role += "<option value='" + 888 + "'>" + 999 + "</option>";
			}
			flag = true;
		}
...
<div class="back"><a href="IGPRR0102_Disp.do?pdid=${IGPRR0113Form.pdid}&prid=${IGPRR0113Form.prid}" target="_self">返回</a></div>
</div>
			...
			<logic:present name="IGPRR01131VO" property="roleList">
				<logic:iterate id="bean" name="IGPRR01131VO" property="roleList">
					<tr id="role_${bean.roleid}">
					  ...
					  <td width="70%">${bean.rolename}<c:if test="${bean.roleid == IGPRR0113Form.ppsuperid}">(主办部门)</c:if><input type="hidden" name="role" value="${bean.roleid}_${bean.rolename}"><input type="hidden" name="roleid" value="${bean.roleid}"></td>
					</tr>
				</logic:iterate>
			</logic:present>
	</table>
