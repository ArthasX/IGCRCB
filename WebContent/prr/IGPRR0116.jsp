...
<html>
<bean:define id="id" value="IGPRR0116" toScope="request" />
<bean:define id="title" value="工作信息" toScope="request" />
<!-- header1 -->
<jsp:include page="/include/header3.jsp" />
...
function selectRole(){
	...
	document.location.replace(getAppRootUrl() + "/IGPRR0116_Disp.do?isServiceStart=${IGPRR0101Form.isServiceStart}&parprid=${IGPRR0101Form.parprid}&psdcode=<%=IGPRDCONSTANTS.PROCESS_START_STATUS%>&pdid="+pdid+"&roleid="+roleid);
...
function setParams(prcorid,prcortype,prcortitle){
	...
	var value="";
	var rvalue = document.getElementsByName("pivarvalue[" + num + "]")[0].value;
	...
	document.getElementsByName("pivarvalue[" + num + "]")[0].value= cvalue;
	num = 0;
}
...
function setParamIGCOM0302(eid, elabel,ename,eiid, eilabel, einame,ecategory,eiversion, pidid) {
	if(setEntityObj != null){
		...
		if("" == eiid){
			document.getElementsByName("entityId[" + num + "]")[0].value = "";
			document.getElementsByName("entityname[" + num + "]")[0].value = "";
		} else {
			document.getElementsByName("entityId[" + num + "]")[0].value = pidid + "#" + eiid + "#" + eiversion;
			document.getElementsByName("entityname[" + num + "]")[0].value = einame + "(" + eiversion + ")";
			if(pidid != 'A'){
				document.getElementsByName("pivarvalue[" + tp + "]")[0].value = "N/A";
			}
		}
...
function setParticipant(obj){
	...
	if(obj != null && obj.length > 0){
		for(var i=0;i<obj.length;i++){
			...
			td4.innerHTML = '<a href="javascript:void(0);" onclick="deleteParticipant(this);">删除</a>' +
...
<div id="container" align="center"><!--content  右侧内容-->
<div id="content"  align="center"><html:form action="/IGPRR0116" styleId="form"
	onsubmit="return checkSubmit(this);" enctype="multipart/form-data">
	<div id="location">
	<div class="image"></div>
	<p class="fonts1">流程管理
	<logic:equal parameter="ptid" value="8">
	（${IGPRR0101Form.prpdname}——待发起）
	</logic:equal>
	<logic:notEqual parameter="ptid" value="8">流程信息 </logic:notEqual></p>
	</div>
	<div>
	...
	<table cellspacing="2" class="table_style1">
		...
		<logic:present name="IGPRR01011VO" property="processTitleDefTB">
			...
			<tr>	
				...
				<td colspan="3"><html:text styleId="prtitle" property="prtitle" size="82" errorStyleClass="inputError imeActive" />
				</td>
			</tr>
			</logic:equal>
			...
			<tr>
				...
				<td colspan="3">
				<%-- <html:textarea styleId="prdesc" property="prdesc" cols="60" rows="6"  errorStyleClass="inputError imeActive"></html:textarea></td> --%>
	...
	<html:hidden property="isServiceStart" styleId="isServiceStart" name="IGPRR0101Form" />
