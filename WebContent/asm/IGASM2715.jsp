...
<html:html>
<bean:define id="id" value="IGASM2715" toScope="request"/>
<bean:define id="title" value="风险点属性信息编辑画面" toScope="request"/>
...
<!-- /header1 -->
	<script type="text/javascript">	
		...
		function confirmDisp(ident){
			var message1 = '<bean:message key="IGCO10000.W003" arg0="风险点配置信息" />';

			<logic:present name="IGASM27041VO" property="configItemVWInfoMap">
			<logic:iterate id="map" name="IGASM27041VO" property="configItemVWInfoMap" indexId="number">
        ...
        function toback(){
        	 ...
        	 if(looksign=="1"){
	        	form.action = getAppRootUrl() + "/IGASM2701.do";
        	 }else{
				form.action = getAppRootUrl() +"/IGASM2714.do";        		 
        	 }
        	form.submit();
        }
        var num = 0;
      ...
      //删除附件
    	function delFile(fileid){
    		 if( window.confirm("您是否确定删除附件？")){
    			form.action = getAppRootUrl() + "/IGASM2704.do?delfile["+fileid+"]="+1+"&ident="+0;
    	        form.submit();
    		 }
      }
    	 function selectVersion(){
    			...
    			window.location.href="IGASM2704_Disp.do?eiid="+eiid+"&eiversion="+a;
    	      }
    	 function searchuser(){	
...
<div id="container">
    ...
    <!--content  右侧内容-->
    <div id="contentWrap" style="margin-bottom:10px;">
        <div id="content">

            <div id="location">
                <div class="image"></div>
                <p class="fonts1"><ig:navigation extname1="${urlable}"/> &gt;&gt; 风险点属性编辑 </p>
            </div>

		    <html:form styleId="form" action="/IGASM2704" enctype="multipart/form-data" onsubmit="return checkSubmit(this);">
                 <div class="title">
                     <div class="name">风险点基本信息</div>
                 </div>
                 <div id="results_list">
				    <table class="table_style">
						<tr>
								...
								<th width="8%" >风险项</th>
								<th width="8%">风险点编号</th>
								<th width="16%">风险点名称</th>
								<th width="12%">风险点说明</th>
								<th width="6%">登记人</th>
								<th width="6%">登记日期</th>
								<th width="8%">更新时间</th>
							</tr>
							 <logic:present name="IGASM27041VO" property="riskPointList" >
		               <logic:iterate id="bean" name="IGASM27041VO" property="riskPointList" indexId="index">
                  ...
                  <div class="title">
                      <div class="name">风险点属性信息</div>
                  </div>
    <div class="message"><ig:message /></div>
    <logic:present name="IGASM27041VO" property="flag">
    <logic:equal value="true" name="IGASM27041VO" property="flag">	
	<logic:present name="IGASM27041VO" property="configItemVWInfoMap">
		<div id="TabbedPanels1" class="TabbedPanels">
		<ul class="TabbedPanelsTabGroup">
			<logic:iterate id="map" name="IGASM27041VO"
				...
				<li class="TabbedPanelsTab" tabindex="0"><ig:codeValue
					...
					<option value="">版本选择&nbsp&nbsp当前版本${IGASM2704Form.selecteiversion}.0 </option>
					 <logic:present name="IGASM27041VO" property="checkVersionMap" >
					<%-- <html:optionsCollection name="IGASM27041VO" property="checkVersionMap" label="${versionMap.value}" value="${versionMap.value}"/> --%>
		               <logic:iterate id="versionMap" name="IGASM27041VO" property="checkVersionMap" indexId="index">
		...
		<div class="TabbedPanelsContentGroup" id="identify"><logic:present
			name="IGASM27041VO" property="configItemVWInfoMap">
			<logic:iterate id="map" name="IGASM27041VO"
				property="configItemVWInfoMap" indexId="number">
				...
				<table class="table_style2">
					...
					<logic:iterate id="bean" name="list" indexId="index">
						<tr onmouseover="mouseOverNoHand(this);" onmouseout="mouseOutNoHand(this);" <logic:equal name="bean" property="cseq" value="1">style="display: none;"</logic:equal>>
							...
							<logic:equal name="bean" property="cattach" value="5">
								<logic:present name="IGASM2704Form" property="civalue[${ADcount}]">
									<bean:define id="civalue" name="IGASM2704Form" property="civalue[${ADcount}]" type="java.lang.String" />
								...
								<logic:notPresent name="IGASM2704Form" property="civalue[${ADcount}]">
									<logic:iterate id="checkboxbean" name="ADlist${ADmapcount}">
	 ...
	 <div class="title">
              <div class="name">风险点关系信息</div>
     </div>
	<div id="TabbedPanels1" class="TabbedPanels">
		<ul class="TabbedPanelsTabGroup">
		<li class="TabbedPanelsTab"  style="width:150px" onclick="changCheckToRisk()">所属领域检查项</li>
		</ul>
			...
			<div class="TabbedPanelsContent">
				...
				<table class="table_style2">
					<tr>
						<th width="10%" align="center">检查项编号</th>
						<th width="30%" align="center">检查项名称</th>
						<th width="10%" align="center">登记人</th>
						<th width="30%" align="center">检查项说明</th>
						<th width="20%" align="center">建议检查频度</th>
					</tr>
				   <logic:present name="IGASM27041VO" property="searchRiskCheckList" >
	               <logic:iterate id="bean" name="IGASM27041VO" property="searchRiskCheckList" indexId="index">
						<tr>
							<td>${bean.eilabel }</td>
							<td>${bean.einame }</td>
							<td>${bean.eiusername }</td>
		...
		<li class="TabbedPanelsTab" style="width:150px;" onclick="changeCheckToZb();">所属领域风险指标</li>
			...
			<div class="TabbedPanelsContent" >
				...
				<table class="table_style2">
					...
					<logic:present name="IGASM27041VO" property="riskToZhRelateList" >
	               <logic:iterate id="bean" name="IGASM27041VO" property="riskToZhRelateList" indexId="index">
						<tr>
							<td>${bean.eilabel }</td>
							<td>${bean.einame }</td>
							<td>${bean.eidesc }</td>
    ...
    <html:hidden property="selecteiversion"    styleId="selecteiversion" value="${IGASM2704Form.selecteiversion}"/>
    <html:hidden property="eiid" styleId="eiid" value="${IGASM2704Form.eiid}"/>
    <html:hidden property="eiversion" styleId="eiversion" value="${IGASM27041VO.entityData.eiversion}"/>
    <html:hidden property="eiorgsyscoding" styleId="eiorgsyscoding" value="${IGASM27041VO.entityData.eiorgsyscoding}" />
	</html:form>
		</div>
    </div>
</div>
