...
<html:html>
<bean:define id="id" value="IGASM1604" toScope="request" />
<bean:define id="title" value="数据配置信息编辑画面" toScope="request" />
...
<script type="text/javascript">	
		...
		function confirmDisp(ident){
			var message1 = '<bean:message key="IGCO10000.W003" arg0="数据配置信息" />';

			<logic:present name="IGASM16041VO" property="configItemVWInfoMap">
			<logic:iterate id="map" name="IGASM16041VO" property="configItemVWInfoMap" indexId="number">
        ...
        function toback(){
        	form.action = getAppRootUrl() + "/IGASM1604.do?org.apache.struts.taglib.html.CANCEL=1";
        	form.submit();
        }
        var num = 0;
      ...
      //删除附件
    	function delFile(fileid){
    		 if( window.confirm("您是否确定删除附件？")){
    			form.action = getAppRootUrl() + "/IGASM1604.do?delfile["+fileid+"]="+1+"&ident="+0;
    	        form.submit();
    		 }
      }
	</script>
...
<div id="location">
<div class="image"></div>
<p class="fonts1"><ig:navigation extname1="${urlable}"/>&gt;&gt; 数据配置编辑</p>
...
<html:form styleId="form" action="/IGASM1604"
	...
	<table class="table_style">
		...
		<tr>
			...
			<td align="center">${IGASM16041VO.entityData.eilabel}</td>
			<td align="center">${IGASM16041VO.entityData.einame}</td>
			<td>${IGASM16041VO.eiorgname}</td>
			<td>${IGASM16041VO.entityData.eidesc}</td>
			<td align="center">${IGASM16041VO.entityData.eiusername}</td>
			<td align="center">${IGASM16041VO.entityData.eiinsdate}</td>
						...
						<tr onmouseover="mouseOverNoHand(this);"
							...
							<div class="nowrapDiv"><logic:equal name="bean"
								...
								<html:text property="civalue[${ADcount}]" styleId="civalue"
									size="40" errorStyleClass="inputError" styleClass="input1" />
							</logic:equal> <logic:equal name="bean" property="cattach" value="2">
								<html:select property="civalue[${ADcount}]" style="width:53.5%;"
									errorStyleClass="inputError imeActive">
									<html:options collection="ADlist${ADmapcount}" property="value"
								...
								<html:text property="civalue[${ADcount}]"
									...
									readonly="true" />
								<img src="images/seek.gif" border="0" width="16" height="16"
									alt="查询"
								...
								<logic:equal name="bean" property="cattach" value="5">
								<logic:present name="IGASM1604Form" property="civalue[${ADcount}]">
									<bean:define id="civalue" name="IGASM1604Form" property="civalue[${ADcount}]" type="java.lang.String" />
								...
								<logic:notPresent name="IGASM1604Form" property="civalue[${ADcount}]">
									<logic:iterate id="checkboxbean" name="ADlist${ADmapcount}">
...
<!--	<html:submit property="btn_insert" value="保存" styleClass="button" onclick="return confirmDisp(0);" />-->
	<html:submit property="btn_insert" value="保存" styleClass="button" onclick="return confirmDisp(1);" />
	</logic:equal>
	</logic:present>
	<html:cancel styleClass="button" value="返回" />
	</div>
	<html:hidden property="eiid" styleId="eiid" value="${IGASM1604Form.eiid}" />
	<html:hidden property="eiversion" styleId="eiversion" value="${IGASM16041VO.entityData.eiversion}" />
	<html:hidden property="eiorgsyscoding" styleId="eiorgsyscoding" value="${IGASM16041VO.entityData.eiorgsyscoding}" />
</html:form></div>
</div>
</div>
