...
<html:html>
<bean:define id="id" value="IGASM1704" toScope="request"/>
<bean:define id="title" value="组件配置信息编辑画面" toScope="request"/>
...
<!-- /header1 -->
	<script type="text/javascript">	
		...
		function confirmDisp(ident){
			var message1 = '<bean:message key="IGCO10000.W003" arg0="组件配置信息" />';

			<logic:present name="IGASM17041VO" property="configItemVWInfoMap">
			<logic:iterate id="map" name="IGASM17041VO" property="configItemVWInfoMap" indexId="number">
        ...
        function toback(){
        	form.action = getAppRootUrl() + "/IGASM1704.do?org.apache.struts.taglib.html.CANCEL=1";
        	form.submit();
        }
        var num = 0;
      ...
      //删除附件
    	function delFile(fileid){
    		 if( window.confirm("您是否确定删除附件？")){
    			form.action = getAppRootUrl() + "/IGASM1704.do?delfile["+fileid+"]="+1+"&ident="+0;
    	        form.submit();
    		 }
      }
	</script>
...
<div id="container">
    ...
    <div id="contentWrap">
        <div id="content">
            ...
            <div id="location">
                <div class="image"></div>
                <p class="fonts1"><ig:navigation extname1="${urlable}"/>&gt;&gt; 组件配置编辑 </p>
                <div class="back"><html:link href="javascript:toback();">返回</html:link></div> 
            </div>

		    <html:form styleId="form" action="/IGASM1704" enctype="multipart/form-data" onsubmit="return checkSubmit(this);">
                 <div class="title">
                 ...
                 <div id="results_list">
				    <table class="table_style">
						...
						<tr>
						  <td>${IGASM17041VO.entityData.ename}</td>
						  <td>${IGASM17041VO.entityData.eilabel}</td>
						  <td>${IGASM17041VO.entityData.einame}</td>
						  <td>
						  	${IGASM17041VO.entityData.eidesc}
						   
						  </td><!--
						  <td>
	                        <html:select property="eistatus" errorStyleClass="inputError" tabindex="4">
	                            <ig:optionsCollection ccid="ENTITY_ITEM_STATUS_CODE" isMakeBlankRow="true" isCodeLabel="true"/>
	                        </html:select>
						  </td>
                         -->
                         <td align="center">${IGASM17041VO.entityData.eiusername}</td>
                         <td align="center">${IGASM17041VO.entityData.eiinsdate}</td>
				...
				<table class="table_style2">
					...
					<logic:iterate id="bean" name="list" indexId="index">
						<tr onmouseover="mouseOverNoHand(this);" onmouseout="mouseOutNoHand(this);" <logic:equal name="bean" property="cseq" value="1">style="display: none;"</logic:equal>>
							...
							<logic:equal name="bean" property="cattach" value="5">
								<logic:present name="IGASM1704Form" property="civalue[${ADcount}]">
									<bean:define id="civalue" name="IGASM1704Form" property="civalue[${ADcount}]" type="java.lang.String" />
								...
								<logic:notPresent name="IGASM1704Form" property="civalue[${ADcount}]">
									<logic:iterate id="checkboxbean" name="ADlist${ADmapcount}">
	...
	<html:hidden property="eiid" styleId="eiid" value="${IGASM1704Form.eiid}"/>
	<html:hidden property="eiversion" styleId="eiversion" value="${IGASM17041VO.entityData.eiversion}"/>
	<html:hidden property="eiorgsyscoding" styleId="eiorgsyscoding" value="${IGASM17041VO.entityData.eiorgsyscoding}" />
    </html:form>
		</div>
    </div>
</div>
