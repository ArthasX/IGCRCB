...
<%@ page import="com.deliverik.infogovernor.fin.model.entity.BudgetTB"%>
<%
	String contextPath = request.getContextPath();
%>
<html:html>
<bean:define id="id" value="IGPRJ0109" toScope="request" />
<bean:define id="title" value="项目修改" toScope="request" />
<!-- header1 -->
<jsp:include page="/include/header1.jsp" />
<!-- /header1 -->
<script type="text/javascript">   
	var bodyLoadTrs = new Array();
	function prjSubmit(){  
	...
	function setBudget(bid,bamount_show,bamount,btitle){
		document.forms[0].pbid.value = bid;
		document.forms[0].bamount_show.value = bamount_show;
		if(bamount!=null){
			var m = parseFloat(bamount).toString();
			document.forms[0].pamount.value = m;
		}else{
			document.forms[0].pamount.value = '';
		}
		document.forms[0].pbname.value = btitle;
	}
	...
	function setPanrentParem(parm){
		...
		for(var i=0;i<parm.length;i++){
			if(null!=bodyLoadTrs[parm[i][0]]){
				parm[i]=bodyLoadTrs[parm[i][0]];
			}
	...
	function cannotChance(){
		document.getElementById("pbzdevelop").readOnly=true;
		document.getElementById("pbzhardware").readOnly=true;
		document.getElementById("pbzsoftware").readOnly=true;
		document.getElementById("pbzproject").readOnly=true;
		document.getElementById("pbzelse").readOnly=true;
		document.getElementById("pbcimplement").readOnly=true;
		document.getElementById("pbcelse").readOnly=true;
		document.getElementById("sum_btn").disabled=true;
	...
	function bodyLaod(){
		var ro='';
		isUseChange('${IGPRJ0109Form.isUseBudget}');
		if('${IGPRJ0109Form.hasPc}'=='Y'){
			 cannotChance();
			 ro = '  readonly="true"';
		}
		<logic:present name="IGPRJ01091VO" property="pbsummaryList">
			<logic:iterate id="bean" name="IGPRJ01091VO" property="pbsummaryList">
				var index = bodyLoadTrs.length;
				var bid = '${bean.bid}';
				var btitle = '${bean.btitle}';
				var bsub1 = ('${bean.sub1+bean.use1}'.substring(0, '${bean.sub1+bean.use1}'.indexOf('.')+3));
				var bsub2 = ('${bean.sub2+bean.use2}'.substring(0, '${bean.sub2+bean.use2}'.indexOf('.')+3));
				var bsub3 = ('${bean.sub3+bean.use3}'.substring(0, '${bean.sub3+bean.use3}'.indexOf('.')+3));
				var bsub4 = ('${bean.sub4+bean.use4}'.substring(0, '${bean.sub4+bean.use4}'.indexOf('.')+3));
				var bsub5 = ('${bean.sub5+bean.use5}'.substring(0, '${bean.sub5+bean.use5}'.indexOf('.')+3));
				var bsub6 = ('${bean.sub6+bean.use6}'.substring(0, '${bean.sub6+bean.use6}'.indexOf('.')+3));
				var bsub7 = ('${bean.sub7+bean.use7}'.substring(0, '${bean.sub7+bean.use7}'.indexOf('.')+3));
				...
				td1.rowSpan=2;
				td10.rowSpan=2;
				td1.innerHTML=btitle+'<input type="hidden" name="bname_check" value="'+btitle+'" />';
				td2.innerHTML='可用预算';
				td3.innerHTML=bsub1+'<input type="hidden" name="bzdevelop_check" value="'+bsub1+'" />';
				td4.innerHTML=bsub2+'<input type="hidden" name="bzhardware_check" value="'+bsub2+'" />';
				td5.innerHTML=bsub3+'<input type="hidden" name="bzsoftware_check" value="'+bsub3+'" />';
				td6.innerHTML=bsub4+'<input type="hidden" name="bzproject_check" value="'+bsub4+'" />';
				td7.innerHTML=bsub5+'<input type="hidden" name="bzelse_check" value="'+bsub5+'" />';
				td8.innerHTML=bsub6+'<input type="hidden" name="bcimplement_check" value="'+bsub6+'" />';
				td9.innerHTML=bsub7+'<input type="hidden" name="bcelse_check" value="'+bsub7+'" />';
				td10.innerHTML='<a href="javascript:deletetrs('+bid+')">删除</a>';
				tr.id=bid;
				tr=tb.insertRow(rowIndex+1);
				td2 = tr.insertCell(0);
				td3 = tr.insertCell(1);
				...
				td8 = tr.insertCell(6);
				td9 = tr.insertCell(7);
				td2.innerHTML='使用预算<input type="hidden" name="bid" value="'+bid+'" />';
				td3.innerHTML='<input type="text" name="bzdevelop" value="${bean.use1}" size="5">';
				td4.innerHTML='<input type="text" name="bzhardware" value="${bean.use2}" size="5"></td>';
				td5.innerHTML='<input type="text" name="bzsoftware" value="${bean.use3}" size="5">';
				td6.innerHTML='<input type="text" name="bzproject" value="${bean.use4}" size="5">';
				td7.innerHTML='<input type="text" name="bzelse" value="${bean.use5}" size="5">';
				td8.innerHTML='<input type="text" name="bcimplement" value="${bean.use6}" size="5">';
				td9.innerHTML='<input type="text" name="bcelse" value="${bean.use7}" size="5">';
				index+=2;
				bodyLoadTrs[bid]=new Array(bid,btitle,bsub1,bsub2,bsub3,bsub4,bsub5,bsub6,bsub7);
			</logic:iterate>
		</logic:present>
	}
	...
	function checkys(){
		...
		for(var i=0;i<sub1.length;i++){
			...
			if(parseFloat(sub1[i].value)>parseFloat(checksub1[i].value)){
				alert(checkbname[i].value+'资本金开发大使用预算于可用资本金开发！');
				return true;
			}
...
<div id="container">
 ...
 <div id="contentWrap">
   <div id="content">
      <div id="location">
       <div class="image"></div>
        <p class="fonts1">服务管理 &gt;&gt; 项目管理 &gt;&gt; 项目修改</p>
        <div class="back">
			<a href="IGPRJ0101_BACK.do" target="_self">返回</a>
		</div>
     </div>
      
   <div id="formwrapper">
       <html:form action="IGPRJ0901_UPDATE.do" method="post">
       <ig:message />
       <fieldset>
        	<legend>预算信息</legend>
         <div>
        ...
        <html:hidden property="hasPc"/>
       <div>
            <label for="Name"><strong><span class="red">*</span>项目名称</strong>：</label>
             <html:text name="IGPRJ0109Form" property="pname" size="40"/>
            <br />
       </div>
       <div>
            <label for="Name"><strong>项目编号</strong>：</label>
           <html:text name="IGPRJ0109Form" property="pcode" size="40"/>
            <br />
       </div>
        ...
        <div>
            ...
            <table>
            	<tr>
            		<td>
	           			开发:<html:text property="pbzdevelop" size="16" />(万元)
			        </td>
			    </tr>
			    <tr>
			        <td>
						硬件:<html:text property="pbzhardware" size="16" />(万元)
					</td>
				</tr>
			    <tr>
					<td>
						软件:<html:text property="pbzsoftware" size="16" />(万元)
					</td>
				</tr>
			    <tr>
					<td>
						工程:<html:text property="pbzproject" size="16" />(万元)
					</td>
				</tr>
			    <tr>
					<td>
						其他:<html:text property="pbzelse" size="16" />(万元)
					</td>
				</tr>
        	</table>
        	</div>
        ...
        <div>
            ...
            <table>
            	<tr>
            		<td>
			           	实施:<html:text property="pbcimplement" size="16" />(万元)
			        </td>
			    </tr>
			    <tr>
			        <td>
						其他:<html:text property="pbcelse" size="16" />(万元)
					</td>
				</tr>
				</table>
			</div>
        ...
        <div>
            ...
            <html:text property="plimit" size="16" readonly="true" />(万元)&nbsp;&nbsp;&nbsp;&nbsp;
            <input type="button" id="sum_btn" class="button" onclick="sum()" value="计算预算金额">
          	<br/>
        </div>
      
       <div>
          <label for="Name"><strong><span class="red">*</span>开始时间</strong>：</label>
          	 <html:text name="IGPRJ0109Form" property="popentime" size="16" readonly="true" />
	                        <img src="<%=contextPath %>/images/date.gif" onClick="calendar($('popentime'))" border=0 />
          <br />
        </div>
        <div>
          <label for="Name"><strong><span class="red">*</span>计划结束时间</strong>：</label>
          
			 <html:text name="IGPRJ0109Form" property="pplanclosetime" size="16" readonly="true" />
                            <img src="<%=contextPath %>/images/date.gif" onClick="calendar($('pplanclosetime'))" border=0 />
        ...
        <div>
          <label for="Name"><strong><span class="red">*</span>实际结束时间</strong>：</label>
			 ...
			 <html:text name="IGPRJ0109Form" property="pclosetime" size="16" readonly="true" />
                            <img src="<%=contextPath %>/images/date.gif" onClick="calendar($('pclosetime'))" border=0 />
         ...
         <div>
            <label for="Name"><strong>项目简介</strong>：</label>
     		<html:textarea name="IGPRJ0109Form" property="pdesc" cols="80" rows="6"></html:textarea>
          <br />
         </div>
          <div>
            <label for="Name"><strong>项目目的</strong>：</label>
             <html:textarea name="IGPRJ0109Form" property="paim" cols="80" rows="6"></html:textarea>
            <br />
       	</div>
       	<div>
            <label for="Name"><strong>项目用途</strong>：</label>
             <html:textarea name="IGPRJ0109Form" property="ppurpose" cols="80" rows="6"></html:textarea>
            <br />
       	</div>
        <div>
            <label for="Name"><strong>需求部门</strong>：</label>
            <html:select name="IGPRJ0109Form" property="pbranch" style="width: 100px;" errorStyleClass="inputError">
