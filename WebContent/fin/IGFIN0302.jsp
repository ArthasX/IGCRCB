...
%>
<html:html>
<bean:define id="id" value="IGFIN0302" toScope="request" />
...
<!--content  右侧内容-->
   ...
   <div id="location">
       <div class="image"></div>
        <p class="fonts1">服务管理 &gt;&gt; 预算管理 &gt;&gt; 财务统计报告</p>
    </div> 
     <div id="search">
    <html:form styleId="form" action="/IGFIN0303" onsubmit="return checkSubmit(this);">
     <div class="img"></div>
       		...
       		<tr>
				...
				<tr class="<ig:rowcss index="<%=i %>"/>">
					...
					<td>
						...
						%>
						<a href="IGFIN0304.do?searchtype=<%=vo.getBType() %>&bopentime=<%=form.getBopentime()%>&bclosetime=<%=form.getBclosetime()%>&flg=B&type1=<%=form.getSearchtype()%>"><%=vo.getSumBudget()%></a>
						<%} %>
						</div>
					</td>
					<td>
					...
					<td>
						...
						%>
						<a href="IGFIN0304.do?searchtype=<%=vo.getBType() %>&bopentime=<%=form.getBopentime()%>&bclosetime=<%=form.getBclosetime()%>&flg=E&type1=<%=form.getSearchtype()%>"><%=vo.getSumExpense()%></a>
						<%} %>
						</div>
					</td>
					<td>
