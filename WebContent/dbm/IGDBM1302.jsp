...
%>
<html:html>
<bean:define id="id" value="IGDBM1302" toScope="request" />
<bean:define id="title" value="工作新增画面" toScope="request" />
<!-- header1 -->
...
var gid='IGRIS0202';
	...
	function confirmDisp(){
		...
		if(document.forms[0].wmremindh.value.trim()=="0"||document.forms[0].wmremindm.value.trim()=="0"){
			alert("请选择提醒时间！");
			return false;
		}
...
<div id="formwrapper">
	<html:form styleId="form" action="/IGDBM1302_Insert" onsubmit="return checkSubmit(this);" enctype="multipart/form-data">
	...
	<fieldset>
		...
		<div><label for="Name"><strong><span class="red">*</span>发起人</strong>：</label>
			<p class="p2">${IGDBM1302Form.wmusername }</p>
		<br />
		</div>	
		...
		<div>
          ...
          <span  id="onlyone"><input name="wmfrequency" type="radio" value="once" onclick="toQuency(this.value)" <%=onceChecked %>  />立即发起 </span>
           <input name="wmfrequency" type="radio" value="day" onclick="toQuency(this.value)" <%=dayChecked %>/>每日
    	...
    	<div id="page"  style="display:none;height:auto; ">
    		...
    		<logic:present name="IGDBM13011VO" property="monthList">
				<logic:iterate id="bean" name="IGDBM13011VO" property="weekList" indexId="index">
					<c:if test="${bean.ischecked != null }"><input type="checkbox" style="border-color: red;" name="rcfrequencyweek" id="wmfrequencyweek" data-on="${bean.weekdata }" data-off="${bean.weekdata }" value="${bean.weekvalue }" checked="checked"/></c:if>
			...
			<logic:present name="IGDBM13011VO" property="monthList">
				<logic:iterate id="bean" name="IGDBM13011VO" property="monthList" indexId="index">
					<c:if test="${bean.ischecked != null }">
						<input type="checkbox" name="wmfrequencymonth" id="wmfrequencymonth" data-on="${bean.dayvalue }" data-off="${bean.dayvalue }" value="${bean.dayvalue }" checked="checked" />
					</c:if>
					<c:if test="${bean.ischecked == null }">
						<input type="checkbox" name="wmfrequencymonth" id="wmfrequencymonth" data-on="${bean.dayvalue }" data-off="${bean.dayvalue }" value="${bean.dayvalue }" />
					</c:if>
				</logic:iterate>
			</logic:present>
			</div>
		...
		<input  type="checkbox" name="wmfrequencymonthlastday" id="wmfrequencymonthlastday" data-on="是" data-off="否" value="1" checked="checked"/>
		</c:if>
		<c:if test="${IGDBM1302Form.wmfrequencymonthlastday == null }">
		<input type="checkbox" name="wmfrequencymonthlastday" id="wmfrequencymonthlastday" data-on="是" data-off="否" value="1"/>
		</c:if></span>	
		</div>
...
<%-- 		<html:hidden property="wmid" styleId="wmid" /> --%>
		...
		<html:hidden property="wmcron" styleId="wmstatus" />
