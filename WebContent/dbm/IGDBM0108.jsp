...
<html:html>
<bean:define id="id" value="IGDBM0108" toScope="request" />
<bean:define id="title" value="工作流程综合分析" toScope="request" />
...
<script type="text/javascript">
         ...
         var xiSwfUrlStr = "images/dbmswf/ProcessWork.swf";
         var flashvars = {};
         var params = {};
         var attributes = {};
         attributes.id = "processWork";
         attributes.name = "processWork";
         params.wmode = "Opaque";
     	...
     	function quit(){
 		 ...
 		 function getFlexSubmitValue(typeValue,yearValue,pn1Value,pn2Value,pn3Value,pn4Value){
	   		 ...
	   		 var objAjax = new Ajax.Request(getAppRootUrl() + ajaxAction,{asynchronous:false, 
	   				parameters: 'type=processWork&&requestMode=allMode&typeValue='+
 		 ...
 		 function getProcessNavigateValue(typeValue,yearValue,navigateValue,navigateType){
    		 ...
    		 var res = "";
    		 var objAjax = new Ajax.Request(getAppRootUrl() + ajaxAction,{asynchronous:false, 
    				parameters: 'type=processWork&requestMode=navigateValue&typeValue='+
    			...
    			if(navigateType =='pn1'){ //设置月工作类型数量及占比
    				setPieGraph(res.split('|')[0],res.split('|')[1],res.split('|')[2],res.split('|')[3]);
    			}
    			if(navigateType =='pn2'){ //设置工作平均解决时间趋势
    				setProcessLine2(res);
    			}
    			if(navigateType =='pn3'){//设置工作数量趋势
    				setProcessLine1(res);
    			}
    			if(navigateType =='pn4'){//设置工作及时解决率趋势
    				setProcessLine3(res);
    			}
 		 }
         ...
         function setPieGraph(p1value,p2value,p3value,p4value){
        	 processWork.setPieGraph(p1value,p2value,p3value,p4value);
         }
         
      	 //设置工作数量趋势
         function setProcessLine1(strXml){
        	 processWork.setProcessLine1(strXml);
         }
         
       //设置工作平均解决时间趋势
         function setProcessLine2(strXml){
        	 processWork.setProcessLine2(strXml);
         }
         
       //设置工作及时解决率趋势
         function setProcessLine3(strXml){
            processWork.setProcessLine3(strXml);
         }
