...
<html:html>
<bean:define id="id" value="IGDBM0105" toScope="request" />
<bean:define id="title" value="问题流程综合分析" toScope="request" />
...
<script type="text/javascript">
         ...
         var xiSwfUrlStr = "images/dbmswf/ProcessProblem.swf";
         var flashvars = {};
         var params = {};
         var attributes = {};
         attributes.id = "processProblem";
         attributes.name = "processProblem";
         params.wmode = "Opaque";
     	...
     	function quit(){
 		 ...
 		 function getFlexSubmitValue(typeValue,yearValue,pn1Value,pn2Value,pn3Value,pn4Value){
	   		 ...
	   		 var objAjax = new Ajax.Request(getAppRootUrl() + ajaxAction,{asynchronous:false, 
	   				parameters: 'type=processProblem&&requestMode=allMode&typeValue='+
 		 ...
 		 function getProcessNavigateValue(typeValue,yearValue,navigateValue,navigateType){
    		 ...
    		 var res = "";
    		 var objAjax = new Ajax.Request(getAppRootUrl() + ajaxAction,{asynchronous:false, 
    				parameters: 'type=processProblem&requestMode=navigateValue&typeValue='+
    			...
    			if(navigateType =='pn1'){ //设置月问题类型数量及占比
    				setPieGraph(res.split('|')[0],res.split('|')[1],res.split('|')[2],res.split('|')[3]);
    			}
    			if(navigateType =='pn2'){ //设置问题平均解决时间趋势
    				setProcessLine2(res);
    			}
    			if(navigateType =='pn3'){//设置问题数量趋势
    				setProcessLine1(res);
    			}
    			if(navigateType =='pn4'){//设置问题及时解决率趋势
    				setProcessLine3(res);
    			}
 		 }
         ...
         function setPieGraph(p1value,p2value,p3value,p4value){
        	 processProblem.setPieGraph(p1value,p2value,p3value,p4value);
         }
         
      	 //设置问题数量趋势
         function setProcessLine1(strXml){
        	 processProblem.setProcessLine1(strXml);
         }
         
       //设置问题平均解决时间趋势
         function setProcessLine2(strXml){
        	 processProblem.setProcessLine2(strXml);
         }
         
       //设置问题及时解决率趋势
         function setProcessLine3(strXml){
            processProblem.setProcessLine3(strXml);
         }
