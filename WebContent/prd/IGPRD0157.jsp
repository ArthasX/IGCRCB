...
<html:html>
<bean:define id="id" value="IGPRD0157" toScope="request" />
...
<script type="text/javascript">	
	function toback(pdid,psdid,psdcode) {
	 ...
	 function add(pdid,psdid,psdcode){
	     form.action = getAppRootUrl() + "/IGPRD0141_Disp.do?ppsdid=${IGPRD0141Form.ppsdid}&pdid="+pdid+"&psdid="+psdid + "&psdcode="+psdcode;
	     form.submit();
	 }
</script>
<body>
...
<div class="msg_box"><html:form styleId="form" action="/IGPRD0126_List"
	...
	<div class="msgbtn">
		<logic:equal name="form" property="mode" value="0">
		      <html:submit property="btn_add_conf" value="继续新增" styleClass="button" onclick="add('${pdid}','${psdid}','${psdcode}');"/>
               <html:submit property="btn_next" value="返回" styleClass="button" onclick="toback('${pdid}','${psdid}','${psdcode}')"/>
	    </logic:equal>
	    <logic:equal name="form" property="mode" value="1">
	         <html:submit property="btn_next" value="修改成功返回" styleClass="button" onclick="toback('${pdid}','${psdid}','${psdcode}')"/>
	    </logic:equal>
	</div>
</html:form></div>
</div>
