...
<html:html>
<bean:define id="id" value="IGPRD0111" toScope="request" />
...
function showDIV(obj){
	...
	if(obj.value == "2" || obj.value == "B" || obj.value == "E" || obj.value == "C" || obj.value == 'S'){
		...
		document.getElementById("processDefinition").style.display = "none";
		...
		if(obj.value == "5"){
			...
			document.getElementById("pidrows").value = "1";
			document.getElementById("processDefinition").style.display = "";
			setProcess();
		}else{
			document.getElementById("processDefinition").style.display = "none";
			document.getElementById("processRows").style.display = "none";
...
function setDefaultDIV(){
	...
	if($F("mode") == "1"){
		if($F("pidtype") == "2" || $F("pidtype") == "B" || $F("pidtype") == "E" || $F("pidtype") == "C" || $F("pidtype") == "S"){
			...
			if($F("pidtype") == "P"){
				document.getElementById("participantRoleTypeDiv").style.display = "";
				var participantRoles = "${IGPRD0111Form.participantRoles}";
				var roletype = "${IGPRD0111Form.participantRoleType}";
				if(participantRoles != null && participantRoles != ""){
					var infos = participantRoles.split("_#_");
					var hashArr = new Array();
					for(var i=0;i<infos.length;i++){
						var info = infos[i].split("_sp_");
						hashArr[info[1]] = info[0];
					}
					changeRoleType(roletype,hashArr);
				}
			}else{
				document.getElementById("participantRoleTypeDiv").style.display = "none";
			}
 ...
 function toback(){
	 IGPRD0111Form.action = getAppRootUrl() + "/IGPRD0110_Back.do";
 ...
 function checkForm(){
	...
	if($F("jsfunction") != ""){
 	...
 	if(document.getElementById("pidtype").value == "P"){
 		var roleinfoObj = document.getElementById("role_to");
 		if(roleinfoObj.options.length > 0){
 			var value = "";
 			for(var i=0;i<roleinfoObj.options.length;i++){
 				if(i > 0){
 					value += "_#_";
 				}
 				value += roleinfoObj.options[i].text + "_sp_" + roleinfoObj.options[i].value;
 			}
 			document.getElementById("participantRoles").value = value;
 		}
 	}
 	...
 	if($F('pidoption').trim().strlen()>5000){
		alert("取值范围不能大于"+Math.floor(5000/strByteFlag)+"个汉字！");
		return false;
	}
...
function setEntity(url){
	...
	jq(document).ready(function(){
		...
		jq("#info_add").click(function(){
			jq("#role_from option:selected").appendTo("#role_to");
		});
		//添加全部
		jq("#info_addAll").click(function(){
			jq("#role_from option").appendTo("#role_to");
		});
		//删除
		jq("#info_del").click(function(){
			jq("#role_to option:selected").appendTo("#role_from");
		});
		//删除全部
		jq("#info_delAll").click(function(){
			jq("#role_to option").appendTo("#role_from"); 			
		});
		//双击添加
		jq("#role_from").dblclick(function(){
			jq("#role_from option:selected").appendTo("#role_to");
		});
		//双击删除
		jq("#role_to").dblclick(function(){
			jq("#role_to option:selected").appendTo("#role_from");
		});
	});
	function changeRoleType(roletype,hashArr){
		var role_from = document.getElementById("role_from");
		var role_to = document.getElementById("role_to");
		role_from.options.length = 0;
		role_to.options.length = 0;
		
		//设置请求的Action
		igAjax.ajaxAction = "//IGPRD0111_Ajax.do";
		...
		igAjax.back = function(req, json){
			 ...
			 if(result != null && result != ""){
				 var infos = result.split("_#_");
				 var optionItem = null;
				 for(var i=0;i<infos.length;i++){
					 var info = infos[i].split("_sp_");
					 if(hashArr != null && hashArr[info[1]] != null && hashArr[info[1]] != ""){
						 optionItem = new Option(hashArr[info[1]],info[1]);
						 role_to.options.add(optionItem);
					 }else{
						 optionItem = new Option(info[0],info[1]);
						 role_from.options.add(optionItem);
					 }
				 }
			 }
		};
	...
	function setProcess(){
		var obj = document.getElementById("process_pdinfo");
		obj.options.length = 1;
		var par = {};
	    par.pdid = document.getElementById("pdid").value;
	    jQuery.ajax({
			url:"IGPRD0111_GetProcessDefinitions.do",
			type:"POST",
			dataType :"json",
			data:par,
			cache:false,
			success:function(data){
				...
				if(data != null && data.length > 0){
					for(var i=0;i<data.length;i++){
						var item = new Option(data[i].label,data[i].value);
						if(data[i].value == ccid){
							item.selected = true;
						}
...
<div id="location">
<div class="image"></div>
<p class="fonts1"><ig:navigation /> &gt;&gt; 表单管理 &gt;&gt; 
<logic:equal name="IGPRD0111Form" property="mode" value="0">表单登记</logic:equal>
<logic:equal name="IGPRD0111Form" property="mode" value="1">表单变更</logic:equal>
...
<div id="formwrapper">
<html:form styleId="form" action="/IGPRD0111" enctype="multipart/form-data" onsubmit="return checkSubmit(this);" >
	<!--  message -->
	<ig:message />
	<!--  /message -->
	...
	<br />
     ...
     <div style="margin-left:0px;">
     	...
     	<html:select property="participantRoleType" name="IGPRD0111Form" styleId="participantRoleType" style="width:205px;" onchange="changeRoleType(this.value,null);">
     ...
     <table>
     	<tr>
     		<td>
     			<select size="15" id="role_from" style="width:207px;"></select>
     		</td>
     		<td>
	     		<input type="button" style="width:30px;" id="info_add" title="添加" value=">"/><br />
		       	<input type="button" style="width:30px;" id="info_addAll" title="全部添加" value=">>"/><br />
	        	<input type="button" style="width:30px;" id="info_del" title="删除" value="<"/><br />
		       	<input type="button" style="width:30px;" id="info_delAll" title="全部删除" value="<<"/><br />
     		</td>
     		<td>
     			<select size="15" id="role_to" style="width:207px;"></select>
     		</td>
     	</tr>
     </table>
     </div>
	...
	<html:hidden property="pidoption" name="IGPRD0111Form"/>
	<html:hidden property="participantRoles" name="IGPRD0111Form"/>
	<html:hidden property="piddefault" styleId="piddefault" name="IGPRD0111Form"/>
	...
	<div id="processDefinition" style="display:none;">
		<label for="Name"><strong>流程定义</strong>：</label>
		<select name="process_pdinfo" id="process_pdinfo" onchange="setCcid(this.value);">
			<option></option>
		</select>
	</div>
