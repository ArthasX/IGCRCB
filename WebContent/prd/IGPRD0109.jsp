...
<html:html>
<bean:define id="id" value="IGPRD0109" toScope="request" />
...
<script type="text/javascript">	
		...
		{
			curnum=obj.id;
			openSubWindow('/IGPRD0113_Disp.do?pidmode=0&pdid='+pdid+'&tableColumn=N', 'newpage', '800', '600');
		}
			...
			for(var i=0;i<number;i++){
				if(document.getElementsByName("imgname")[i].id==curnum){
					if(type==7){
						...
						var tempoption = tempselectedoptions();
						tb.deleteRow(curnum);
						currRow=tb.insertRow(curnum);
						cell=currRow.insertCell();
						h3 = "<img id='"+curnum+"' name='imgname' src='images/tree.gif' style='cursor: hand;' alt='选择流程变量' width='16' height='16' border='0' onClick='selectVariable(\""+"<bean:write name='IGPRD0109Form' property='pdid'/>"+"\", this);'/>";
						p="<select name='vvalue'>"+tempoption+"</select>";
						h = "<input style='margin-left: 165' type='button' class='delbutton' value='删除' onclick='deleteitem(this);deleteVariable()'>";
						v = "<select name='logicSign' style='width: 80px'><option value="+<%=IGPRDCONSTANTS.LOGICSIGN1 %>+">等于</option><option value='"+<%=IGPRDCONSTANTS.LOGICSIGN2 %>+"'>不等于</option></select>";
		...
		function tempselectedoptions(){
			var temp = '<option value=""></option>';
			var objAjax = new Ajax.Request(getAppRootUrl() + ajaxAction,{asynchronous:false, 
				parameters: 'type=processOrgSelectedOptions&pdid=<bean:write name="IGPRD0109Form" property="pdid"/>',
...
<div id="content"><html:form action="/IGPRD0109" styleId="form"
	...
	<div id="location">
	<div class="image"></div>
	<p class="fonts1"><ig:navigation /> &gt;&gt; 状态列表 &gt;&gt; 规则定义</p>
	<div class="back">
		<a href="IGPRD0104_Back.do?pdid=${IGPRD0109Form.pdid}&ppsdid=${IGPRD0109Form.ppsdid}" target="_self">返回</a>
	</div> 
	</div>
			...
			<logic:iterate id="map" name="IGPRD01091VO" property="processParticipantButtonDefMap">
				...
				<logic:equal name="role_id" value="${bean.roleid}">
					...
					<logic:iterate id="buttondef" name="list">
						<logic:present name="buttondef" property="processButtonDefTB.pbdflag">
	...
	<html:hidden property="ppsdid" styleId="ppsdid" name="IGPRD0109Form"/>
