...
<html:html>
<bean:define id="id" value="IGPRD0108" toScope="request" />
...
function pidrequiredCreate(){
	...
	for(var i=0;i < requireds.length;i++){
	 ...
	 function toback(){
		 IGPRD0108Form.action = getAppRootUrl() + "/IGPRD0107_Disp.do?psdid="+IGPRD0108Form.psdid.value+"&ppdid="+IGPRD0108Form.ppdid.value+"&pdid="+IGPRD0108Form.pdid.value+"&ppsdid="+IGPRD0108Form.ppsdid.value;
...
<div id="location">
<div class="image"></div>
<p class="fonts1">系统管理 &gt;&gt; 流程定义 &gt;&gt; 流程管理 &gt;&gt; 状态列表 &gt;&gt; 参与者 &gt;&gt; 权限</p>
<div class="back"><a href="javascript:toback();" target="_self">返回</a></div> 
</div>
	<div id="results_list"><!--  message --> <ig:message /> <!--  /message -->
	<div class="role_title">当前状态：${IGPRD01081VO.processStatusDef.psdname}</div>
	<table id="tab_del" class="table_style13" align="left" <c:if test="${IGPRD01081VO.processStatusDef.psdtype=='6'}">style="display:none"</c:if>>
			<tr>
						...
						<logic:present name="IGPRD01081VO" property="processParticipantButtonDefVWInfoList">
							<logic:iterate id="ppbdvw" name="IGPRD01081VO" property="processParticipantButtonDefVWInfoList">
								...
								<logic:notEqual name="psd" property="psdcode" value="<%=IGPRDCONSTANTS.PROCESS_START_STATUS %>">
									<bean:define id="flag" value="0"/>
									<logic:equal name="ppbdvw" property="pbdid" value="<%=IGPRDCONSTANTS.BUTTON_TERMINATE %>">
										<bean:define id="flag" value="1"/>
									</logic:equal>
								...
								<logic:empty name="ppbdvw" property="ppbdid">
									...
									<logic:equal name="ppbdvw" property="pbdid" value="<%=IGPRDCONSTANTS.BUTTON_UPDATE %>">
										<html:text property="ppbdname" size="10" value="保存" />
										提示信息：
										<html:text property="ppbddesc" size="10" value="保存" />
									</logic:equal>
...
<div id="processDefinitionDiv" style="width:98%;margin:5px auto 10px;display:none">
	...
	<div id="results_list">
	<table class="table_style13" align="left" id="table" width="100%">
			<tr>
				<th width="30%">表单名称</th>
				<th width="50%">权限</th>
				<th width="20%">必填</th>
	...
	<div id="operate" style="padding-left:325px;">
	<logic:present name="IGPRD01081VO" property="processParticipantDef">
		<span> 
		<%if(!IGPRDCONSTANTS.PROCESSDEFINITION_IS_IN_USE.equals(request.getAttribute("isInUse").toString())){ %>
		<html:submit property="btn_delete" value="提交" styleClass="button"  onclick="return check();"/>
	...
	<html:hidden property="ppsdid" styleId="ppsdid" value="${IGPRD0108Form.ppsdid}"/>
