...
<html>
<bean:define id="id" value="IGCRC1201" toScope="request" />
<bean:define id="title" value="文档借阅发起" toScope="request" />
<!-- header1 -->
...
function checkForm(action,vbrequired){
	...
	if(getFormObj("文档类型").value.indexOf("纸质") > 0){
		if(null == getFormObj("预计归还时间") || getFormObj("预计归还时间").value == ""){
			alert("请输入预计归还时间");
			return false;
		}
	}
...
function selectEntity(index, pidid,syscoding){
	num = index;
	openSubIGCRC1302('?pidid='+pidid + '&esyscoding=' + syscoding);
	var a = document.getElementsByName("prtitle")[0].value;
}

function setParamIGCOM0302(eid, elabel,ename,eiid, eilabel, einame, ecategory, pidid, eiversion) {
	//alert("eid:"+eid+"|elabel:"+elabel+"|ename:"+ename+"|eiid:"+eiid+"|eilabel:"+eilabel+"|einame:"+einame+"|ecategory:"+ecategory+"|pidid:"+pidid+"|eiversion:"+eiversion);
	...
	if("" != eiid){
		//ajax请求数据
		jQuery.ajax({
			url:"IGASM0504_Ajax.do?eiid="+eiid,
			type:"POST",
			dataType :"json",
			cache:false,
			//成功回调函数
			success:function(data){
				//为文本域 文档描述 赋值
				document.getElementsByName("pivarvalue[3]")[0].value = data[0].eidesc;
				document.getElementById("pidid0112001004").style.display = "none";
				document.getElementById("pidid0112001004").insertAdjacentHTML("afterEnd",data[0].eidesc);
				
				for(var i=0;i<data.length;i++){
					//如果细项的值为null,修改为空
					if(null == data[i].civalue){
						data[i].civalue = "";
					}
					//将流程中不存在的表单进行过滤
					if(data[i].cname != '是否可下载' && data[i].cname != '归档部门'){
						if(data[i].cname == '文档分类'){
							$("pivarvalue[5]").value = data[i].civalue;
							$("pivartreename[5]").value = data[i].civalue.substring(15);
							document.getElementById("pidid0112001015").style.display = "none";
							document.getElementsByTagName("img")[7].style.display = "none";
							document.getElementById("pidid0112001015").insertAdjacentHTML("afterEnd",data[i].civalue.substring(15));
						}else{
							getFormObj(data[i].cname).value = data[i].civalue;
							document.getElementById(getFormObj(data[i].cname).id).style.display = "none";
							document.getElementById(getFormObj(data[i].cname).id).insertAdjacentHTML("afterEnd",data[i].civalue);
							if(data[i].cname == '文档类型' && data[i].civalue.indexOf('纸质') > 0){
								for(var t=0;t<document.getElementsByTagName("label").length;t++){
									if(document.getElementsByTagName("label")[t].innerText == '预计归还时间：'){
										document.getElementsByTagName("label")[t].outerHTML = "<LABEL style=\"FLOAT: right\" for=Name><STRONG><SPAN class=red>*</SPAN>预计归还时间：</STRONG></LABEL>";
									}
								}
							}
						}
 ...
 */
function showItem(){
	if(document.getElementsByName("entityName[0]")[0]==null || document.getElementsByName("entityName[0]")[0].value == ""){
		getFormObj("文档描述").parentElement.parentElement.style.display = "none";
		getFormObj("文档类型").parentElement.parentElement.style.display = "none";
		getFormObj("文档密级").parentElement.parentElement.style.display = "none";
		getFormObj("文档编号").parentElement.parentElement.style.display = "none";
		getFormObj("保密性要求").parentElement.parentElement.style.display = "none";
		getFormObj("存放位置").parentElement.parentElement.style.display = "none";
	}else{
		getFormObj("文档描述").parentElement.parentElement.style.display = "";
		getFormObj("文档类型").parentElement.parentElement.style.display = "";
		getFormObj("文档密级").parentElement.parentElement.style.display = "";
		getFormObj("文档编号").parentElement.parentElement.style.display = "";
		getFormObj("保密性要求").parentElement.parentElement.style.display = "";
		getFormObj("存放位置").parentElement.parentElement.style.display = "";
	}
}
...
<div id="content"><html:form action="/IGPRR0101" styleId="form"
	...
	<fieldset>
	<legend>工单信息</legend>
	<table cellspacing="2" class="table_style1" id = "tab1">
...
<%-- 				<html:textarea property="prdesc" cols="60" rows="6"  errorStyleClass="inputError imeActive"></html:textarea></td> --%>
				<div class="editDiv" style="width:440px;min-height:85px;" contenteditable="true" onkeydown="editDivContent(this);" onblur="getProcessDescValue(this);">
				${IGPRR0101Form.prdesc }
				</div>
				<html:hidden property="prdesc" style="prdesc" name="IGPRR0101Form"/>
			</tr>
			</logic:equal>
