...
<html:html>
<bean:define id="id" value="IGCRC0503" toScope="request" />
					...
					<c:if test="${vo.processStatusDef.assignflag == '1'}">
					   		...
					   		for(var i=0;i<status${index}.length;i++){
						   		if(status${index}[i].type=="checkbox" &&
						   			...
						   			noHasStatusUser${index} ++;
						   		}
						   	}
					   		if(noHasStatusUser${index} == 0) {
						   		alert("请选择${vo.processStatusDef.psdname}参与者！");	
						       	return false;
							}
					   		if(noHasStatusUser${index} > 1) {
					   			alert("${vo.processStatusDef.psdname}参与者只能选择一个！");	
						       	return false;
					   		}
					</c:if>
				</logic:iterate>
