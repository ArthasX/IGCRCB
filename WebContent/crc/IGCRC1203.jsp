...
function initAttFormFile(){
	...
	if('待发起' == '<ig:processDefineTypeValue prid="${IGPRR01021VO.process.prid}"/>')
		showItem(true);
}
 ...
 * 将表单内容置为只读
 */
function showItem(isOnLoad){
	if(isOnLoad){
		var img = getFormObj("文档分类").parentElement.getElementsByTagName("img")[0];
		if(img != undefined)
			getFormObj("文档分类").parentElement.removeChild(img);
		var html = getFormObj("文档描述").innerHTML;
		var node = document.createElement("span");
		node.id = "info0";
		node.innerHTML = html;
		getFormObj("文档描述").parentElement.appendChild(node);
	    getFormObj("文档描述").style.display = "none";
		var array = new Array ("文档类型","文档分类","文档密级","文档文件","文档编号","文档保管期限","保密性要求","存储方式","存放位置");
		for(var i=0;i<array.length;i++){
			var value = getFormObj(array[i]).value;
			var node = document.createElement("span");
			node.id = "info"+(i+1);
			node.innerHTML = value;
			getFormObj(array[i]).parentElement.appendChild(node);
			getFormObj(array[i]).style.display = "none";
		}
	}else{
		var array = new Array("文档描述","文档类型","文档密级","文档编号","保密性要求","存放位置");
		for(var i=0;i<array.length;i++){
			getFormObj(array[i]).parentElement.parentElement.style.display = "block";
		}
	}
}
/**
 ...
 */
 function hideItem(){
	var array = new Array("文档描述","文档类型","文档密级","文档编号","保密性要求","存放位置");
	for(var i=0;i<array.length;i++){
		getFormObj(array[i]).parentElement.parentElement.style.display = "none";
	}
	removeItem(10);
}
/**
 * 将表单内容移除
 */
 function removeItem(length){
	 for(var i=0;i<length;i++){
			jQuery("#info"+i).remove();
		}
}
</script>
	...
	if("" != eiid){
		...
		jQuery.ajax({
			...
			success:function(data){
				...
				document.getElementById("pidid0112001004").insertAdjacentHTML("afterEnd","<span id='info0'>"+data[0].eidesc+"</span>");
				
// 				document.getElementsByName("prtitle")[0].value = data[0].eidesc+"_文档借阅";
				for(var i=0;i<data.length;i++){
					...
					if(data[i].cname != '是否可下载'){
						if(data[i].cname == '文档分类'){
							...
							document.getElementById("pidid0112001015").insertAdjacentHTML("afterEnd","<span id='info"+(i+1)+"'>"+data[i].civalue.substring(15)+"</span>");
						}else{
							getFormObj(data[i].cname).value = data[i].civalue;
							document.getElementById(getFormObj(data[i].cname).id).style.display = "none";
							document.getElementById(getFormObj(data[i].cname).id).insertAdjacentHTML("afterEnd","<span id='info"+(i+1)+"'>"+data[i].civalue+"</span>");
