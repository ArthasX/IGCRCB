...
<bean:define id="id" value="IGCRC0202" toScope="request" />
...
//确认按钮
function insteadComfirm(){
	
	if(confirm("是否确认提交？")){
		var psdid = '${IGPRR01021VO.status}';//流程节点
		var prid = '${IGPRR01021VO.process.prid}';//流程id
		var selectUserid = jQuery('#insteadPpusername option:selected').val();//所选用户
		//获取故障原因内容
		var becauseVal = jQuery("#pidid"+pdid+"030").html();
		//获取处理方法内容
		var dealVal = jQuery("#pidid"+pdid+"049").val();
		//获取原因分析内容
		var causeVal = jQuery("#pidid"+pdid+"039").val();
		var objAjax = new Ajax.Request(getAppRootUrl() + ajaxAction,{asynchronous:false, method:'POST',
			parameters: 'type=confirmPpsubstatus&psdid='+psdid+'&prid='+prid+'&selectUserid='+selectUserid+'&becauseVal='+becauseVal+'&dealVal='+dealVal+'&causeVal='+causeVal,
		...
		location.href="IGPRR0102_Disp.do?prid="+prid;
	}
...
function upgrade(){
	if(confirm("是否确认升级？")){
		...
		var res = "";
		var objAjax = new Ajax.Request(getAppRootUrl() + ajaxAction,{asynchronous:false, method:'POST',
			parameters: 'type=upgrade&prid='+prid,
			onSuccess:  function(req, json){
				 var result=req.responseText;
					if(result != null && result != ""){
						res = result;
		...
		//如果升级成功隐藏按钮
		if(res == 1){
			jQuery("#upgradebtn").hide();
			location.href="IGPRR0102_Disp.do?prid="+prid;
		}else{
			jQuery("#upgradebtn").show();
		}
...
//获取参与者子状态
function getPpsubstatus(){
	var psdid = '${IGPRR01021VO.status}';//流程节点
	var prid = '${IGPRR01021VO.process.prid}';//流程id
	var userid = '${IGPRR01021VO.userid}';//用户id
	var source = getFormObj("事件来源").value;
	var res = "";
	var objAjax = new Ajax.Request(getAppRootUrl() + ajaxAction,{asynchronous:false, method:'POST',
		parameters: 'type=getPpsubstatus&psdid='+psdid+'&prid='+prid+'&userid='+userid+'&source='+source,
		onSuccess:  function(req, json){
			 var result=req.responseText;
				if(result != null && result != ""){
					res = result;
	...
	var prstatus = '${ IGPRR01021VO.process.prstatus}';//节点状态
	var array = res.split("##");
	var roleIdFlag = array[0];//是否是服务台角色 是:1 否:0
	var userjson =  eval("(" + array[1] + ")"); //用户列表
	var showFlag = 0;//下拉框是否显示(是否还有未确认用户) 是:1 否:0
	var updateFlag = array[2];//是否可以升级 是:1 否:0
	var alarmRoleIdFlag = array[3];//是否是监控告警 是:1 否:0
	
	var selectObj = jQuery("#insteadPpusername");//下拉列表
	
	for(var i = 0; i < userjson.length;i++){
		if(userjson[i].ppstatus == 0){//如果确认状态为0时,添加可选列表
			showFlag = 1;
			if(userid == userjson[i].ppuserid){
				selectObj.append("<option value='"+userjson[i].ppuserid+"' selected='selected'>"+ userjson[i].ppusername +"</option>");
			}else{
				selectObj.append("<option value='"+userjson[i].ppuserid+"'>"+ userjson[i].ppusername +"</option>");
			}
		}
	}
	...
	//判断事件如果是监控告警事件
	if(source == "监控告警"){
		//如果是处理节点,判断代理人是否为运行值班岗,判断是否显示
		if(prstatus == 'W' && alarmRoleIdFlag == 1 && showFlag == 1){
			jQuery("#insteadPeople").show();
			jQuery("#confirmbtn").show(); //运行值班岗 确认按钮
			jQuery("#btns").hide();
		}else{
			jQuery("#insteadPeople").hide();
			jQuery("#confirmbtn").hide();
			jQuery("#btns").show();
		}
	}else{//如果是正常事件
		//如果是处理节点,判断代理人列表是否显示(服务台角色),判断显示按钮
		if(prstatus == 'W' && roleIdFlag == 1 && showFlag == 1){
			jQuery("#insteadPeople").show();
			jQuery("#confirmbtn").show();//服务台 确认按钮
			jQuery("#btns").hide();
		}else{
			jQuery("#insteadPeople").hide();
			jQuery("#confirmbtn").hide();
		...
		//如果是处理节点且是服务台角色
		if(prstatus == 'W' && roleIdFlag == 1 && updateFlag == 1){
			jQuery("#upgradebtn").show();
		}else{
			jQuery("#upgradebtn").hide();
		}
...
<body onload="initAttFormFile();initFlowChart();getPpsubstatus();">
...
<div id="content"><html:form action="/IGPRR0102" styleId="form"
	...
	<logic:present name="IGPRR01021VO" property="processInfoDefmap">
		...
		<table cellspacing="2" class="table_style1">
			...
			<bean:define id="process" name="IGPRR01021VO" property="process"/>
			
			<tr id="insteadPeople" style="display: none">
				<th><label for="Name"><span class="red">*</span> <strong>代替确认人：</strong></label></th>
				<td>
					<select id="insteadPpusername">
					</select>
				</td>
				<td></td>
				<td></td>
...
<%-- 	</logic:notEqual> --%>
	...
	<div class="action" id="confirmbtn" style="display: none;">
		<img src="images/gengxin.gif" width="16" height="16" />
		<a href="###" onclick="insteadComfirm()">确认</a>
	</div>
	<div class="action" id="upgradebtn" style="display: none;">
		<img src="images/gengxin.gif" width="16" height="16" />
		<a href="###" onclick="upgrade()">升级</a>
	</div>
	<div id="btns">
