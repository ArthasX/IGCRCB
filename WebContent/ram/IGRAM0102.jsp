...
<html>
<bean:define id="id" value="IGRAM0102" toScope="request" />
...
function packagingTableValue(){
	...
	if(pidids != null){
		for(var i=0;i<pidids.length;i++){
			var tvalue = values[i].rownumber + "";
			if(values[i].rownumber && tvalue.indexOf("#") < 0){
...
<%-- 		<html:text property="prusername" size="18" errorStyleClass="inputError imeActive"  readonly="true"></html:text>
 --%>		<p class="p2">&nbsp;&nbsp;<bean:write name="IGPRR0101Form" property="prusername"/></p>
	...
	<logic:present name="IGPRR01011VO" property="processTitleDefTB">
			...
			<tr>
				...
				<%-- <div class="editDiv" style="width:440px;" contenteditable="true" onkeydown="editDivContent(this);" onblur="getProcessDescValue(this);">
	...
	<% 
			Map<String,List<IG007Info>> fenLeiMap = new LinkedHashMap<String,List<IG007Info>>(); 
			String fenleiName = "";
			String fenleiStr = "";
			Integer prjs = 1;
			List<IG007Info> keyList = new ArrayList<IG007Info>();
		%>
		...
		<% 
			for(int i = 0; i < keyList.size(); i++){
				IG007Info ig007 = keyList.get(i);
				if(StringUtils.isNotEmpty(ig007.getPidunit())){
					if(fenLeiMap.get(ig007.getPidunit()) == null){
						fenLeiMap.put(ig007.getPidunit(), new ArrayList<IG007Info>());
						fenLeiMap.get(ig007.getPidunit()).add(ig007);
					}else{
						fenLeiMap.get(ig007.getPidunit()).add(ig007);
					}
				}
		...
		%>
		<c:set var="groupIndex" value="1"></c:set>
<fieldset>
	<legend>评估内容</legend>
	<br/>
	<table cellspacing="2" class="table_style1_1">
		<tr>
			 <th width="8%">评分分类</th> <th width="45%" >评分项</th><th width="100%"></th><!-- <th width="7%"></th><th width="15%"></th><th width="35%"></th> -->
		</tr>	
		<%
			Iterator iter = fenLeiMap.keySet().iterator();
			while(iter.hasNext()){
				String iterNext = String.valueOf(iter.next());
				%>
				<tr>
					<th rowspan="<%=fenLeiMap.get(iterNext).size() %>">
						<%=iterNext %>
					</th>
				<%
				List<IG007Info> prjList =  fenLeiMap.get(iterNext);
				for(int i = 0; i < prjList.size(); i++){
					...
					%>
						<td>
							${groupIndex } <%= prjList.get(i).getPidname() %> : <%= prjList.get(i).getPiddesc()%> 
							<c:set var="groupIndex" value="${groupIndex +1}"></c:set>
						</td>
						<td>
						<ig:formToTag 
						name="IGPRR0102Form" 
						pidid="<%=prjList.get(i).getPidid() %>" 
						property="pivarvalue[${groupIndex + prjpidid-1}]" 
						formType="${psid.pidtype }" 
						pidaccess="${psid.pidaccess }"
						pidmode="${psid.pidmode }"
						size="<%= textSize%>"	
						...
						align="${psid.align }"
						/>
						<html:hidden property="pivarname" value="<%=prjList.get(i).getPidname() %>" /> 
						<html:hidden property="pivarlabel" value="<%=prjList.get(i).getPidlabel() %>" /> 
						<html:hidden property="pivartype" value="<%=prjList.get(i).getPidtype()%>" /> 
						<html:hidden property="pidid" value="<%=prjList.get(i).getPidid() %>" />
						<html:hidden property="pidaccess" value="<%=prjList.get(i).getPidaccess() %>" />
						<html:hidden property="piid" value="<%=String.valueOf(prjList.get(i).getPiid()) %>" />
						<html:hidden property="pidmode" value="<%=prjList.get(i).getPidmode() %>" />
						<html:hidden property="privatescope" value="<%=prjList.get(i).getPrivatescope() %>" />
						</td>
						</tr>
					<%
				}
