...
<html:html>
<bean:define id="id" value="IGLOM0108" toScope="request" />
<bean:define id="title" value="部门接待汇总画面" toScope="request" />
<!-- header1 -->
...
<script type="text/javascript">	
	...
	function checkSearch(){
		...
		if(Number($F('month_from')) > Number($F('month_to'))){
			...
			document.forms[0].action = getAppRootUrl() + "/IGLOM0108.do";
	...
	function exportExcel() {
		...
		if(Number($F('month_from')) > Number($F('month_to'))){
			...
			document.forms[0].action = getAppRootUrl() + "/IGLOM0108.do?report=1";
			document.forms[0].submit();
		}
...
<div id="contentWrap">
<div id="content"><html:form styleId="form" action="/IGLOM0108"
	onsubmit="return checkSubmit(this);">
	<div id="location">
	<div class="image"></div>
	<p class="fonts1">后勤管理 &gt;&gt; 接待管理 &gt;&gt; 部门接待汇总</p>
	</div>
	<div id="search">
	<div class="img"></div>
	<div class="conditions">&nbsp;&nbsp;&nbsp;&nbsp;<span class="red">*</span>年度：
	<html:select property="year" name="IGLOM0108Form">
	...
	<table width="100%" class="table_style">
		...
		<tr>
			...
			<th width="12%">结束数量</th>
			<th width="12%">未结束数量</th>
			<th width="10%">费用总额</th>
		</tr>
		<!-- body部 -->
		<logic:present name="IGLOM01081VO" property="recptionOrgVWList">
			<logic:iterate id="bean" name="IGLOM01081VO"
				property="recptionOrgVWList" indexId="index">
				<tr onmouseover="mouseOver(this);" onmouseout="mouseOut(this,'');"
					onclick="doLook('IGLOM0113.do?rapporgid_org=${bean.rapporgid}&year_org=${IGLOM0108Form.year}&month_from_org=${IGLOM0108Form.month_from}&month_to_org=${IGLOM0108Form.month_to}');">

					<td>
					<div style="display: none">${bean.rapporgid}</div>
					</td>
					<!-- 申请部门 -->
					<td>
					...
					<!-- 结束数量 -->
					<td>
					<div class="nowrapDiv">${bean.disnum}</div>
					</td>
					<!-- 未结束数量 -->
					<td>
					<div class="nowrapDiv">${bean.ennum}</div>
					</td>
					<!-- 费用总额 -->
					<td>
			...
			<logic:iterate id="bean" name="IGLOM01081VO"
				property="recptionOrgVWList" length="1">
				<tr>
					<td></td>
					<td>合计:</td>
					<td align="center">${IGLOM01081VO.sumrnum}</td>
					<td align="center">${IGLOM01081VO.sumdisnum}</td>
					<td align="center">${IGLOM01081VO.sumennum}</td>
