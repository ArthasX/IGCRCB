...
<html:html>
<bean:define id="id" value="IGLOM0513" toScope="request" />
<bean:define id="title" value="食堂物品消耗画面" toScope="request" />
<!-- header1 -->
...
<script type="text/javascript">	
	...
	function operation(name,id){
		if(name=="search"){
			document.forms[0].action=getAppRootUrl() + "/IGLOM0512_Disp1.do";
			document.forms[0].submit();

		}else if(name=="insert"){
			
			 if(window.confirm("您确定要出库这些物品吗？")){
				 var count=0;
				...
				for(var i=0;i<check.length;i++){
					if(check[i].checked==true){
						if($F('goinnums['+i+']').trim()==""){
							alert("请填写物品编号为\""+seqcodes[i].value+"\"的物品的出库数量！");
							return false;
						}else if(!checkIsNumberForCSE($F('goinnums['+i+']'))){
							alert("物品编号为\""+seqcodes[i].value+"\"的物品的出库数量应该为正整数！");
							return false;
						}
						count = 1;
					}else{
				 ...
				 document.forms[0].action=getAppRootUrl() + "/IGLOM0512_Insert.do";
				 document.forms[0].submit();
			 } 
		}else if(name=="read"){
...
<div id="contentWrap">
<div id="content">
<html:form styleId="form" action="/IGLOM0512_Disp1">
<div id="location">
<div class="image"></div>
<p class="fonts1">后勤管理 &gt;&gt; 物品管理 &gt;&gt; 食堂物品消耗</p>
<div class="back"><a href="IGLOM0512_Disp.do" target="_self">返回</a></div> 
</div>
	<div id="search">
	<div class="img"></div>
...
<br>
<span>登记人：</span><span>${IGLOM0512Form.gorequsername}</span>&nbsp;&nbsp;
<span>接待工作名称：</span><span>${IGLOM0512Form.rname}</span>
...
<html:hidden property="rid"/>
			...
			<logic:present name="IGLOM02051VO" property="itemsCategoryList">
				<logic:iterate id="bean" name="IGLOM02051VO" property="itemsCategoryList" indexId="index">
					...
					<tr class="<ig:rowcss index="${index}"/>">
						...
						<td>
							<html:text name="IGLOM0512Form" property="goinnums[${ADcount}]"  size="10" errorStyleClass="inputError" />
	...
	<logic:present name="IGLOM02051VO" property="itemsCategoryList">
		<span> 
			<html:button property="btn_add" value="出库" styleClass="button" onclick="operation('insert')"  />
