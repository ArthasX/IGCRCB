...
<html:html>
<bean:define id="id" value="IGLOG0042" toScope="request" />
<bean:define id="title" value="规则条件模板录入" toScope="request" />
<!-- header1 -->
...
function addWHMFile(index) 
{
	...
	var priorities="${IGLOG0041VO.alarmPriorities}";
...
function resetpage(){
	window.location.href = getAppRootUrl() + '/IGLOG0043_DISP.do';
}

function toback(obj){
	if(obj == "1"){
		window.location.href = getAppRootUrl() + "/IGCOM0202.do?linkID=IGMTM0201";
	}else{
...
<div id="location">
<div class="image"></div>
<p class="fonts1">监控管理 &gt;&gt; 监控配置 &gt;&gt; 添加规则模板</p>
	<c:if test="${IGLOG0041VO.couldReturn == 'yes'}">
		<div class="back">
			<html:link href="javascript:toback(${IGLOG0042Form.isRealTimeCollect });">返回</html:link>
		</div> 
	</c:if>
</div>
<div id="formwrapper"><html:form styleId="form"
	action="/IGLOG0043_INSERT" onsubmit="return checkSubmit(this);" enctype="multipart/form-data">
	<!--  message -->
	<ig:message />
	<!--  /message -->
	<fieldset>
    ...
    <html:select name="IGLOG0042Form" property="typeid" style="width: 85px;" errorStyleClass="inputError" tabindex="4">
	...
	<p class="p2">&nbsp;&nbsp;
		<html:radio property="tempstate" styleId="tempstate1" value="1" >可用</html:radio>
		<html:radio property="tempstate" styleId="tempstate2" value="0">停用</html:radio>
	</p><br />
	</div>
	<div><label for="Name"><strong>备注</strong>：</label> 
	<html:textarea cols="45" rows="6"
		property="tempdesc" 
		errorStyleClass="inputError imeActive" /><br />
	</div>
	</fieldset>
	...
	<input name="add" type="button" class="button" value="添加" onclick="addWHMFile(${xh});" /><br/><br/>
	<table class="table_style" id="tb2">
		...
		<logic:present name="IGLOG0041VO" property="myTempList">
			<logic:iterate id="bean" name="IGLOG0041VO" property="myTempList" indexId="index">
				
				<tr class="<ig:rowcss index="${index}"/>">
				<c:set value="${index}" var="xh"></c:set>
					<td align="center">${index+1}</td>
					<td width="30%">
						<html:select name="IGLOG0042Form" styleId="priority" property="priority"
