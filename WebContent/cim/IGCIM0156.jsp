...
<html:html>
<bean:define id="id" value="IGCIM0156" toScope="request"/>
...
function treeDoubleClickFuncForBJ(eiid){
	...
	try{
		...
		setTimeout("treeDoubleClickFuncForBJ(" + treeEiid + ")",1000);
	}
}

//告警列表
function setWarnDGForBJ(treeEiid){
	try{
		var res="";
		var objAjax = new Ajax.Request(getAppRootUrl() + ajaxAction,{asynchronous:false, 
			parameters: 'type=WarnDGTopoForBJ&eiid='+treeEiid,
			 onSuccess:  function(req, json){
			 var result=req.responseText;
				if(result != null && result != ""){
					res=result;
		...
		setTimeout("setWarnDGForBJ("+treeEiid+")",1000);
	}
}
