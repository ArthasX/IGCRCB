...
<html:html>
<bean:define id="id" value="IGCIM0124" toScope="request"/>
        ...
        function toback(){
        	if(backurl){
        		...
        		if(backurl.indexOf("/IGCIM0150_Disp.do?readonly=1")>-1){
        			form.action+='&eiid=${IGCIM01241VO.entityData.eiid}';
        		}
        	}else{
	        	form.action = getAppRootUrl() + "/IGCIM0124.do?org.apache.struts.taglib.html.CANCEL=1";
        	}
    ...
    <div id="contentWrap">
        <div id="content">
            ...
            <div id="location">
                <div class="image"></div>
                <p class="fonts1">
                	<ig:navigation extname1="${urlable}"/> &gt;&gt; 应用配置信息
                </p>
                <div class="back">
		    ...
		    <html:form styleId="form" action="/IGCIM0124" enctype="multipart/form-data" onsubmit="return checkSubmit(this);">
                 <div class="title">
...
<c:if test="${num > 0}">        
<div class="title">
<div class="name"><bean:message bundle="asmResources" key="label.IGCIM0124.historyVersionInfo"/></div>
</div>
<div class="message"></div>              
    <div id="results_list">
    <table width="100%" class="table_style" >
		 <tr>
			<th width="50%"><bean:message bundle="asmResources" key="label.IGCIM0124.VersionINO"/></th>
			<th width="50%"><bean:message bundle="asmResources" key="label.IGCIM0124.ModefyTime"/></th>     
		 </tr>
			...
			<logic:iterate id="bean" name="IGCIM01241VO" property="configItemVersionTime"
				...
				<tr>
				       ...
				       <c:if test="${IGCIM01241VO.flag==true}">
				          <a href='javascript:openVersionInfoDetail(${IGCIM01241VO.entityData.eiid},${bean.civersion},${bean.cismallversion})'>${bean.civersion}<bean:message bundle="asmResources" key="label.IGCIM0124.Vesion"/></a>
				       </c:if>
				       <c:if test="${IGCIM01241VO.flag!=true}">${bean.civersion}<bean:message bundle="asmResources" key="label.IGCIM0124.Vesion"/> </c:if>
		       		</td>
		       		<td>
		          		${bean.ciupdtime}
		       		</td>
    	...
    	<logic:present name="IGCIM01241VO" property="configItemVersionTime">
				...
				<c:if test="${num > 1}">
			    	<html:button property="btn_compare" styleClass="button"  onclick="openEiVersionWindow('${IGCIM01241VO.entityData.eiid}');" >
			    		<bean:message bundle="asmResources" key="button.IGCIM0124.btn_compare"/> 
			    	</html:button>	
    			</c:if>
		</logic:present>
	</c:if>               
					...
					<logic:iterate id="bean" name="list" indexId="index">
						<tr onmouseover="mouseOverNoHand(this);" onmouseout="mouseOutNoHand(this);" <logic:equal name="bean" property="cseq" value="1">style="display: none;"</logic:equal>>
							...
							<logic:equal name="bean" property="cattach" value="2">
								<html:select property="civalue[${ADcount}]" style="width:53.5%;" styleClass="selBox" errorStyleClass="inputError imeActive">
...
<script type="text/javascript">
	jQuery(function(){
		if(backurl){
			jQuery(":file").attr("readonly","true");
			jQuery(".record_list img").hide();
			jQuery("[name^='civalue']").not(":textarea").attr("readonly","true").addClass("input1");
			jQuery(":submit:first").hide();
			jQuery(".selBox").attr("disabled","true");
		}
