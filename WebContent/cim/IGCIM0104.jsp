...
<div class="image"></div>
<p class="fonts1">
	<ig:navigation extname1="${urlable}"/> &gt;&gt; CI属性编辑
	</p>
...
<html:form styleId="form" action="/IGCIM0104"
		...
		<logic:iterate id="bean" name="IGCIM01041VO" property="configItemVersionTime"
			...
			<tr>
			       ...
			       <c:if test="${IGCIM01041VO.flag==true}">
			          <a href='javascript:openVersionInfoDetail_SOC(${IGCIM01041VO.entityData.eiid},${bean.civersion},${bean.cismallversion})'>${bean.civersion}<c:if test="${IGCIM01041VO.entityData.emodeltype == '1'}">.${bean.cismallversion}</c:if><bean:message bundle="asmResources" key="label.IGCIM0104.VesionInfo"/></a>
			       </c:if>
			       <c:if test="${IGCIM01041VO.flag!=true}">${bean.civersion}<c:if test="${IGCIM01041VO.entityData.emodeltype == '1'}">.${bean.cismallversion}</c:if><bean:message bundle="asmResources" key="label.IGCIM0104.VesionInfo"/></c:if>
	       		</td>
	       		<td>
	          		${bean.ciupdtime}
	       		</td>
