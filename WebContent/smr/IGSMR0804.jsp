...
<bean:define id="id" value="IGSMR0804" toScope="request" />
<bean:define id="title" value="核心网络系统运行情况页面" toScope="request" />
<!-- header1 -->
...
function goTo(){
	...
	var result;
		...
		var objAjax = new Ajax.Request(getAppRootUrl() + ajaxAction,{asynchronous:false, 
			 parameters: '&year='+year+'&pagesize='+ (parseInt(size)-1 < 0?0:parseInt(size)-1) + '&type=getRegulatoryPage',
...
//	 	if(size != 0){
// 		form.action = getAppRootUrl() + "/IGSMR0804_SEARCH.do?pagesize=" + (size-1);
// 		form.submit();
// 	}
}
...
function backTo(){
	...
	if((size+1)*4 <= countall_val){
		var objAjax = new Ajax.Request(getAppRootUrl() + ajaxAction,{asynchronous:false, 
			 parameters: '&year='+year+'&pagesize='+parseInt(size)+1+'&type=getRegulatoryPage',
			 onSuccess:  function(req, json){
				 result=req.responseText;
				 res_list.innerHTML= result.split("#_#")[0];				
		     }
...
<br/>
<div id="search">
<html:form action="/IGSMR0804_SEARCH" styleId="form">
	<div class="img"></div>
...
<div id="results_list">
	<table width="98%" border="0" align="center" cellspacing="0">
		<tr align="center">
			...
			background-repeat : repeat-x; border: 1px solid #D3D3D3;">核心网络系统运行情况</th>
			<th width="3%">&nbsp;</th>
		</tr>
