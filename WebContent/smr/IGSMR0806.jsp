...
<bean:define id="id" value="IGSMR0806" toScope="request" />
<bean:define id="title" value="银行卡系统运行情况页面" toScope="request" />
<!-- header1 -->
...
function goTo(){
	...
	var objAjax = new Ajax.Request(getAppRootUrl() + ajaxAction,{asynchronous:false, 
		 parameters: '&year='+year+'&pagesize='+ (parseInt(size)-1 < 0?0:parseInt(size)-1) + '&type=getBankCarPage',
...
// 	var size = document.getElementById("pagesize").value;
// 	if(size != 0){
// 		form.action = getAppRootUrl() + "/IGSMR0806_SEARCH.do?pagesize=" + (size-1);
// 		form.submit();
// 	}
}
...
function backTo(){
	...
	if((size+1)*4 <= countall_val){
		var objAjax = new Ajax.Request(getAppRootUrl() + ajaxAction,{asynchronous:false, 
			 parameters: '&year='+year+'&pagesize='+parseInt(size)+1+'&type=getBankCarPage',
			 onSuccess:  function(req, json){
				 result=req.responseText;
				 res_list.innerHTML= result.split("#_#")[0];				
		     }
...
<br/>
<div id="search">
<html:form action="/IGSMR0806_SEARCH" styleId="form">
	<div class="img"></div>
...
<div id="results_list">
	<table width="98%" border="0" align="center" cellspacing="0">
		<tr align="center">
			...
			background-repeat : repeat-x; border: 1px solid #D3D3D3;">银行卡系统运行情况</th>
			<th width="3%">&nbsp;</th>
		</tr>
