...
<html:html>
<bean:define id="id" value="IGSMR0705" toScope="request" />
...
function stop(instanceID){
	...
	form.action="IGSMR0302_STOP.do?instanceID="+instanceID+"&type=WD3";
	form.submit();
}
function sendmessage(){
	...
	if(null != alids && alids.length > 0){
		var url="IGSMR0302_Modal.do?prid="+prid;
		...
		<div id="container">
			...
			<div id="contentWrap">
				<div id="content">
					...
					<div id="search">
					<html:form action="/IGSMR0703_DISP" styleId="form">
					
					 <div class="img"></div>
					 <div class="conditions">
                       <span>发起时间</span>：
						<span>&nbsp;&nbsp;从</span><html:text property="propentime" styleId="propentime" errorStyleClass="inputError imeDisabled" size="16" readonly="true"/>
						<img src="images/date.gif" align="middle" onclick="calendar($('propentime'))" border="0" style="cursor: hand" />
						<span>&nbsp;&nbsp;到</span><html:text property="proclosetime" styleId="prclosetime" errorStyleClass="inputError imeDisabled" size="16" readonly="true"/>
						<img src="images/date.gif" align="middle" onclick="calendar($('proclosetime'))" border="0" style="cursor: hand" />
			...
			<logic:present name="IGSMR07031VO" property="map_see">
			<logic:iterate id="map_see" name="IGSMR07031VO" property="map_see">
			...
			<html:link action="/IGPRR0102_Disp.do?prtype=WD3&prid=${info.prid}&jump=95" style="text-decoration:underline;">			
		 	${info.prtitle }(报送空表)		 			 	
		 	</html:link>
		 	</logic:present>
			...
			<td rowspan="${listsize}">不定期报表审批</td>
			</c:if>										
			...
			<img src="images/stop_16.png"  border="0" onclick="stop(${info.instanceID});" style="cursor: hand"/>
						...
						<logic:present name="IGSMR07031VO" property="map_see">
						<bean:size id="size" name="IGSMR07031VO" property="map_see"/>
						<c:if test="${size!=0 }">
