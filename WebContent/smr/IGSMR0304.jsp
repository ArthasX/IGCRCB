...
<html:html>
<bean:define id="id" value="IGSMR0304" toScope="request" />
...
function stop(instanceID){
	...
	form.action="IGSMR0302_STOP.do?instanceID="+instanceID;
	form.submit();
}
		...
		<div id="container">
			...
			<div id="contentWrap">
				<div id="content">
					...
					<div id="search">
					<html:form action="/IGSMR0302_DISP" styleId="form" onsubmit="return checkForm();">
					 ...
					 <div class="conditions">
                       <span><span class="red">*</span>年份</span>：
						...
						<table class="table_style">
							<tr>
							    <th width="3%"><input id="checkbox" name="checkbox" type="checkbox" onclick="selectAll('checkbox','prid');"/></th>
								<th width="28%">报表名</th>
								<th width="10%">状态</th>
								<th width="13%">处理人</th>
								<th width="13%">发起时间</th>
			...
			<logic:present name="IGSMR03021VO" property="map_see">
			<logic:iterate id="map_see" name="IGSMR03021VO" property="map_see">
			...
			<html:link action="/IGPRR0102_Disp.do?prtype=WD1&prid=${info.prid}&jump=87" style="text-decoration:underline;">			
		 	${info.prtitle }
<!-- 		 	(报送空表)		 			 	 -->
		 	</html:link>
		 	</logic:present>
			...
			<td rowspan="${listsize}">年报审批</td>
			</c:if>									
			<c:if test="${info.status=='0' }">
			<td rowspan="${listsize}">
			<img src="images/stop_16.png" style="cursor:hand" border="0" onclick="stop(${info.instanceID});"/>
			</td>
			</c:if>	
			</c:if>			
			</tr>                                       
						...
						<logic:present name="IGSMR03021VO" property="map_see">
						<bean:size id="size" name="IGSMR03021VO" property="map_see"/>
						<c:if test="${size!=0 }">
