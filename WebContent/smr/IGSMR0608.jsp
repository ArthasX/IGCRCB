...
<html:html>
<bean:define id="id" value="IGSMR0608" toScope="request" />
...
function checkForm(type,formobj){
	...
	if(type=="1"){
		if( window.confirm("是否确认审批？")){
			formobj.action = getAppRootUrl() + "/IGSMR0604.do?type="+type;
			formobj.submit();
		}
		else {
			return ;
	...
	if(type=="3"){
        if( window.confirm("是否确认导出？")){
            formobj.action = getAppRootUrl() + "/IGSMR0604_EXPORT.do";
            formobj.submit();
        }
        else {
            return ;
		...
		<div id="container">
			...
			<div id="contentWrap">
				<div id="content">
					...
					<html:form action="/IGSMR0604_Disp" styleId="form">
                        <ig:message/>
						<div id="results_list">
							<table class="table_style">
								...
								<logic:present name="IGSMR06031VO" property="lst_Report">
									<logic:iterate id="bean" name="IGSMR06031VO"
										...
										<tr onmouseover="mouseOver(this);"
											...
											<html:link
													action="/IGPRR0102_Disp.do?prtype=WD2&prid=${bean.prid}&instanceID=${IGSMR0603Form.instanceID}&jump=90">
									 			${bean.prtitle }
<!-- 									 			(报送空表) -->
									 		</html:link>
									 		</logic:present>
						...
						<html:hidden property="instanceID" value="${IGSMR0603Form.instanceID }"/>
					</html:form>
				</div>
			</div>
		</div>
