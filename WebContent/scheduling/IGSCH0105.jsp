...
<bean:define id="id" value="IGSCH0102" toScope="request" />
...
<!--container 左菜单 右侧内容-->
   ...
   <div id="location">
       <div class="image"></div>
        <p class="fonts1">流程管理 &gt;&gt; 任务调度 &gt;&gt; 任务调度信息修改</p>
          <div class="back"><a href="IGSCH0101_BACK.do" target="_self">返回</a></div>
    </div>
        <div id="formwrapper">
		<html:form action="/IGSCH0101_ModifyOk" onsubmit="return checkSubmit(this);">
		<ig:message />
       <fieldset>
        <legend>任务信息</legend>
       <div>
        ...
        <div>
            ...
            <%for(int i = 0; i < 24; i++){ 
            	if(i<10){%>
            	<option value="<%=String.valueOf(i) %>" <%=i==iHour?"selected=\"selected\"":"" %>><%="0"+String.valueOf(i) %></option>
            <%} else{%>
           ...
           <%for(int i = 0; i < 60; i++){
            	if(i<10){%>
            	<option value="<%=String.valueOf(i) %>" <%=i==iMinute?"selected=\"selected\"":"" %>><%="0"+String.valueOf(i) %></option>
            <%} else{%>
