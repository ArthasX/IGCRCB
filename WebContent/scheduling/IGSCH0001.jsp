...
<html:html>
<bean:define id="id" value="IGSCH0001" toScope="request" />
...
function back(year,month){
	...
	form.action="IGSCH0001.do?forward=back&year=" + year + "&month=" + month + "&type=<%=type%>";
	form.submit();
}

function next(year,month){
	var form = document.getElementById("submitForm");
	form.action="IGSCH0001.do?forward=next&year=" + year + "&month=" + month + "&type=<%=type%>";
	form.submit();
}
...
function goPage(date){
	...
	if(!checkDateByContent("<%=IGStringUtils.getCurrentDate() %>",date)){
		form.action="IGSDL0110.do?date=" + date;
		form.submit();
