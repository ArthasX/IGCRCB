...
function checkForm(){
	...
	if($F('qjtype').trim()=="S"){
		...
		if($F('qjinfo').strlen()>256){
			alert("短信内容不能大于"+Math.floor(256/strByteFlag)+"个汉字！");
			return false;
		}
	}

	if($F('qjname').strlen()>128){
		alert("任务名称不能大于"+Math.floor(128/strByteFlag)+"个汉字！");
		return false;
	}
	...
	if(confirm("是否确认提交?")){
		if(document.getElementById("ptsavepg").value == ""){
			return true;
		}
...
function showIncident(){
	...
	url = "IGSYM1201_TREE.do" + "?ccid="+ccid+"&cid="+cid+"&syscode="+syscode;
...
function showDiv(obj){ 
	if(obj.value == "S"){
		...
		document.getElementById("ptsavepg").value = "";
		...
		document.getElementById("qjinfo").value = "";
	}
}
</script>
<bean:define id="id" value="IGSCH0201" toScope="request" />
<bean:define id="title" value="定时任务" toScope="request" />
...
<!--container 左菜单 右侧内容-->
   ...
   <div id="location">
      <div class="image"></div>
      <p class="fonts1">流程管理 &gt;&gt; 任务调度 &gt;&gt; 任务登记</p>
        <div class="back">
         <a href="IGSCH0001_Disp.do?type=<bean:write name="IGSCH02011VO" property="type" />" target="_self">返回</a>
        </div>
    </div>
        <div id="formwrapper">
		<html:form action="IGSCH0201.do">
        <ig:message />
        <fieldset>
        <legend>任务信息</legend>
       ...
       <div>
            <label for="Name"><strong>开始日期</strong>：</label>
             <p class="p2">
             	<bean:write name="IGSCH02011VO" property="beginDate" />
             	<html:hidden property="qjcrttime" value="${IGSCH02011VO.beginDate}" />
             </p>      
          <br />
       </div>
       
       <div>
            <label for="Name"><span class="red">*</span><strong>调度方式</strong>：</label>
       ...
       <div id="SMSInfo" style="display:none">    
            <label for="Name"><span class="red">*</span><strong>短信内容</strong>：</label>
        ...
        <div id="workType" style="display:none">  
            <label for="Name"><strong><span class="red">*</span>工作类型</strong>：</label>
             ...
             <%if("WP".equals(request.getParameter("type"))){ %>
             	<img src="images/tree.gif" border="0" width="16" height="16" alt="查询" onclick="openDialog('IGPRM0002.do?ptqrtzflag=${IGSCH02011VO.type}&type=0')" style="cursor: hand"/>
             <%}else if("WE".equals(request.getParameter("type"))){ %>
             	<img src="images/tree.gif" border="0" width="16" height="16" alt="查询" onclick="openDialog('IGPRM0002.do?ptqrtzflag=${IGSCH02011VO.type}&type=1')" style="cursor: hand"/>
             <%} %>
    ...
    <html:hidden property="type" value="${IGSCH02011VO.type}" />
