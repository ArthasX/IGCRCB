...
<html:html>
<bean:define id="id" value="IGPRM0000" toScope="request" />
...
function selectType(ptstartpg,flag,pdid){
	...
	ptstartpg=ptstartpg.replace(new RegExp('.do'),socFLAG);
	//openSubWindow(ptstartpg+'?flag='+flag+'&prpdid='+pdid , '', '800', '600');
	window.showModalDialog(getAppRootUrl() + "/IGCOM0401.do?prtype=WA&ptid=7&flag="+flag+"&prpdid="+pdid,null,"dialogWidth:1024px;dialogHeight:650px;status:no;help:no;resizable:yes");
	//window.showModalDialog(ptstartpg+"?flag="+flag+"&prpdid="+pdid,null,"dialogWidth:800px;dialogHeight:650px;status:no;help:no;resizable:yes;");
	window.close();
}
function selectRole(ptstartpg,pdid,ptid){
	var obj=window.showModalDialog(getAppRootUrl() + '/IGPRR0101_Disp_SOC.do?psdcode=<%=IGPRDCONSTANTS.PROCESS_START_STATUS%>&pdid='+pdid+'&ptid='+ptid,null,"dialogWidth:1024px;dialogHeight:650px;status:no;help:no;resizable:yes");
	//window.showModalDialog("/IGPRR0101_Disp_SOC.do?psdcode=<%=IGPRDCONSTANTS.PROCESS_START_STATUS%>&pdid="+pdid+"&ptid="+ptid,null,"dialogWidth:800px;dialogHeight:650px;status:no;help:no;resizable:yes");
