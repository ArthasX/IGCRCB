		...
		if( tp == 1){
			if(window.confirm("您在同一表单中选择了相同资产，是否确认进行下一步操作？")){
				<logic:notEmpty name="IGEXTWA0102Form" property="prid">
					document.forms[0].action=getAppRootUrl() + "/IGEXTWA0122_DispII_SOC.do?prid="+${IGEXTWA0102Form.prid}+"&prstatus="+status;
...
function deleteact(prid){
	if(confirm("是否删除当前指派工作信息")){
		document.forms[0].action =getAppRootUrl() + "/IGEXTWA0102_Delete_SOC.do?prid="+prid;
		document.forms[0].submit();
		return true;
	}
	else{
...
function terminate(prid){
	if(confirm("是否中止当前指派工作信息")){
		document.forms[0].action = getAppRootUrl() + "/IGEXTWA0110_SOC.do?prid="+prid+"&isSOC=1";
		document.forms[0].submit();
		return true;
	}else{
		return false;
...
<div id="content"><html:form action="/IGEXTWA0102_SOC" styleId="form" onsubmit="return checkSubmit(this);" enctype="multipart/form-data">
	...
	<div><label for="Name"><strong>相关资产</strong>：</label>
		...
		<logic:iterate id="entity" name="IGEXTWA01031VO" property="pieList"
			type="com.deliverik.framework.workflow.prd.model.IG731Info">
		<a href="javascript:void(0)" onclick="openSubIGCOM0303('<%=entity.getEiid() %>','')" id="<%=entity.getPieid() %>"><%=entity.getSoc0118TB().getEiname() %></a><img id="<%=entity.getPieid() %>ENT" src="images/delate.gif" align="middle" onClick="delEntity('<%=entity.getPieid() %>','A')" border="0" style="cursor: hand" alt="删除"/>
		&nbsp;
		</logic:iterate>
		</p>
	</div>
							...
							<logic:present name="IGEXTWA01031VO" property="processInfoEntityList">
								<logic:iterate id="entity" name="IGEXTWA01031VO" property="processInfoEntityList"
									...
									<% if(info.getPiid().equals(entity.getPiid())){ %>
										...
										<%=entity.getSoc0118TB().getEiname() %></a><img id="<%=entity.getPieid() %>ENT" src="images/delate.gif" align="middle" onClick="delEntity('<%=entity.getPieid() %>', ${infoDef.pidid})" border="0" style="cursor: hand" alt="删除"/><input type="hidden" name="a${infoDef.pidid}" value="<%=entity.getPieid() %>"/>&nbsp;
									<% } %>
							</logic:iterate>
