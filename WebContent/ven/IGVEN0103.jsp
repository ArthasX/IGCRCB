...
<html:html>
<bean:define id="id" value="IGVEN0103" toScope="request"/>
...
<!-- /header1 -->
	...
	function deleteRec(){
        ...
        var deleteEiid = document.getElementsByName("deleteEiid");
        var property = {
    			eiids : new Array,
        ...
        var temp = 0;
        var eiversion = document.getElementsByName("eiversion");
        var riskly =  document.getElementsByName("riskly");
        var riskarea= document.getElementsByName("riskarea");
        var riskitem = document.getElementsByName("riskitem");
        var eilabel = document.getElementsByName("eilabels");
        var einame = document.getElementsByName("einames");
        if(deleteEiid.length > 0){
            var bchecked = false;
            for( var i=0; i<deleteEiid.length; i++ ){
                if (deleteEiid[i].checked == true){
                	property.eiids[temp] = deleteEiid[i].value;
                	property.eiversions[temp] = eiversion[i].value;
                	property.risklys[temp] = riskly[i].value;
                	property.riskareas[temp] = riskarea[i].value;
                	property.riskitems[temp] = riskitem[i].value;
                	property.eilabels[temp] = eilabel[i].value;
                	property.einames[temp] = einame[i].value;
                	temp++;
                    bchecked = true;
                }
            }
    		...
    		window.opener.risFunction.setParams2(property);
	       	window.close();
	}
...
//				 document.forms[0].eid.value = "";
	 ...
	 function onExcel(){
			form.action = "IGASM3114_Excel.do";
			form.submit();
		}
	function onSearch(){
			form.action = "IGASM3101_window.do";
			form.submit();
...
<div id="container">
    ...
    <div id="contentWrap">
        <div id="content">
            <html:form styleId="form" action="/IGASM3101_window"
            ...
            <div id="location">
                ...
                <div id="results_list">
			                ...
			                <logic:present name="IGASM31011VO" property="searchRiskIndexSearchVWList" >
			                    <logic:iterate id="bean" name="IGASM31011VO" property="searchRiskIndexSearchVWList" indexId="index">
			                        <tr class="<ig:rowcss index="${index}"/>">
			                            <!-- 风险领域 -->
			                            <td> 
			                            	<input type="checkbox"   style="width:20px;" name="deleteEiid" value="${bean.eiid}"/>
											<input type="hidden" name="eiversion" value="${bean.eiversion}">	
											<input type="hidden" name="riskly" value="${bean.riskly}">
											<input type="hidden" name="riskarea" value="${bean.riskarea}">
			     ...
			     <div id="operate">
						  <logic:present name="IGASM31011VO" property="searchRiskIndexSearchVWList" >
						  		<html:submit property="btn_delete" value="确认所选" styleClass="button" onclick="return deleteRec();" />
                ...
                <html:hidden property="eiid_NotIn" styleId="eiid_NotIn" value="${IGASM3101Form.eiid_NotIn}"/>
            </html:form>
        </div>
    </div>
