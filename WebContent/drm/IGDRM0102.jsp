...
<html >
<bean:define id="id" value="IGDRM0102" toScope="request" />
...
<script src="js/prr/${fn:substring(IGPRR0102Form.pdid,0,5) }.js" charset="GBK"></script>
...
jQuery(function(){
	...
	//隐藏场景类型表单
	jQuery("#pidid"+pdid+"016").parent().parent().hide();
	//应急资源
...
// 	jSouEiid.parent().parent().hide();
	...
	//old场景应急处置步骤
	var jOldStepPdid = jQuery("#pidid"+pdid+"012");
	jOldStepPdid.parent().parent().hide();
	//原资产ID
	var jOldEiid = jQuery("#pidid"+pdid+"013");
	jOldEiid.parent().parent().hide();
	//原应急资源资产ID
	var jOldSouEiid = jQuery("#pidid"+pdid+"014");
	jOldSouEiid.parent().parent().hide();
	//原prid
	var jOldprid = jQuery("#pidid"+pdid+"015");
	jOldprid.parent().parent().hide();
	
 	//场景名称：").attr("readonly":"readonly"); 
 	jQuery("#pidid"+pdid+"011").attr("readonly","readonly"); 
	// 场景描述
 	jQuery("#pidid"+pdid+"002").attr("readonly","readonly"); 
	//隐藏查看表单日志按钮
	jQuery("a[class='a1']").hide();
	...
	if(jQuery("#psdid[value$='001']").length > 0){
		...
		var jOpinions = jQuery("#pidid"+pdid+"010");
		if(""==jOpinions.val()){
			jOpinions.parent().parent().hide();
		}
	}
	//场景类型
	var sceneType = jQuery("#pidid"+pdid+"016").val();
	if(null!=stepPdid&&""!=stepPdid){
	var jStautsInfoDiv = jQuery("#stautsInfoDiv");
	if (jQuery("#psdid[value$='001']").length > 0||jQuery("#psdid[value$='004']").length > 0) {
		var isMask = "0";
		//审批节点不能编辑
		if(jQuery("#psdid[value$='004']").length > 0){
			isMask = "1";
		}
		//jShowFlow.show();
	...
	//关联预案是否可执行 下拉监听 
	var jOpenTableImg = jQuery("#openTableImg").parent();
...
function getFlowManger(){
	if (jQuery("#psdid[value$='001']").length > 0||jQuery("#psdid[value$='004']").length > 0) {
		...
		var stepPdid = jQuery("#pidid"+pdid+"004").val();
		//old场景应急处置步骤pdid
		var oldpdid = jQuery("#pidid"+pdid+"012").val();
		//场景类型
		var sceneType = jQuery("#pidid"+pdid+"016").val();
		if(null!=stepPdid&&""!=stepPdid){
...
jQuery(function(){
	if(psdid.substring(7,10)=='001'){
		var sceneType = jQuery("#pidid"+pdid+"016").val();
		jQuery("#pidid"+pdid+"016").hide();
		jQuery("<option  value="+sceneType+">"+sceneType+"</option>").after("#pidid"+pdid+"016");//添加下拉框的option
		jQuery("#pidid"+pdid+"016").after(sceneType);
		jQuery("#pidid"+pdid+"016").parent().parent().find("span").remove();	
		//发起节点审批信息无值则隐藏
