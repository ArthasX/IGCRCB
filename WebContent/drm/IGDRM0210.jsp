...
function grantDutyPerson(){
	if( window.confirm("请确认是否要进行授权操作?")){
		var prroleid = document.getElementById("ppidstr").value.split("_")[1];
		var objAjax = new Ajax.Request(getAppRootUrl() + ajaxAction,{asynchronous:false, 
			parameters: 'type=grantDutyPerson&prid=${IGPRR0102Form.prid}&roleid=' + prroleid,
...
function noticeRoleManager(){
	if( window.confirm("请确认是否要短信通知部门负责人?")){
		var prroleid = document.getElementById("ppidstr").value.split("_")[1];
		var objAjax = new Ajax.Request(getAppRootUrl() + ajaxAction,{asynchronous:false, 
			parameters: 'type=noticeRoleManager&prid=${IGPRR0102Form.prid}&roleid=' + prroleid,
			 onSuccess:  function(req, json){
			 var result=req.responseText;
				if(result != null && result != ""){
					alert(result);
...
function assignOA(prid, prstatus, psdid, usertype){
	if("super" == usertype) {
		...
		var prroleid = document.getElementById("ppidstr").value.split("_")[1];
		window.location.href = "IGPRR0113_Admin.do?flag=1&roleid=" + prroleid + "&prid=" + prid + "&prstatus=" + prstatus + "&psdid=" + psdid;
	}
}
function assignNormal(prid, prstatus, psdid, usertype){
	if("super" == usertype) {
		...
		var prroleid = document.getElementById("ppidstr").value.split("_")[1];
		window.location.href = "IGPRR0113_Admin.do?flag=0&roleid=" + prroleid + "&prid=" + prid + "&prstatus=" + prstatus + "&psdid=" + psdid;
	}
}
...
<%-- 	</logic:notEqual> --%>
	<div id="stautsInfoContainer" style="margin-left: 10px;margin-bottom: 10px;width:950px;">
		<div class="title" style="width:950px;">
			...
			<div style="margin-top: 8px;"><img border=0 style='width:10px;height:10px;' id=openTableImg src='images/addd.gif' /></div>
		</div>
	...
	<html:hidden property="parameters" styleId="parameters" name="IGPRR0102Form" />
...
jQ(function(){
	...
	jQ.ajax({
		...
		//nodata
		jSInfoContainer.append('<tr><th width="5%">序号</th><th width="15%">状态名称</th><th width="15%">处置说明</th><th width="50%">处置结果</th><th width="50%">附件</th></tr>');
