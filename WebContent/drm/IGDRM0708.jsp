...
<html:html>
<bean:define id="id" value="IGDRM0708" toScope="request" />
		...
		<fieldset>
		    ...
		    <div id="results_list">
		       <table class="table_style">
		           ...
		           <tr>
		               <td>
		               	${IGDRM07081VO.entityData.ename}
		               		<input type="hidden" id="esyscoding" value="${IGDRM07081VO.entityData.esyscoding}">
		               </td>
		               <td>${IGDRM07081VO.entityData.eilabel}</td>
		               <td>${IGDRM07081VO.entityData.einame}</td>
		               <!--<td>${IGDRM07081VO.entityData.organizationTB.orgname}</td>-->
		               <td>${IGDRM07081VO.entityData.eidesc}</td>
		               <td>${IGDRM07081VO.entityData.eiusername}</td>
		           </tr>
		       </table>
		    </div>
	    </fieldset>
			<logic:present name="IGDRM07081VO" property="configItemVWInfoMap">
				<c:if test="${fn:length(IGDRM07081VO.configItemVWInfoMap)>0 }">
			    <fieldset>
			    	<legend class="name">详细信息</legend>
			    	<br/>
					<logic:iterate id="map" name="IGDRM07081VO" property="configItemVWInfoMap" indexId="number">
...
jQ(function(){
	...
	if(/999040.*/.test(esyscoding)){
		ename = "应急资源";
	}else if(/888001.*/.test(esyscoding)){
		ename = "专项预案";
