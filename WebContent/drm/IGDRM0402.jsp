...
<%-- 							<html:hidden property="ercode" styleId="ercode"/> --%>
			...
			<iframe id="iframe" name="iframe" src="IGDRM0402.do" width="730px" height="520px" frameborder="0"  style="padding-top: 10px;border:1px solid #BB0500;" onload="getCount();">
			</iframe>
			</div>
	</div>
</div>
...
jQuery(function(){
	...
	if(document.getElementById) {
		tree = new WebFXTree(nodeMap['000'].name);
...
function submitForm(ercode , erid , esyscode){
	alert(ercode + erid + esyscode);
	window.location.href = getAppRootUrl() + "/IGDRM0402_Disp.do?" + "ercode=" + ercode + "&erid=" + erid + "&esyscode=" + esyscode;
