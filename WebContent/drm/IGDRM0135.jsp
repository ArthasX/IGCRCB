...
function checkForm(vbrequired,vbrecord,btnid,action,type,code,showConfirm,msgFlag){
	packagingTableValue();
	...
	if(pdid.substring(0,5) == '01385' && jQ("#psdid").val() == pdid + '003' && action=="<%=IGPRDCONSTANTS.BUTTON_SUBMIT%>" ){
		//提示场景参与人表格处有角色没有指定到人
		if(jQ("#"+pdid+"023_tableForm > tbody > tr").length > 1){
			var len = jQ("#"+pdid+"023_tableForm > tbody > tr > td > table > tbody > tr").length;
			if(len > 0){
				for(var mi = 0;mi < len ;mi++){
					var $childTr = jQ("#"+pdid+"023_tableForm > tbody > tr > td > table > tbody > tr").eq(mi);
					var childTdTd = $childTr.find("> td:eq(1)");
					var text = childTdTd.text();
					if(!text){
						var config = "场景参与人没有指定到人，是否指定到人？";
						if(window.confirm(config)) {
							go="0";
							return false;
						}else{
							break;
	...
	if(pdid.substring(0,5) == '01386' && jQ("#psdid").val() == pdid + '006' && action=="<%=IGPRDCONSTANTS.BUTTON_SUBMIT%>" ){
		//提示场景参与人表格处有角色没有指定到人
		if(jQ("#"+pdid+"027_tableForm > tbody > tr").length > 1){
			var len = jQ("#"+pdid+"027_tableForm > tbody > tr > td > table > tbody > tr").length;
			if(len > 0){
				for(var mi = 0;mi < len ;mi++){
					var $childTr = jQ("#"+pdid+"027_tableForm > tbody > tr > td > table > tbody > tr").eq(mi);
					var childTdTd = $childTr.find("> td:eq(1)");
					var text = childTdTd.text();
					if(!text){
						var config = "场景参与人没有指定到人，是否指定到人？";
						...
						<logic:present name="psid">
							...
							<logic:notEqual value="8" name="psid" property="pidtype">
								...
								<logic:equal value="1" name="psid" property="pidrequired">
									...
									if(tb){
										var fhas = false;
										var imgs = tb.parentElement.childNodes;
										if(imgs != null && imgs.length > 0){
											for(var i=0;i<imgs.length;i++){
												if (imgs[i] && imgs[i].style && imgs[i].style.display != "none" && imgs[i].getAttribute("id") && imgs[i].getAttribute("id").indexOf("IMG") >= 0) {
													fhas = true;
													break;
												}
											}
