...
function delEntity(pieid, pidid, property){
	...
	if(flag){
		...
		if(pidid ==pdid+'021'){
			...
			deleteSelect2(pdid+"010");
			deleteSelect2(pdid+"013");
			entityChange();
		}
	}
	return flag;
...
function delSingleEntity(pieid, pidid, property,pidid,syscoding,index,required){
	...
	if(flag){
		...
		if(syscoding.substring(0,6)=='999050'){
			...
			var cjpidid = pidid.substring(0,7)+'013';
			...
			document.forms[0].pieids.value = pieids;
			checkAsm(cjpidid, property);
			
			if(jQ("#tbentity"+pdid+"013").is(":visible")==false){
...
function delSelect2SingleEntity(pieid, pidid, property,pidid,syscoding,index,required){
	...
	if(flag){
		...
		if(pidid == pdid+'010'){
			...
			deleteSelect2(pdid+"013");
			entityChange();
		}else if(pidid == pdid+'013'){
			deleteSelect2(pdid+"013");
			jQ("#assetSelector"+pdid+"010").trigger("change");
		}
	}
}
