...
<html:html>
<bean:define id="id" value="IGDRM1103" toScope="request" />
...
<script type="text/javascript">	
		var gid='IGDRM0108';
        function exportExl(){
        	window.location.href=getAppRootUrl()+"/IGDRM1103_Export.do?pdid="+jQuery("#pdid").val()+"&prid="+jQuery("#prid").val()+"&eiid="+jQuery("#eiid").val();
        }
...
<!-- 		<div id="showFlow"></div> -->
	...
	<div class="enter"><html:button property="btn_back" styleClass="button" value="导出" onclick="exportExl()"/></div>
	...
	<html:hidden property="prid" styleId="prid" name="IGDRM0108Form" />
</html:form></div>
</div>
</div>
...
// 				stepPdid = jQuery(item).parent().next().next().children("div").text().trim();
		...
		if(null!=stepPdid&&""!=stepPdid){
			if(sceneType&&sceneType=='组合场景'){
				...
				jQ.ajax({
					type: "post",
					url:"IGDRM1103_Detail.do",
					data:'pdid='+stepPdid+'&prid='+jQuery("#prid").val(),
					async:false,
					dataType:"html",
					success:function(html){
