...
<html>
<bean:define id="id" value="IGVIR0201" toScope="request" />
...
function checkVMInfo(){
	//校验项目信息
	var vmnames = document.getElementsByName("vmnames");
	var vmcpus = document.getElementsByName("vmcpus");
	var vmmems = document.getElementsByName("vmmems");
	var vmdisks = document.getElementsByName("vmdisks");
	var checkRes = true;
	for (var i = 1; i < vmnames.length; i++) {
		if(vmnames[i].value === ""){
			alert("序号为【" + i + "】的虚机名称不能为空");
			checkRes = false;
			break;
		} else if (vmcpus[i].value === "") {
			alert("序号为【" + i + "】的CPU不能为空");
			checkRes = false;
			break;
		} else if (vmmems[i].value === "") {
			alert("序号为【" + i + "】的内存不能为空");
			checkRes = false;
			break;
		} else if (vmdisks[i].value === "") {
			alert("序号为【" + i + "】的硬盘不能为空");
			checkRes = false;
			break;
		}
...
// 	var projectName = getFormObj("申请项目名称").value;
// 	var expectDeployTime = getFormObj("期望部署时间").value;
// 	var projectExpireTime = getFormObj("项目到期时间").value;
// 	var projectType = getFormObj("项目类别").value;
// 	var projectDesc = getFormObj("项目说明").innerHTML;
// 	var projectApplyReason = getFormObj("申请原因").innerHTML;
	
// 	document.getElementById("projectName").value = projectName;
// 	document.getElementById("expectDeployTime").value = expectDeployTime;
// 	document.getElementById("projectExpireTime").value = projectExpireTime;
// 	document.getElementById("projectType").value = projectType;
// 	document.getElementById("projectDesc").value = projectDesc;
// 	document.getElementById("projectApplyReason").value = projectApplyReason;
	
	// 删除模板行
// 	var root = document.getElementById("vminfo");
// 	root.deleteRow(1);
	getFormObj("项目名称").value = document.getElementById("projectName").value;
	...
	// 提交数据
	jQuery.post(getAppRootUrl() + "/IGVIR0201_SaveVM.do", jQuery("#form").serialize(),
		function(data) {
			getFormObj("项目主键").value = data.split("@_@")[0];
			getFormObj("虚机主键").value = data.split("@_@")[1];
// 		   alert("项目主键：" + data);
		} 		
	);
}
...
<%-- 				<html:textarea property="prdesc" cols="60" rows="6"  errorStyleClass="inputError imeActive"></html:textarea></td> --%>
	...
	<jsp:include page="IGVIR0201_INC.jsp"></jsp:include>
	<div class="enter">
	<div>
