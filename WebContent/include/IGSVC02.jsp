...
<%@ page import="com.deliverik.infogovernor.svc.bl.IGSVC02BLType"%>
...
<logic:present name="${vo}" property="process">
	...
	<table cellspacing="2" class="table_style1">
		...
		<tr>
			...
			<td colspan="5">
				...
				<logic:greaterEqual name="entity" property="pieid" value="0">
		...
		<tr>
			...
			<th>优先级：</th>
			<td><ig:codeValue ccid="PROCESS_PRIORITY_I" cid="${process.prPriority}" /></td>
		</tr>
		<tr>
			<th>紧急程度：</th>
		...
		<tr>
			<th>事件描述：</th>
			<td colspan="3"><pre><bean:write name="process" property="prdesc" /></pre></td>
		</tr>
	...
	<table width="100%">
		<tr>
			<td width="80%"></td>
			<td width="20%" align="left"><a href="javascript:openHistoryPage('/IGSVC0219.do?prid=${process.prid}');">查看状态日志</a></td>
		</tr>
	</table>
	</div>
			...
			<bean:define id="count" name="${vo}" property="processInfo" type="java.util.List"/>
				...
				<%if(index.intValue() == count.size()-1 && count.size() % 2 != 0){%>
					<td colspan="3">
						...
						<% } else if(info.getPivarvalue().contains("_tree_")){ %>
							<%=info.getPivarvalue().split("_tree_")[1] %>
		...
		<logic:notEmpty name="${vo}" property="perMap">
			...
			<table cellspacing="2" class="table_style1">
					<tr>
						...
						<th width="15%"><%=IGRIS02BLType.PROCESS_PRT_USERNAME%>：</th>
						<td width="18%"><%=map.get(IGRIS02BLType.PROCESS_PRT_USERNAME_ID)==null?"":map.get(IGRIS02BLType.PROCESS_PRT_USERNAME_ID)%></td>
						<th width="15%"><%=IGRIS02BLType.PROCESS_AFT_USERNAME%>：</th>
						<td width="19%"><%=map.get(IGRIS02BLType.PROCESS_AFT_USERNAME_ID)==null?"":map.get(IGRIS02BLType.PROCESS_AFT_USERNAME_ID)%></td>
					</tr>
					<tr>
						<th>发起人部门：</th>
						<td><bean:write name="process" property="prorgname" /></td>
						<th><%=IGRIS02BLType.PROCESS_PRT_ORGNAME%>：</th>
						<td><%=map.get(IGRIS02BLType.PROCESS_PRT_ORGNAME_ID)==null?"":map.get(IGRIS02BLType.PROCESS_PRT_ORGNAME_ID)%></td>
						<th><%=IGRIS02BLType.PROCESS_AFT_ORGNAME%>：</th>
						<td><%=map.get(IGRIS02BLType.PROCESS_AFT_ORGNAME_ID)==null?"":map.get(IGRIS02BLType.PROCESS_AFT_ORGNAME_ID)%></td>
					</tr>
					<tr>
						<th>发起人联系方式：</th>
						<td><bean:write name="process" property="prinfo" /></td>
						<th><%=IGRIS02BLType.PROCESS_PRT_USERCONNECT%>：</th>
						<td><%=map.get(IGRIS02BLType.PROCESS_PRT_USERCONNECT_ID)==null?"":map.get(IGRIS02BLType.PROCESS_PRT_USERCONNECT_ID)%></td>
						<th><%=IGRIS02BLType.PROCESS_AFT_USERCONNECT%>：</th>
						<td><%=map.get(IGRIS02BLType.PROCESS_AFT_USERCONNECT_ID)==null?"":map.get(IGRIS02BLType.PROCESS_AFT_USERCONNECT_ID)%></td>
					</tr>
					<tr>
						<th>发起人角色：</th>
	...
	<table cellspacing="2" class="table_style1">
		...
		<tr>
			...
			<td width="35%"><bean:write name="${vo}" property="expectStatusTime" />
				<ig:ProcessStatusWarningImgTag 
	            processType="${process.prtype}" prid = "${process.prid}" prStatus = "${process.prstatus}" 
	            prStrategyVersion="${process.prStrategyVersion}" prPdid="${process.prpdid}" prUrgency="${process.prPriority}"/>
			</td>
		</tr>
		<tr>
			...
			<td width="35%"><bean:write name="${vo}" property="expectProcessTime" />
			    ...
			    prPdid="${process.prpdid}" prUrgency="${process.prPriority}"/>
			</td>
		</tr>
	</table>
	</div>
	...
	<div class="title" style="cursor: hand;">
		<div class="name" onclick="toggleElement('fileMessage','fileMessageImg')">
		 	附件信息<img id="fileMessageImg" src="images/add_1.jpg">
		</div>
	</div>
	...
	<c:choose>
		...
		<table width="100%">
			<tr>
				...
				<a href="###" onclick="initProcessQuick('${process.prid}','I','IGSVC0207_Disp.do?prid=${process.prid}')">发起相关流程</a>
				</td>
			</tr>
		</table>
		</div>
...
<logic:present name="${vo}" property="knowledgeProcessList">
<bean:size id="num" name="${vo}" property="knowledgeProcessList"/>
<c:if test="${num > 0}">
<div class="title" style="cursor: hand;">
	<div class="name" onclick="toggleElement('knowledgeMessage','knowledgeMessageImg')">
	 	参考知识<img id="knowledgeMessageImg" src="images/add_1.jpg">
	</div>
</div>
<div class="record_list" id="knowledgeMessage" style="display: none;">
<table cellspacing="2" class="table_style1">
	<logic:present name="${vo}" property="knowledgeProcessYesList">
		<bean:size id="num_yes" name="${vo}" property="knowledgeProcessYesList"/>
			<c:if test="${num_yes > 0}">
				<tr>
					<th width="15%">有效知识：</th>
					<td colspan="3">
						<logic:iterate id="bean" name="${vo}" property="knowledgeProcessYesList">
							<a href="javascript:void(0)" onclick="openKnowledgeByKnid('${bean.knowledge.knid}','${bean.knowledge.knversion}')">${bean.knowledge.kntitle}</a><br/>
						</logic:iterate>
					</td>
				</tr>
			</c:if>
	</logic:present>
	<logic:present name="${vo}" property="knowledgeProcessNoList">
		<bean:size id="num_no" name="${vo}" property="knowledgeProcessNoList"/>
			<c:if test="${num_no > 0}">
				<tr>
					<th width="15%">已查阅知识：</th>
					<td colspan="3">
						<logic:iterate id="bean" name="${vo}" property="knowledgeProcessNoList">
							<a href="javascript:void(0)" onclick="openKnowledgeByKnid('${bean.knowledge.knid}','${bean.knowledge.knversion}')">${bean.knowledge.kntitle}</a><br/>
						</logic:iterate>
					</td>
				</tr>
