...
<html:html>
<bean:define id="id" value="IGMTP0104" toScope="request" />
...
function back(year,month){
	...
	form.action="IGMTP0104_Disp.do?forward=back&showYear=" + year + "&showMonth=" + month + "&dadcategory=${IGDUT0201Form.dadcategory}";
	form.submit();
}

function next(year,month){
	var form = document.getElementById("submitForm");
	form.action="IGMTP0104_Disp.do?forward=next&showYear=" + year + "&showMonth=" + month + "&dadcategory=${IGDUT0201Form.dadcategory}";
	form.submit();
}

function next1(qprpdid){
	if(window.confirm("您是否确认进行下一步操作？")){
		var form = document.getElementById("submitForm");
		form.action="IGMTP0104_NEXT.do?pdid="+qprpdid;
		form.submit();
	}
	      ...
	      <table width="700" border="0" align="center" cellspacing="0" cellpadding="0" style="margin-top:40px;">
	    	<tr height="20">
				...
				<td width="239" align="right" style="font-weight:bold">
					计划开始时间：${IGMTP0103Form.begin_time }
					<input type="hidden" name="begin_time" value="${IGMTP0103Form.begin_time }">
				</td>
				<td width="238" align="right" style="font-weight:bold">
					计划结束时间：${IGMTP0103Form.end_time }
					<input type="hidden" name="end_time" value="${IGMTP0103Form.end_time }">
            ...
            <tr height="12" align="center">
             ...
             <td width="12" ><img src="images/igmtp3.gif" width="14" height="18" border="0"/></td>
              <td width="60" align="left">当前日期</td>          
		  		  ...
		  		  <div class="enter">
		  		  <html:submit styleClass="button" onclick="next1('${IGMTP0101Form.qprpdid }');" value="下一步"/>
		  		  </div>
		  		   </div>
</div>
</div>
