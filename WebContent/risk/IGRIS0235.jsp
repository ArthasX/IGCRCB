...
<html:html>
<bean:define id="id" value="IGRIS0235" toScope="request" />
...
<!--
	...
	window.onload=function(){
	 ...
	 function toback(){
    	form.action = getAppRootUrl() + "/IGCOM0202.do?linkID=IGRIS0225";
    	form.submit();
    }
//-->
</script>
...
//	    newDiv.style.left = (parseInt(document.body.scrollWidth) - 340) / 2 + "px"; // 屏幕居中
	    newDiv.style.left = (event.clientX + 15) + "px"; // 屏幕居中
...
<div id="search">
	...
	<html:form action="/IGRIS0235_sel" enctype="multipart/form-data" styleId="form">
	<div>
			...
			<logic:present name="IGRIS02013VO" property="igris02012voList">
				...
				<%-- 	<bean:define id="bean" name="bean1" property="riskCheck" ></bean:define> --%>
						<tr class="<ig:rowcss index="${index}"/>">
							...
							<textarea style="display:none;" rows="6" cols="60" id="jcxsm_${index}">${bean1.rccommon}</textarea>
							</td>
							<td class="rlcomment" onclick="showArea('jcff_${index}','检查方法')">
							<ig:SubstringShowEllipsisWriteTag len="5" showEllipsis="true">
								${bean1.rcdesc}
							</ig:SubstringShowEllipsisWriteTag>
							<textarea style="display:none;" rows="6" cols="60" id="jcff_${index}">${bean1.rcdesc}</textarea>
							</td>
							<td>${bean1.rcrtestmode}</td>
							<td>
								<ig:codeValue ccid="RISKCHECKRESULT_CODE" cid="${bean1.rcrresult}" ></ig:codeValue>					
							</td>
							<td class="rlcomment" onclick="showArea('jgsm_${index}','结果说明')">
								<ig:SubstringShowEllipsisWriteTag len="6" showEllipsis="true">
									<bean:write name="bean1" property="rcrcomment"/>
								</ig:SubstringShowEllipsisWriteTag>	
								<textarea style="display:none;" rows="6" cols="60" id="jgsm_${index}"><bean:write name="bean1" property="rcrcomment"/></textarea>		
							</td>
							<td>
							...
							<td id="assignwork">
								...
								<c:if test="${bean1.rcrresult != 'A' && bean1.rcrresult != 'B' && bean1.rcrresult != 'C'}">
									<logic:empty name="bean1" property="assignprid" >
										<a href="IGCOM0202.do?linkID=IGWKM0104&psdcode=Z&pdid=01140&ptid=8&actname=ACT04RIS0202&parameters=${bean1.rcrid}">发起</a>
									</logic:empty>
									<logic:notEmpty name="bean1" property="assignprid">
									<a href="IGPRR0102_Disp.do?prid=${bean1.assignprid}&jump=472" >
										查看
									</a>
									</logic:notEmpty>
								</c:if>
