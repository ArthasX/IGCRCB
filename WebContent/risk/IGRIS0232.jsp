...
%>
<html:html>
<bean:define id="id" value="IGRIS0232" toScope="request" />
<bean:define id="title" value="检查策略修改画面" toScope="request" />
<!-- header1 -->
...
window.onload=function(){
	...
	document.getElementById("selectjs").onchange=function(){
		...
		var eiid = '${IGRIS0202Form.eiids}'.split(",");
		var eiversion = '${IGRIS0202Form.eiversions}'.split(",");
		var einame = '${IGRIS0202Form.einames}'.split(",");
		
		var eidescDemp = '${IGRIS0202Form.eidesc}';
		var rcommonDemp = '${IGRIS0202Form.rcommon}';
		var eidesc = clearBr(eidescDemp).split(",");
		var rcommon = clearBr(rcommonDemp).split(",");
		
		var riskitem = '${IGRIS0202Form.riskitem}'.split(",");
		var eistatus = '${IGRIS0202Form.eistatus}'.split(",");
		
		fuzhi(eiid,eiversion,einame,eidesc,rcommon,riskitem,eistatus);
		setParams(eiid,eiversion,einame,eidesc,rcommon,riskitem,eistatus);
}
//去除换行
...
// 		}
	...
	function optionEntity(){
		...
		openSubWindow('/IGRIS0601_OPTION.do?eiids='+eiids+'', '_blank','1000','600','center:1');	
	}
...
<div id="formwrapper">
	...
	<%-- 	<div><label for="Name"><strong><span class="red">*</span>工作执行期限(小时)</strong>：</label> 
		...
		<div>
          ...
          <span  id="onlyone">
           <input name="rcfrequency" type="radio" value="once" onclick="toQuency(this.value)" <%=onceChecked %>  />立即发起 </span>
