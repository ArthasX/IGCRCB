...
<html>
<bean:define id="id" value="IGRIS0227" toScope="request" />
<bean:define id="title" value="指派检查工作发起画面" toScope="request" />
<!-- header1 -->
...
function setIGCYB0101Params(username,orgname,orgid,useremail,userphone,usermobile,userid){
	if(gid=='0'){
		...
		document.getElementById("rcname").value = username;
		/* document.getElementById("rcname").value = userid; */
	}else{
		document.getElementById("rcname").value = username;
		/* document.getElementById("rcname").value = userid; */
	}
...
function checkForm(action){
	...
	<logic:present name="IGPRR01011VO" property="processTitleDefTB">
		...
		<logic:equal value="0" name="processTitleTB" property="pdescaccess">
			...
			if($F('prdesc').strlen()>1500){
				alert(desc+"不能大于"+Math.floor(1500/strByteFlag)+"个汉字！");
				return false;
			}
		</logic:equal>
	</logic:present>
	...
	if( $("rcname").value == "" ||  $("rcname").value == null){
		alert("请输入处理人！");
		return false;
	}
	...
	if( window.confirm("您是否确认提交？")){
		...
		if($F('prtitle').trim()=="" && $F('prdesc').trim() =="" ){
			...
			$("prtitle").value = "${IGPRR0101Form.prpdname}";
		}
		getFormObj("处理人").value= $("rcname").value;
		return true;
	} else {
		return false;
	}
...
function initAttFormFile(){
	if('${IGPRR0101Form.parameters}' != ''){
		var rcrid = '${IGPRR0101Form.parameters}';
		var rcname = '';
		var objAjax = new Ajax.Request(getAppRootUrl() + ajaxAction,{asynchronous:false, 
			parameters: 'type=getCheckResultUsername&rcrid='+rcrid,
		...
		//$("rcname").value = rcname.split("#")[1];
		//getFormObj("人员").value= rcname + "#" + rcrid;
	}else{
		//点击“检查问题发起”菜单隐藏返回超链接
...
function selectRiskCheck(){
	openSubWindow('/IGRIS0202_sell.do' , '', '800', '600');
}
...
function setRiskCheck(rcid,rcname,rcclass,rcrid){
	$("rcname").value=rcname;
// 	if("2"==rcclass){
// 		getFormObj("问题来源").value="安全检查";
// 	}else{
// 		getFormObj("问题来源").value="日常工作";
// 	}
	getFormObj("人员").value=rcid+"#"+rcname+"#"+rcrid;
}
...
function initWorkAjax(){
	...
	if('${IGPRR0101Form.parameters}' != ''){
		...
		var objAjax = new Ajax.Request(getAppRootUrl() + ajaxAction,{asynchronous:false, 
			...
			var result=req.responseText;
				rcname =result;
				var myjson = eval(JSON.parse(result));
				if(myjson.length>0){
					rccommon = myjson[0].rccommon.replace(new RegExp("\n","gm"),"<br>");
					jQuery(".editDiv").text();
					$("prtitle").value = myjson[0].rcname;
					$("prdesc").value = myjson[0].rcdesc;
					$("rcname").value = myjson[0].username;
					getFormObj("检查任务名称").value = myjson[0].cwrname;
					jQuery("input[name='pivarname'][value='检查方法']").parent().children("div.editDiv").css("width","405px").html(rccommon)
					jQuery("input[name='pivarname'][value='检查方法']").parent().children("div.editDiv").next().val(rccommon);
					getFormObj("人员").value= myjson[0].rcid+"#"+ myjson[0].username + "#" + rcrid;
				}
		    }
		});
	}
...
<body onload="initAttFormFile();initWorkAjax();">
...
<div id="content"><html:form action="/IGPRR0101" styleId="form"
	...
	<div class="image"></div>
	<p class="fonts1">信息安全
	<logic:equal name="IGPRR0101Form" property="ptid" value="8">
	&gt;&gt; 检查任务管理 &gt;&gt; 指派检查工作   &gt;&gt; （${IGPRR0101Form.prpdname}——待发起）
	</logic:equal>
	<logic:notEqual name="IGPRR0101Form" property="ptid" value="8">流程信息 </logic:notEqual></p>
	<div class="back" id="back"><html:link href="javascript:toback();">返回</html:link></div> 
	</div>
	<div>
	...
	<logic:present name="IGPRR01011VO" property="processInfoDefmap">
		...
		<% 
			...
			<%=trEndTag%>
				...
				<input type="hidden" id="rcname" readonly="readonly"/>
	</table>
