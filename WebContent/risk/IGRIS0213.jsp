...
<html:html>
<bean:define id="id" value="IGRIS0213" toScope="request" />
<bean:define id="title" value="单项检查查询画面" toScope="request" />
<!-- header1 -->
...
<!--
	...
	function readInfo(rcrid){
		...
		form.action="IGRIS0206_Read.do?rcrid="+rcrid;
		form.submit();

	}
	
function checkForm(){
	if($F('rccategoryname').trim()==""){
		alert("请选择检查项类别！");
		return false;
	}else {
		return true;
...
function selectRcname(){
	openSubWindow('/IGRIS0201_ResultDisp.do', '_blank', '950', '600');
}
...
<div id="contentWrap">
<div id="content">
<html:form styleId="form" action="/IGRIS0206" onsubmit="return checkSubmit(this);" >
<div id="location">
<div class="image"></div>
<p class="fonts1">
...
<div id="search">
	...
	<div>
	<label><span class="red">*</span>检查项类别：</label>
	...
	<label>检查项名称：</label><html:text property="rcname" styleId="rcname" errorStyleClass="inputError imeDisabled" size="16" readonly="true"/>
	...
	<table class="table_style">
			<tr>
				<th width="15%">检查项类别</th>
				<th width="15%">检查项名称</th>
				<th width="16%">计划检查时间</th>
				<th width="16%">实际检查时间</th>
				<th width="10%">责任组</th>
				<th width="10%">责任人</th>
				<th width="10%">执行人</th>
				<th width="14%">结果</th>
					...
					<tr onmouseover="mouseOver(this);" onmouseout="mouseOut(this,'');" onclick="readInfo('${bean1.rcrid}')">
						...
						<td>
							...
							<%}else if(bean1.getRcrrealtime()!=null && bean1.getRcrrealtime().substring(0,10).compareTo(bean1.getRcrplandate())>0){ %>
								<img src="images/yellow_light.gif" alt="未按时执行检查"  width="16" height="16" />${bean1.rcrrealtime}
							<%}else{%>
								${bean1.rcrrealtime}
							<%}%>
						</td>
