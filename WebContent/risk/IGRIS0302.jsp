...
<html>
<bean:define id="id" value="IGRIS0302" toScope="request" />
<bean:define id="title" value="检查问题发起画面" toScope="request" />
<!-- header1 -->
...
function checkForm(action){
	...
	var name = getFormObj('检查来源').value;
	if(name=="其他监管机构"){
		var nvalue = getFormObj('监管机构名称').value;
...
function initAttFormFile(){
	if('${IGPRR0101Form.parameters}' != ''){
		...
		var objAjax = new Ajax.Request(getAppRootUrl() + ajaxAction,{asynchronous:false, 
			parameters: 'type=getRiskCheckName&rcrid='+rcrid,
		...
		getFormObj("检查工作").value= rcname + "#" + rcrid;
	}else{
		//点击“检查问题发起”菜单隐藏返回超链接
...
// 	}
	getFormObj("检查工作").value=rcid+"#"+rcname+"#"+rcrid;
}
function toback() {
...
<div id="content"><html:form action="/IGPRR0101" styleId="form"
	...
	<div class="back" id="back"><%-- <html:link href="IGCOM0202.do?linkID=IGRIS0203&actname=ACT04RIS0301">返回</html:link> --%>
	<html:link href="javascript:toback();">返回</html:link></div> 
	</div>
	<div>
	...
	<logic:present name="IGPRR01011VO" property="processInfoDefmap">
		...
		<tr>	
			<th><label for="Name"><span class="red"></span><strong>检查工作</strong>：</label></th>
			<td>
				<input type="rcname" id="rcname" readonly="readonly"/>
				<img src="images/seek.gif" border="0" width="16" height="16" alt="查询" onclick="selectRiskCheck()" style="cursor: hand"/>
			</td>
		</tr>
	</table>
