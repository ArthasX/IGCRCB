...
// Should jsProgressBarHandler hook itself to all span.progressBar elements? -
// default : true
var autoHook = true;
...
var defaultOptions = {
	...
	showText : true, // show text with percentage in next to the progressbar?
						// - default : true
	width : 120, // Width of the progressbar - don't forget to adjust your
					// image too!!!
	boxImage : 'images/bramus/percentImage.png', // boxImage : image around
													// the progress bar
	barImage : 'images/bramus/percentImage_back.png', // Image to use in the
														// progressbar. Can be
														// an array of images
														// too.
	height : 12, // Height of the progressbar - don't forget to adjust your
					// image too!!!
...
JS_BRAMUS.jsProgressBar.prototype = {
	...
	backIndex : null, // index in the array of background images currently
						// used
	...
	initialize : function(el, percentage, options) {
		...
		this.imgWidth = this.options.width * 2; // define the width of the image
												// (twice the width of the
												// progressbar)
		this.initialPos = this.options.width * (-1); // Initial postion of
														// the background in the
														// progressbar (0% is
														// the middle of our
														// image!)
		this.pxPerPercent = this.options.width / 100; // Define how much
														// pixels go into 1%
		this.initialPerc = percentage; // Store this, we'll need it later.

		// enfore backimage array
		if (this.options.barImage.constructor != Array) { // used to be (but
															// doesn't work in
															// Safari): if
															// (this.options.barImage.constructor.toString().indexOf("Array")
															// == -1) {
	...
	checkPreloadedImages : function() {
		...
		if (parseInt(this.numPreloaded, 10) >= parseInt(
				this.options.barImage.length, 10)) {
			...
			if (parseInt(this.numPreloaded, 10) <= parseInt(
					this.options.barImage.length, 10)) {
				// $(this.el).update(this.id + ' : ' + this.numPreloaded + '/' +
				// this.options.barImage.length);
				setTimeout(function() {
					this.checkPreloadedImages();
				}.bind(this), 100);
			}
	...
	initVisuals : function() {
		...
		$(this.el).update(
				'<img id="'
						...
						+ this.initialPos
						+ 'px 50%; background-image: url('
						+ this.options.barImage[this.backIndex]
						+ '); padding: 0; margin: 0;" class="percentImage" />'
						+ ((this.options.showText == true) ? '<span id="'
								+ this.id
								+ '_percentText" class="percentText">0%</span>'
								: ''));
	...
	setPercentage : function(targetPercentage, clearQueue) {
		...
		if (clearQueue) {
			...
			this.percentage = (this.queue.length != 0) ? this.queue[0]
					: targetPercentage;
			this.timer = null;
			this.queue = [];
	...
	processQueueEntry : function(targetPercentage) {
		...
		if ((targetPercentage.toString().substring(0, 1) == "+")
				|| (targetPercentage.toString().substring(0, 1) == "-")) {
		...
		// if we don't need to animate, just change the background position
		// right now and return
		if (this.options.animate == false) {

			// remove the entry from the queue
			this.queue.splice(0, 1); // @see:
										// http://www.bram.us/projects/js_bramus/jsprogressbarhandler/#comment-174878
		...
		// define if we need to add/subtract something to the current percentage
		// in order to reach the target percentage
		...
		if (curPercentage != newPercentage) {
			...
			this.timer = setTimeout(function() {
				this.processQueueEntry(targetPercentage);
			}.bind(this), 10);
	...
	_setBgPosition : function(percentage) {
		...
		$(this.id + "_percentImage").style.backgroundPosition = (this.initialPos + (percentage * this.pxPerPercent))
				+ "px 50%";

		// adjust the background image and backIndex
		var newBackIndex = Math.floor((percentage - 1)
				/ (100 / this.options.barImage.length));

		if ((newBackIndex != this.backIndex)
				&& (this.options.barImage[newBackIndex] != undefined)) {
			$(this.id + "_percentImage").style.backgroundImage = "url("
					+ this.options.barImage[newBackIndex] + ")";
		}
...
JS_BRAMUS.jsProgressBarHandler.prototype = {
	...
	initialize : function() {
		...
		$$('span.progressBar').each(
				function(el) {

					// create a progressBar for each element
					this.pbArray[el.id] = new JS_BRAMUS.jsProgressBar(el,
							parseInt(el.innerHTML.replace("%", "")));
