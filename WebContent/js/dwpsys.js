...
function checkProductionChange(){
	...
	if(changeType== '系统网络类' || changeType == '数据类'){
		getFormObj("是否需要UAT验证").parentElement.parentElement.style.display = "none"; 
		jQ(getFormObj("标准环境及UAT变更执行开始时间")).parent().parent().hide().next().hide();
	...
	if(affect=="否"){
		// 业务影响范围表单
		jQ(getFormObj("业务影响范围")).parent().parent().hide();		
	}
}
...
//			jQ("input[name='pivarname'][value='标准环境及UAT变更执行过程']");
//		}
//	}
//}
/**
...
function checkResultsReview(){
	...
	if(getFormObj("是否需要UAT验证").value == '否'){
		getFormObj("标准环境及UAT变更执行开始时间").parentElement.parentElement.style.display = "none";
		getFormObj("标准环境及UAT变更执行结束时间").parentElement.parentElement.style.display = "none";
		jQ("input[name='pivarname'][value='标准环境及UAT变更执行过程']").parent().parent().hide();
	}
	var affect =getFormObj("是否影响业务").value;
	if(affect=="否"){
		// 业务影响范围表单
...
function checkLaunchedSubmit2(){
	...
	if(type == '紧急变更'){
		...
		if(obj != null && jQ(getFormObj("变更原因")).parent().find("[name='pidaccess']").val() == '3'){//如果变更原因为 可写
			if(obj.innerHTML == ''){
				alert("请输入变更原因！");
				return false;
			}
...
function hideBusForms_2(){
	if(!(jQ(getFormObj("所选变更类型")).parent().find("[name='pidaccess']").val() == '1')){
		getFormObj("所选变更类型").parentElement.parentElement.style.display = "none";
	}
	var affect =getFormObj("是否影响业务").value;
	if(affect=="否"){
...
function checkProductionChange_2(){
	...
	if(getFormObj("UAT更新及测试情况").value == '不需要更新UAT'){
		jQ(getFormObj("UAT变更执行开始时间")).parent().parent().hide().next().hide();
		jQ(getFormObj("UAT变更执行过程")).parent().parent().hide().next().hide();
	}
	var affect =getFormObj("是否影响业务").value;
	if(affect=="否"){
		// 业务影响范围表单
	...
	if(getFormObj("所选变更类型").value == '系统网络类'){
		...
		jQ(getFormObj("UAT变更执行开始时间")).parent().parent().hide().next().hide();
		jQ(getFormObj("UAT变更执行过程")).parent().parent().hide().next().hide();
		jQ(getFormObj("标准环境执行结束时间")).parent().parent().hide().next().hide();
		jQ(getFormObj("标准环境执行过程")).parent().parent().hide().next().hide();
	}
}
...
jQ(document).ready(function(){
	if(jQ("#logAtt").length > 0){
		var params= {};
		params.prid = jQ("#prid").val();
		//ajax请求数据
		jQ.ajax({
			url:"IGCRC0405.do",
			type:"POST",
			...
			success:function(data){
				if(data.status == "success"){
					for(var i=0;i<data.content.length;i++){
						jQ("#logAtt").append("<a href='downloadSvcFile.do?attId=" + data.content[i].attid + "&type=svc'>" + data.content[i].attname + "</a>");
					}
				}
			},
			//失败回调函数
...
function replaceRedClass(pidName){
	var replaceObj = jQ("#" + getFormObj(pidName).id).parent().prev().find("strong");
...
//	var params = getQueryString("params");
	...
	if(params != null && params != ''){
		...
		//变更名称
		jQ("input[name='prtitle']").val(title);
		//变更描述
		jQ("input[name='prdesc']").val(desc);
		jQ("input[name='prdesc']").prev().html(desc);
		//计划执行开始时间0108405002
		jQ("#pidid"+pdid+"002").val(begintime);
		
		//变更类型 0108405046
		jQ("#pidid"+pdid+"035").val(tree.split("_tree_")[1]);
		jQ("#pidid"+pdid+"035").next().next().val(tree);
		
		//变更类别0108405046
		jQ("#pidid"+pdid+"046").val(type1);
		//变更分类0108405065
		jQ("#pidid"+pdid+"065").val(type2);
		//更新平台0108405063
		var str1 = null;
		var str2 = null;
		...
		jQ("[name = pivarcheckbox[6]]:checkbox").each(function () {
			if(jQ(this).parent().text() == str1){
				jQ(this).click();
				jQ(this).focus();
				if(str2 != null && str2 != ""){
					jQ(this).next().val(str2);
					jQ(this).next().change();
				}
			}
        });
		...
		//变更原因分类0108405071
		jQ("#pidid"+pdid+"071").val(type4);
		//变更原因0108405036
		jQ("#pidid"+pdid+"036").html(text1);
		jQ("#pidid"+pdid+"036").focus();
		//变更内容0108405037
		jQ("#pidid"+pdid+"037").html(text2);
		jQ("#pidid"+pdid+"037").focus();
		//是否影响业务0108405022
		jQ("#pidid"+pdid+"022").val(type5);
		//执行计划0108405042
		jQ("#pidid"+pdid+"042").html(text3);
		jQ("#pidid"+pdid+"042").focus();
		//回退计划0108405043
		jQ("#pidid"+pdid+"043").text(text4);
		jQ("#pidid"+pdid+"043").focus();
		//验证计划0108405044
		jQ("#pidid"+pdid+"044").text(text5);
		jQ("#pidid"+pdid+"044").focus();
