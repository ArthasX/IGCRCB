...
function ex(d,high){
	if(dsp(d)=='none'){
		dsp(d,'block');
		if(high!=undefined){
			d.style.height=high+'px';
		}else{
			d.style.height='0px';
		}
		clearInterval(d.t);
		d.t=setInterval('et("'+d.id+'",'+high+')',t);
	}
}
...
function Accordian(ds,s,tc,num){
	// get all the elements that have id as content
	
	for(var nn=1;nn<=num;nn++){
		var d=ds+nn;
		
		var l=getID(d).getElementsByTagName('div');
		c=[];
		...
		for(i=0;i<l.length;i++){
			...
			if(h.substr(h.indexOf('-')+1,h.length)=='header'){
				d=getID(h.substr(0,h.indexOf('-'))+'-content');
