...
function writeIframe()
{
    ...
    "</tr></table>"+
    "</td></tr>" ;
    strIframe+="<tr><td height=20 align='center'><table id=tableWeek border=1 width='98%' cellpadding=0 cellspacing=0 ";
    ...
    "    <table id=tableDay height='100%' width='98%' border=0 cellspacing=1 cellpadding=0>";
         ...
         for(var y=0; y<7; y++)  strIframe += "<td class=out style='cursor:hand' id='meizzDay"+ (x*7+y) +"'></td>"; strIframe += "</tr>";}
         strIframe += "<tr>";
         for(var x=35; x<42; x++) strIframe += "<td style='cursor:hand' class=out id='meizzDay"+ x +"'></td>";
         strIframe +=
         "</tr></table>"+
    "</td></tr><tr><td height=20  align='center' bgcolor='"+ WebCalendar.lightColor +"'>"+
    "    <table border=0 cellpadding=1 cellspacing=0 width='98%'>"+
    "    <tr align=center>"+
    "        <td title='向前翻 1 小时' onclick='parent.prevH()' style='cursor: hand' class=bg><<</td>"+
    ...
    "        onmouseover='this.bgColor=parent.WebCalendar.darkColor; this.style.color=parent.WebCalendar.lightColor'"+
    "        onmouseout='this.bgColor=parent.WebCalendar.lightColor; this.style.color=parent.WebCalendar.wordColor'></td>"+
    "        <td width=15 title='向后翻 1 分钟' onclick='parent.nextMi()' style='cursor: hand' class=bg>></td>"+
    ...
    " type=button value='确定'title='确定' onclick='parent.returnDate();'/>" +
    "<input style=' background-color: "+
    WebCalendar.btnBgColor +";cursor: hand; padding-top: 4px; width: 33%; height: 100%; border: 0' "+
    " type=button value='清空'title='清空' onclick='parent.hiddenCalendar(1);'/>" +
    "<input style=' background-color: "+
        WebCalendar.btnBgColor +";cursor: hand; padding-top: 4px; width: 33%; height: 100%; border: 0' onfocus='this.blur()'"+
        " type=button value='关闭'title='关闭' onclick='parent.hiddenCalendar()'/></td>"
    "</tr>" +
...
function calendar(obj) //主调函数
{
 ...
 while (e = e.offsetParent){t += e.offsetTop; l += e.offsetLeft;}
    ...
    else WebCalendar.dateReg = /^(\d{1,4})(-|\/)(\d{1,2})\2(\d{1,2}) (\d{1,2}):(\d{1,2})$/;
   // else WebCalendar.dateReg = /^(\d{1,4})$/;
    try{
...
function hiddenCalendar(flag){getObjectById("meizzCalendarLayer").style.display = "none";if(flag==1){WebCalendar.objExport.value="";if(WebCalendar.objExport.onchange){WebCalendar.objExport.onchange();}}};
...
function writeCalendar() //对日历显示的数据的处理程序
{
 ...
 //   WebCalendar.iframe.meizzSecondHead.innerText =appendZero(parseInt(s, 10))+" 秒";
    ...
    for(var i=0; i<42; i++)    //这个循环是根据源数组写到日历里显示
    {
        ...
        WebCalendar.dayObj[i].innerHTML    = a[0];//"<span ondblclick='parent.returnDate();'>"+a[0]+"</span>";
        WebCalendar.dayObj[i].title        = a[2] +"-"+ appendZero(a[1]) +"-"+ appendZero(a[0]);
        WebCalendar.dayObj[i].ondblclick   = returnDate
...
function selectDate()
{
    if(lastCell==null){
        ...
        if (WebCalendar.day[lastCell] == WebCalendar.today)      //设置今天在日历上反应出来的颜色
...
function returnDate(flag) //根据日期格式等返回用户选定的日期
{
    ...
    if(WebCalendar.objExport)
    {
        ...
        if(WebCalendar.objExport.onchange){
        	WebCalendar.objExport.onchange();
        };
    };
