...
! function(a,$) {
    ...
    a.laydate = function(b) {
        ...
        var e, b, h, i, j;
        if(!$.support.leadingWhitespace && !$.support.inlineBlockNeedsLayout){//解决IE8对getElementsByClassName方法不兼容的问题
        	return a = c.trim(a).split(" "), b = d[f](a[0].substr(1)), b ? a[1] ? /^\./.test(a[1]) ? (i = a[1].substr(1), j = new RegExp("\\b" + i + "\\b"), e = [], h = d.getClassNames ? b.getClassNames(i) : b[g]("*"), c.each(h, function(a, b) {
        ...
        if ("" !== d.replace(/\s/g, "")) {
            ...
            if(c.options.isMonth){
            	f.value = c.elem[h.elemv].match(b).join(), e = f.value.match(/\d+/g), e[1] < 1 ? (e[1] = 1, f.auto = 1) : e[1] > 12 ? (e[1] = 12, f.auto = 1) : e[1].length < 2 && (f.auto = 1);
            }else{
        ...
        var $this = c.options.obj;//是否是边缘,默认false
		if($this){
			var $width = c.options.obj.clientWidth;//是否是边缘,默认false
			a.style.left = e.left + ((b ? 0 : c.scroll(1))-(240-$width-2)) + "px",
        ...
        c.addClass(d.body, "laydate_body"), h.tds = b("#laydate_table td"), h.mms = b("#laydate_ms span"), h.year = b("#laydate_y"), h.month = b("#laydate_m"), c.each(b(e.box + " .laydate_ym"), function(a, b) {
            ...
            "function" == typeof c.options.choose && c.options.choose("");
        }), h.otoday = b("#laydate_today"), c.on(h.otoday, "click", function() {
            c.elem[h.elemv] = laydate.now(0, c.options.format), c.close();
            "function" == typeof c.options.choose && c.options.choose(laydate.now(0, c.options.format));
...
function showMonth(obj){
    jQuery("#laydate_table").css("display","none");
    laydate({istime:false,festival:false,format:'YYYY/MM',isMonth:true,
        choose:function(datas){
            jQuery("#laydate_table").css("display","block");
            jQuery(obj).val(datas);
            jQuery(obj).change();
        }});
}
...
function calendar(obj){
    ...
    if(WebCalendar.format != null){
        formater = WebCalendar.format;
    }
    var istimeShow = true;
    if(WebCalendar.timeShow != null){
        istimeShow = WebCalendar.timeShow;
    }
    laydate({istime:istimeShow,format:formater,
        choose:function(datas){
            jQuery(obj).val(datas);
            jQuery(obj).change();
        }});
