...
%>
<html:html>
<bean:define id="id" value="IGSVC0602" toScope="request" />
...
function checkForm(){
	...
	if(!checkDate("propentime_from","propentime_to")){
		...
		document.forms[0].action = getAppRootUrl() + "/IGSVC0603.do";
...
function exportExcel() {
	if(!checkDate("propentime_from","propentime_to")){
		...
		document.forms[0].action = getAppRootUrl() + "/IGSVC0603.do?report=1";
		document.forms[0].submit();
	}
}
</script> 
...
<!--content  右侧内容-->
   ...
   <div id="location">
     ...
     <div id="search">
   <html:form styleId="form" action="/IGSVC0603" onsubmit="return checkSubmit(this);">
         <div class="img"></div>
                            ...
                            <logic:present name="IGSVC05131VO" property="processCountList">
								<logic:iterate id="bean" name="IGSVC05131VO"
									...
									<tr class="<ig:rowcss index="${index}"/>">
										...
										%>
										<a href="IGSVC0604.do?prassetid=${bean.prassetid}&type=003&prtype=I&propentime_from=<bean:write name="IGSVC0602Form" property="propentime_from"/>&propentime_to=<bean:write name="IGSVC0602Form" property="propentime_to"/> 23:59&modes=1"> ${bean.incident}</a>		
										<%
											}
										...
										%>	
										<a href="IGSVC0604.do?prassetid=${bean.prassetid}&type=003&prtype=P&ppropentime_from=<bean:write name="IGSVC0602Form" property="propentime_from"/>&propentime_to=<bean:write name="IGSVC0602Form" property="propentime_to"/> 23:59&modes=1"> ${bean.problem}</a>	
										<%
											}
										%>
										</div>
										...
										%>
										<a href="IGSVC0604_1.do?prassetid=${bean.prassetid}&type=003&prtype=C&propentime_from=<bean:write name="IGSVC0602Form" property="propentime_from"/>&propentime_to=<bean:write name="IGSVC0602Form" property="propentime_to"/> 23:59&modes=1"> ${bean.change}</a>	
										<%
											}
										%>
										</div>
										...
										%>
										<a href="IGSVC0604_2.do?prassetid=${bean.prassetid}&type=003&prtype=W&propentime_from=<bean:write name="IGSVC0602Form" property="propentime_from"/>&propentime_to=<bean:write name="IGSVC0602Form" property="propentime_to"/> 23:59&modes=1"> ${bean.work}</a>	
										<%
											}
										%>
										</div>
