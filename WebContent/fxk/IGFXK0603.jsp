...
<bean:define id="id" value="IGFXK0603" toScope="request" />
...
%>
			...
			<logic:iterate id="map" name="IGPRR01021VO" property="processInfoDefmap" indexId="index" >
				...
				<%
					if("0100201003".equals(psid.getPidid())){
				%>
				<input type="hidden" id="pidid0100201003" name="pivarvalue[${index}]" value="${psdid.pidvalue }"/>
				<%
					if(IGPRDCONSTANTS.PUBLIC_ACCESS_WRITEABLE.equals(psid.getPidaccess())) {
				%>
				<bean:define id="showflag" value="1"/>
				<logic:present name="IGPRR01021VO" property="processInfoEntityList">
					<logic:iterate id="bean" name="IGPRR01021VO" property="processInfoEntityList">
						<bean:define id="showflag" value="0"/>
						<a id="${bean.pieid }" href="javascript:openFxsj(${bean.eiid });">${bean.entityItemVWTB.einame }</a>
						<img id="${bean.pieid }ENT" src="images/delate.gif" align="middle" border="0" style="cursor: hand;" alt='删除' onclick="deleteEntityFx(${bean.pieid});"/>
					</logic:iterate>
				</logic:present>
				<logic:equal value="1" name="showflag">
					<table id="tbentity0100201003" class="tableColumnEntity">
						<tr>
							<td>
								<input type="text" readonly="readonly" name="entityName[0]" asm="0100201003"/>
								<img src="images/seek.gif" width="16" height="16" style="cursor: hand;" alt="查询" onclick="selectFxsj();"/>
								<input type="hidden" name="entityId[0]"/>
								<input type="hidden" name="0100201003"/>
							</td>
						</tr>
					</table>
				</logic:equal>
				<logic:equal value="0" name="showflag">
					<table id="tbentity0100201003" class="tableColumnEntity">
					</table>
				</logic:equal>
				<%} else if(IGPRDCONSTANTS.PUBLIC_ACCESS_READABLE.equals(psid.getPidaccess())){%>
				<logic:present name="IGPRR01021VO" property="processInfoEntityList">
					<logic:iterate id="bean" name="IGPRR01021VO" property="processInfoEntityList">
						<a href="javascript:openFxsj(${bean.eiid });">${bean.entityItemVWTB.einame }</a>
					</logic:iterate>
				</logic:present>
				<%}} else { %>
					<ig:formToTag 
...
<%-- 	</logic:notEqual> --%>
	...
	<html:hidden property="pieids" styleId="pieids" value=""/>
