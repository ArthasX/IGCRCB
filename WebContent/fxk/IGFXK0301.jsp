...
<html>
<bean:define id="id" value="IGFXK0301" toScope="request" />
<bean:define id="title" value="风险计划登记" toScope="request" />
<!-- header1 -->
...
<script type="text/javascript" src="js/prr/<%=(vo.getProcess()!=null && vo.getProcess().getPrpdid()!=null) ? vo.getProcess().getPrpdid().substring(0,5) : "" %>.js" charset="GBK"></script>
</logic:present>
...
function checkForm(action,vbrequired){
	...
	/* if(!checkDateWithTimeShow("prplantime","prduration")){
		...
		//获取表单计划开始时间的值
		var popentime=getFormObj("计划开始日期").value;
		//获取表单计划结束时间的值
		var pclosetime=getFormObj("计划结束日期").value;
...
function person_sel(){
	...
	//生成下拉列表中的值(option)
	var option_user = '<option value = "" ></option>';
	...
	for ( var row_index = 0; row_index < obj_select.length; row_index++) {
		//obj_select[row_index].parentElement.style.display="none";
		//obj_select[row_index].parentElement.parentElement.parentElement.childNodes[1].childNodes[1].style.display="none";
		...
		if(row_index == obj_select.length - 1){
			...
			var abj_manager = obj_select[row_index].parentElement.parentElement.childNodes[3];
			if(abj_manager.getElementsByTagName("option")[1].value = '负责人'){
				//清空本对象
				abj_manager.getElementsByTagName("select")[0].innerHTML = "";
				//获取下拉列表ID
...
function initTable_z(){
	var tb = document.getElementById(arr_pidlabelObj["工作列表"] + "_tableForm");
	if(tb != null){
		tb.style.width = "790px";
		tb.onAdd = function(tr){
			tr.cells[8].style.display = "none";
		}
		for(var i=0;i<tb.rows.length;i++){
			if(tb.rows[i].cells.length > 8){
				tb.rows[i].cells[8].style.display = "none";
			}
		}
	}
}
//====01003 风险计划 End  initAttFormFile();

</script>
<body onload="initAttFormFile();initTable_01003();">
