...
<html>
<bean:define id="id" value="IGFXK0502" toScope="request" />
<bean:define id="title" value="风险清单" toScope="request" />
<!-- header1 -->
...
function immediately(){
	var element = document.getElementsByName("pivarvalue[5]")[0];  //获取元素对象
	element.onfocus = openRiskCheck;	
}

function openRiskCheck(){
	if("" != document.getElementsByName("show_value")[0].value){
		var returnValue = window.showModalDialog("NewBox.jsp",'_blank','1000','600');
		//openSubWindow('/IGSVC0107_Disp.do', '_blank','1000','600');
		if(returnValue != null){
			var riskCheck = returnValue.split('_$_');
			document.getElementById("").value = riskCheck[0];
			document.getElementById("").value = riskCheck[1];
		}
	}
}
