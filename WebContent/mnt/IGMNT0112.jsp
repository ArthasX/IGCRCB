...
<html:html>
<bean:define id="id" value="IGMNT0112" toScope="request" />
<bean:define id="title" toScope="request">
	<bean:message bundle="mntResources" key="ietitle.IGMNT0112" />
</bean:define>
<!-- header1 -->
...
<script type="text/javascript">
	function chkform(){
		...
		for(var i=0;i<test1.length;i++){
			...
			if(test1[i].type=="text"&&test1[i].id!="objectName"&& !reg.test(test1[i].value)){
				alert('<bean:message bundle="mntResources" key="message.IGMNT0112.numOnly" />');
				return false;
			}
		}
		if(count==0){
			alert('<bean:message bundle="mntResources" key="message.IGMNT0112.chkbox" />');
			return false;
		}
	}
	...
	function confirmUpdate(){
		...
		if(window.confirm('<bean:message bundle="mntResources" key="message.IGMNT0112.confirmUP" />')){
			document.forms[0].action="IGMNT0112_Update.do";
...
<div id="contentWrap">
<div id="content"><html:form styleId="form" action="/IGMNT0112"
	...
	<html:select name="IGMNT0112Form" styleId="name" property="name" style="width: 130px;" errorStyleClass="inputError">
		<logic:present name="ADHostList" >
			<html:options collection="ADHostList" property="value" name="" labelProperty="label" />
		</logic:present>
	</html:select>
	<label>
	&nbsp;&nbsp;<bean:message bundle="mntResources" key="label.IGMNT0104.monitorType" />：
	</label> 
	<html:select name="IGMNT0112Form" styleId="objectType" property="objectType" style="width: 130px;" errorStyleClass="inputError">
	...
	<table class="table_style2">
		<!-- header部 -->
		<tr>
			<th width="4%"><input type="checkbox" id="mysel" onclick="selectAll('mysel','selArr')" ></th>
			<th><bean:message bundle="mntResources"
				key="title.IGMNT0112.hostName" /></th>
			<th><bean:message bundle="mntResources"
				key="title.IGMNT0112.capType" /></th>
			<th><bean:message bundle="mntResources"
				key="label.IGMNT0104.moName" /></th>
			<th><bean:message bundle="mntResources"
				key="title.IGMNT0112.custtomErrorNum" /></th>
			<th><bean:message bundle="mntResources"
				key="title.IGMNT0112.custtomWarningNum" /></th>
		</tr>
		<!-- body部 -->
		<logic:present name="IGMNT01121VO" property="soc0423List">
			<logic:iterate id="bean" name="IGMNT01121VO"
				property="soc0423List" indexId="index">
	...
	<div id="operate">
	<logic:present name="IGMNT01121VO" property="soc0423List">
		<input type="button" value="<bean:message bundle="mntResources" key="button.IGMNT0112.updateButton" />" onclick="return confirmUpdate()" class="button"/><!-- paging --> <jsp:include page="/include/paging.jsp" /> <!-- /paging -->
	</logic:present> 
	</div>
</html:form></div>
</div>
