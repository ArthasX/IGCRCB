 ...
 * 创建记录：史凯龙 2014/07/20
 */
public class IGCRC06ExcelBLImpl extends ExcelBaseBLImpl implements
		IGCRC06ExcelBL {
	/** 日志对象 */
	private static final Log log = LogFactory.getLog(IGCRC06ExcelBLImpl.class);

	/**
	 * 获取导出相应信息编写Excel文件，并输出。
	 */
	public ExcelDTO initBody(ExcelDTO dto) throws BLException {
		log.debug("=================获取业务服务请求管理导出开始=================");
		IGCRC06ExcelDTO exceldto = (IGCRC06ExcelDTO) excelDTO;
		// 设置起始行数
...
//				String prdesc = info.getPrdesc();
				String prdesc = piMap.get(info.getPrid()).get("服务请求描述").getPivarvalue();
				...
				addCell(2, i, prdesc);
				// 填充服务请求分类
				String prservicetype = piMap.get(info.getPrid()).get("服务请求分类").getPivarvalue();
				if(StringUtils.isNotEmpty(prservicetype)){
					prservicetype = prservicetype.split("_tree_")[1];
				}
				addCell(3, i, prservicetype);
				// 填充工单状态
				if(IGPRDCONSTANTS.PROCESS_TERMINATE.equals(info.getPrstatus())){
					addCell(4, i, "中止");
				}else{
					addCell(4, i, psdMap.get(info.getPrstatus() + "-" + info.getPrpdid()).getPsdname());
				}
