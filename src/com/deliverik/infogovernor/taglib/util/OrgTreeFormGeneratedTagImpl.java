...
public class OrgTreeFormGeneratedTagImpl extends FormGeneratedTagImpl{
	...
	public String generatedPublicWriteableTag(Map<String, Object> params){
		...
		tagString.append("<input type=\"text\" id=\"pidid")
				 ...
				 .append(StringUtils.isNotEmpty(value)&&value.contains("_OrgTree_")?value.split("_OrgTree_")[1]:"")
				 .append("\" readonly=\"true\" />")
				 ;
		tagString.append("<img src=\"images/tree.gif\" alt=\"查询\" onclick=\"selectOrgTree('")
				 .append(index)
	...
	public String generatedPublicReadableTag(Map<String, Object> params){
		...
		//标签的生成信息
		StringBuffer tagString = new StringBuffer(StringUtils.isNotEmpty(value)&&value.contains("_OrgTree_")?value.split("_OrgTree_")[1]:"");
		...
		if(StringUtils.isNotEmpty(privatescope)){
			...
			if(list!=null&&!list.isEmpty()){
				for(IG899Info ppi:list){
					...
					tagString.append(StringUtils.isNotEmpty(ppi.getPpivalue())&&ppi.getPpivalue().contains("_OrgTree_")?ppi.getPpivalue().split("_OrgTree_")[1]:"");
					if(StringUtils.isNotEmpty(unit)){
						tagString.append("<span>"+unit+"</span>");
					}	
					if(StringUtils.isNotEmpty(remarks) && REMARK_TAG.equals(remarks)){
