...
import com.deliverik.infogovernor.asset.form.IGASM1401Form;
import com.deliverik.infogovernor.asset.vo.IGASM14011VO;
 ...
 *
 */
public class IGASM1401Action extends BaseAction {

	static Log log = LogFactory.getLog(IGASM1401Action.class);
	...
	public ActionForward doProcess(ActionMapping mapping, ActionForm actionForm, HttpServletRequest req, HttpServletResponse res) throws Exception {
		//FORM取得
		IGASM1401Form form = (IGASM1401Form)actionForm;
		...
		if( "DISP".equals(mapping.getParameter())){
			...
			//***************************************************************************
			//类型分类(服务记录)设定
			form = new IGASM1401Form();
			form.setEcategory(CommonDefineUtils.ENTIY_CATEGORY_SERVICE_RECORD);
			req.setAttribute("IGASM1401Form", form);
			//***************************************************************************
			
			addMessage(req, new ActionMessage("IGCO10000.I003"));
			log.debug("========服务记录查询画面初期显示处理终了========");
			return mapping.findForward("DISP");
		} else if("REDISP".equals(mapping.getParameter())){
			//服务记录查询画面再显示处理
			log.debug("========服务记录查询画面再显示处理开始========");
			addMessage(req, new ActionMessage("IGCO10000.I003"));
			log.debug("========服务记录查询画面再显示处理终了========");
			return mapping.findForward("DISP");
		}	

		if( "SEARCH".equals(mapping.getParameter())||"SEARCH1".equals(mapping.getParameter())){
			//分页用DTO取得
			getPaginDTO(req,"IGASM1401");
			
			PagingDTO pDto;
			
			pDto = (PagingDTO)req.getAttribute("PagingDTO");
			//服务记录查询处理
			log.debug("========服务记录查询处理开始========");
			if ("SEARCH1".equals(mapping.getParameter())){
				//由详细画面，编辑画面返回查询画面时的再检索处理
				form = (IGASM1401Form) req.getSession().getAttribute("IGASM1401Form");
				if ( form== null){
					form = new IGASM1401Form();
					form.setEcategory(CommonDefineUtils.ENTIY_CATEGORY_SERVICE_RECORD);
				}else {
			...
			//查询记录最大件数取得
			int maxCnt = getMaxDataCount("IGASM1401");
			...
			//将服务记录信息检索结果设定到VO中
			IGASM14011VO vo = new IGASM14011VO(dto.getEntityItemVWInfoList());
			
			super.<IGASM14011VO>setVO(req, vo);
			
			log.debug("========服务记录查询处理终了========");
		}
		if ("DELETE".equals(mapping.getParameter())){
			//服务记录删除处理
			log.debug("========服务记录删除处理开始========");
