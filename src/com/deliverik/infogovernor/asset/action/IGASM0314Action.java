...
import com.deliverik.infogovernor.asset.vo.IGASM03141VO;

/**
 * 设备下发画面Action处理
 * 
 */
public class IGASM0314Action extends BaseAction {

	static Log log = LogFactory.getLog(IGASM0314Action.class);
	...
	public ActionForward doProcess(ActionMapping mapping,
		...
		// FORM取得
		IGASM0314Form form = (IGASM0314Form) actionForm;
		// BL取得
		...
		if ("DISP".equals(mapping.getParameter())) {
			...
			dto.setUser(user);
			dto.setIgasm0314Form(form);
			dto = ctlBL.searchScrapEntityItemAction(dto);
			IGASM03141VO vo = new IGASM03141VO(dto.getEntityItemVWInfo(),dto.getFlag());
			super.<IGASM03141VO> setVO(req, vo);
		} else if ("INSERT".equals(mapping.getParameter())) {
			User user = (User) req.getSession().getAttribute(SESSION_KEY_LOGIN_USER);
			dto.setIgasm0314Form(form);
			dto.setUserid(user.getUserid());
			dto.setUsername(user.getUsername());
			ctlBL.entityScrapAction(dto);
			...
			ctlBL.entityScrapBatchAction(dto);
			req.getSession().removeAttribute("ADSelectEntity");
			req.getSession().removeAttribute("ADSelectedEntity");
			log.debug("========设备查询画面初期显示处理终了========");
		}
