...
import com.deliverik.infogovernor.asset.form.IGASM0601Form;
import com.deliverik.infogovernor.asset.vo.IGASM06011VO;
 ...
 *
 */
public class IGASM0601Action extends BaseAction {

	static Log log = LogFactory.getLog(IGASM0601Action.class);
	...
	public ActionForward doProcess(ActionMapping mapping, ActionForm actionForm, HttpServletRequest req, HttpServletResponse res) throws Exception {
		//FORM取得
		IGASM0601Form form = (IGASM0601Form)actionForm;
		...
		if( "DISP".equals(mapping.getParameter())){
			//软件查询画面初期显示处理
			log.debug("========软件查询画面初期显示处理开始========");
			
			//***************************************************************************
			//类型分类(软件)设定
			form = new IGASM0601Form();
			form.setEcategory(CommonDefineUtils.ENTIY_CATEGORY_SOFTWARE);
			req.setAttribute("IGASM0601Form", form);
			...
			if(dto.getFlag() && EntityCategory.ENTITYITEM_PERMISSION_MODE_A.equals(ResourceUtility.getString(EntityCategory.ENTITYITEM_PERMISSION_MODE))) {
				...
				String roletype = req.getParameter("roletype")==null?"IGAC06":req.getParameter("roletype");
				...
				sym03dto1.setSubtype("999002");
				String roletype1 = req.getParameter("roletype")==null?"IGAC06":req.getParameter("roletype");
			...
			log.debug("========软件查询画面初期显示处理终了========");
			return mapping.findForward("DISP");
		} else if("REDISP".equals(mapping.getParameter())){
			//软件查询画面再显示处理
			log.debug("========软件查询画面再显示处理开始========");
			addMessage(req, new ActionMessage("IGCO10000.I003"));
			log.debug("========软件查询画面再显示处理终了========");
			return mapping.findForward("DISP");
		}	

		if( "SEARCH".equals(mapping.getParameter())||"SEARCH1".equals(mapping.getParameter())){
			//分页用DTO取得
			getPaginDTO(req,"IGASM0601");
			...
			if ("SEARCH1".equals(mapping.getParameter())){
				...
				form = (IGASM0601Form) req.getSession().getAttribute("IGASM0601Form");
				if ( form== null){
					form = new IGASM0601Form();
					form.setEcategory(CommonDefineUtils.ENTIY_CATEGORY_SOFTWARE);
				}else {
			...
			//查询记录最大件数取得
			int maxCnt = getMaxDataCount("IGASM0601");
			...
			//将软件信息检索结果设定到VO中
			IGASM06011VO vo = new IGASM06011VO(dto.getEntityItemVWInfoList());
			
			super.<IGASM06011VO>setVO(req, vo);
