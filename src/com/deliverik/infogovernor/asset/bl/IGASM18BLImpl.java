 ...
 *
 */
public class IGASM18BLImpl extends BaseBLImpl implements IGASM18BL {

	static Log log = LogFactory.getLog(IGASM18BLImpl.class);
	...
	public static final String EIRRELATIONCODE="'002-006','003-006','006-005','006-011','006-010','012-006','013-006','006-006','001-006'";
...
//	protected IG225BL soc0109BL;
	...
	public IGASM18DTO insertEntityItemAction(IGASM18DTO dto) throws BLException {
		log.debug("========系统信息登记处理开始========");
		//输入Form取得
		IGASM1802Form form = dto.getIgasm1802Form();
		...
		//系统信息登录
		SOC0118Info entityItem = this.soc0118BL.registEntityItem(dto.getIgasm1802Form());
		
		form.setEiidStr(entityItem.getEiid().toString());
		
		dto.setIgasm1802Form(form);
		
		dto.addMessage(new ActionMessage( "IGCO10000.I004", "系统基本信息") );

		log.debug("========系统信息登记处理终了========");
		return dto;
	}
	...
	public IGASM18DTO deleteEntityItemAction(IGASM18DTO dto) throws BLException {
		...
		for( int i=0;i<dto.getDeleteEntityItem().length;i++ ){
			...
			if (entityItem == null){
				throw new BLException("IGCO10000.E004","删除对象系统基本");
			}
		...
		dto.addMessage(new ActionMessage("IGCO10000.I006","系统信息"));

		log.debug("========系统信息删除处理终了========");
		return dto;
	}
	...
	public IGASM18DTO initIGASM1804Action(IGASM18DTO dto) throws BLException {
		...
		log.debug("========系统配置信息编辑画面初期化处理开始========");
		
		IGASM1804Form form = dto.getIgasm1804Form();
		...
		if ( entityItemVWInfo == null ) {
			throw new BLException("IGCO10000.E004","系统");
		}
...
//		dto.addMessage(new ActionMessage("IGCO10000.I007",totalCount));
	 ...
	 */
	public IGASM18DTO editEntityItemAction(IGASM18DTO dto) throws BLException, FileNotFoundException, IOException {
		...
		if(StringUtils.isEmpty(ident)){
			ident = IGASM18BLImpl.SAVE_VERSION;
		}
		// 版本号初始化
		int version = 1;
		...
		// 判断是否升级版本号
		// =====================>> 修改 start
		if (IGASM18BLImpl.SAVE_VERSION_UP.equals(ident)) { // 升级版本号
		...
		} else if (IGASM18BLImpl.SAVE_VERSION.equals(ident)) { // 不升级版本号
...
//		entityItem.setEistatus(form.getEistatus());
		...
		if(version ==1&&versonFlag){
			...
			//与该资产相关的关系码
			eircond1.setEirrelationcode(IGASM18BLImpl.EIRRELATIONCODE);
		...
		dto.addMessage(new ActionMessage("IGCO10000.I005","系统配置信息"));
	...
	public IGASM18DTO initIGASM1805Action(IGASM18DTO dto) throws BLException {
		log.debug("========系统配置信息历史记录画面初期化处理开始========");
		
		//系统信息检索
		SOC0124Info entityItemVWInfo = this.soc0118BL.searchEntityItemVWByKey(
			dto.getIgasm1805Form().getEiid());
		...
		if ((dto.getIgasm1805Form().getCiversion())!=null) {
			dto.getIgasm1805Form().setCiversion(String.valueOf(entityItemVWInfo.getEiversion()));
		}
		//获取历史版本更新时间
		dto.setConfigItemVersionTime(this.soc0118BL.searchVersionUpdateTime(dto.getIgasm1805Form().getEiid()));
	...
	public IGASM18DTO initIGASM1806Action(IGASM18DTO dto) throws BLException {
		log.debug("========系统关系管理画面初期化处理开始========");
		
		//系统信息检索
		SOC0124Info entityItemVWInfo = this.soc0118BL.searchEntityItemVWByKey(
				Integer.parseInt(dto.getIgasm1806Form().getEiid()));
		...
		if((dto.getIgasm1806Form().getEiid())!=null){
		parcond.setPareiid(Integer.parseInt(dto.getIgasm1806Form().getEiid()));}
...
//		
//		cldcond.setCldeiid(dto.getIgasm1806Form().getEiid());
...
//		}
	...
	public IGASM18DTO deleteEntityItemRelationAction(IGASM18DTO dto) throws BLException {
		...
		for( int i=0;i<dto.getDeleteEntityItemRelation().length;i++ ){
			...
			if (entityItemRelation == null){
				throw new BLException("IGCO10000.E004","删除对象系统关系");
			}

			this.soc0119BL.deleteEntityItemRelation(entityItemRelation);
		}
		
		dto.addMessage(new ActionMessage("IGCO10000.I006","系统关系信息"));
	...
	public IGASM18DTO insertEntityItemRelationAction(IGASM18DTO dto) throws BLException {
		...
		IGASM1807Form form = dto.getIgasm1807Form();
		...
		if ( entityItemVWList==null || entityItemVWList.size() == 0 ) {
			throw new BLException("IGCO10000.E004","(资产编号=" + form.getElabel() + ",系统编号=" 
					+ form.getEilabel() + ")系统");
		}
		...
		//资产关系登记
		SOC0119Info ret = 
			this.soc0119BL.registEntityItemRelation(dto.getIgasm1807Form());
		
		dto.setEntityItemRelation(ret);
		
		dto.addMessage(new ActionMessage( "IGCO10000.I004", "系统关系信息") );
	...
	public IGASM18DTO initIGASM1807Action(IGASM18DTO dto) throws BLException {
		...
		if ( ret == null ){
			throw new BLException("IGCO10000.E004","编辑对象系统关系");
		}
	...
	public IGASM18DTO updateEntityItemRelationAction(IGASM18DTO dto) throws BLException {
		...
		dto.addMessage(new ActionMessage( "IGCO10000.I005", "系统关系信息") );

		log.debug("========系统关系变更处理处理终了========");
		return dto;
	}
	...
	public IGASM18DTO searchRelationListAction(IGASM18DTO dto) throws BLException {
		log.debug("========获取系统关联关系处理开始========");
		List<LabelValueBean> list = this.codeDetailBL.searchLabelValueBeanList(CommonDefineUtils.ENTIY_CATEGORY_SYSTEM,CodeDefine.getCodeDefine("ENTITY_ITEM_RELATION_CODE").getCcid());
		dto.setRelationList(list);
		log.debug("========获取系统关联关系处理终了========");
		return dto;
	}
	...
	public IGASM18DTO initEntityItem(IGASM18DTO dto) throws BLException {
		...
		log.debug("========系统信息编辑画面初期显示处理开始========");
		
		IGASM1812Form form = dto.getIgasm1812Form();
		
		//显示用资产信息取得
		SOC0124Info entityItem = this.soc0118BL.searchEntityItemVWByKey(form.getEiid());
		
		if ( entityItem == null ){
			throw new BLException("IGCO10000.E004","变更系统基本");
		}
		...
		boolean isRoleManager = this.userRoleBL.checkRoleManagerByType(dto.getUser().getUserid(), EntityCategory.ROLE_TYPE_SYSTEM);
		form.setRoleManager(isRoleManager);
		form.setPermission(permission);
		
		dto.setIgasm1812Form(form);
		
		//dto.addMessage(new ActionMessage("IGCO10000.I001","系统基本信息"));
		if(!permission) {
			dto.addMessage(new ActionMessage("IGCO10000.E023","修改"));
		}
		log.debug("========系统信息编辑画面初期显示处理终了========");
		return dto;
	}
	/**
	 *系统信息修改处理
...
//		ercond.setCldeid(String.valueOf(entityItem.getEid()));
		...
		dto.addMessage(new ActionMessage("IGCO10000.I005","系统基本信息"));
		log.debug("========系统信息编辑画面初期显示处理终了========");
		return dto;
	}
	...
	public IGASM18DTO searchEntityItemLableAction(IGASM18DTO dto) throws BLException {
		...
		log.debug("========获取系统编号最大值处理开始========");
		
		IGASM1802Form form = dto.getIgasm1802Form();
		
		SOC0118SearchCondImpl cond = new SOC0118SearchCondImpl();
		
		cond.setEilabel(CommonDefineUtils.ENTIY_CATEGORY_SYSTEM_KEY + IGStringUtils.getCurrentYear().substring(2));
		
		String eilabel = this.soc0118BL.searchNextEntityItemLable(cond);
		
		form.setEilabel(eilabel);
		
		log.debug("========获取系统编号最大值处理终了========");
		return dto;
	}
	...
	public IGASM18DTO checkEntityItemDomain(IGASM18DTO dto) throws BLException{
		...
		if(isRoleManager) {
			...
			SOC0117SearchCondImpl entityCond = new SOC0117SearchCondImpl();
			entityCond.setEsyscoding(CommonDefineUtils.ENTIY_CATEGORY_SYSTEM_SYSCODING);
			List<SOC0117Info> entity = this.soc0117BL.searchEntity(entityCond, 0, 0);
			
			if(entity.size() != 1) {
				throw new BLException("IGCO10000.E004","系统模型");
			} else {
