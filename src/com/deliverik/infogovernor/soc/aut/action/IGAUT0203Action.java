...
import com.deliverik.infogovernor.soc.aut.vo.IGAUT02031VO;
 ...
 *
 */
public class IGAUT0203Action extends BaseAction {

	static Log log = LogFactory.getLog(IGAUT0203Action.class);
	...
	public ActionForward doProcess(ActionMapping mapping, ActionForm actionForm, HttpServletRequest req, HttpServletResponse res) throws Exception {
		//FORM取得
		IGAUT0203Form form = (IGAUT0203Form)actionForm;
		...
		if( "DISP".equals(mapping.getParameter())){
			...
			//***************************************************************************
			//类型分类(设备)设定
			form = new IGAUT0203Form();
			//form.setEcategory(CommonDefineUtils.ENTIY_CATEGORY_DEVICE);
			req.setAttribute("IGAUT0203Form", form);
		...
		if( "SEARCH".equals(mapping.getParameter())||"SEARCH1".equals(mapping.getParameter())){
			...
			if ("SEARCH1".equals(mapping.getParameter())){
				...
				form = (IGAUT0203Form) req.getSession().getAttribute("IGAUT0203Form");
				if ( form== null){
					form = new IGAUT0203Form();
					//form.setEcategory(CommonDefineUtils.ENTIY_CATEGORY_DEVICE);
				}else {
			...
			//查询记录最大件数取得
			int maxCnt = getMaxDataCount("IGAUT0203");
			
			//DTO输入参数设定
			dto.setIgaut0203Form(form);
			...
			//设备检索逻辑调用
			dto = ctlBL.searchEntityItemFor0203Action(dto);
			

			//将设备信息检索结果设定到VO中
			IGAUT02031VO vo = new IGAUT02031VO(dto.getEntityItemVWInfoList());
			vo.setDomainid(dto.getDomainid());
			vo.setDomainversion(dto.getDomainversion());
			vo.setShowRelationMap(dto.getShowRelationMap());
			vo.setCreatetime(dto.getCreatetime());
			
			super.<IGAUT02031VO>setVO(req, vo);
