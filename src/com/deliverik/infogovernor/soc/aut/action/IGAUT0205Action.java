...
import com.deliverik.infogovernor.soc.aut.vo.IGAUT02051VO;
 ...
 *
 */
public class IGAUT0205Action extends BaseAction {

	static Log log = LogFactory.getLog(IGAUT0205Action.class);
	...
	public ActionForward doProcess(ActionMapping mapping, ActionForm actionForm, HttpServletRequest req, HttpServletResponse res) throws Exception {
		//FORM取得
		IGAUT0205Form form = (IGAUT0205Form)actionForm;
		...
		if( "DISP".equals(mapping.getParameter())){
			...
			//***************************************************************************
			//类型分类(设备)设定
			form = new IGAUT0205Form();
			//form.setEcategory(CommonDefineUtils.ENTIY_CATEGORY_DEVICE);
			req.setAttribute("IGAUT0205Form", form);
		...
		if( "SEARCH".equals(mapping.getParameter())||"SEARCH1".equals(mapping.getParameter())){
			...
			if ("SEARCH1".equals(mapping.getParameter())){
				...
				form = (IGAUT0205Form) req.getSession().getAttribute("IGAUT0205Form");
				if ( form== null){
					form = new IGAUT0205Form();
					//form.setEcategory(CommonDefineUtils.ENTIY_CATEGORY_DEVICE);
				}else {
			...
			//查询记录最大件数取得
			int maxCnt = getMaxDataCount("IGAUT0205");

			//DTO输入参数设定
			dto.setIgaut0205Form(form);
			...
			//设备检索逻辑调用
			dto = ctlBL.searchEntityItemFor0205Action(dto);
			

			//将设备信息检索结果设定到VO中
			IGAUT02051VO vo = new IGAUT02051VO(dto.getEntityItemVWInfoList());
			vo.setDomainid(dto.getDomainid());
			vo.setDomainversion(dto.getDomainversion());
			vo.setShowRelationMap(dto.getShowRelationMap());
			vo.setCreatetime(dto.getCreatetime());
			
			super.<IGAUT02051VO>setVO(req, vo);
