...
import com.deliverik.infogovernor.soc.aut.vo.IGAUT02011VO;
 ...
 *
 */
public class IGAUT0201Action extends BaseAction {

	static Log log = LogFactory.getLog(IGAUT0201Action.class);
	...
	public ActionForward doProcess(ActionMapping mapping, ActionForm actionForm, HttpServletRequest req, HttpServletResponse res) throws Exception {
		//FORM取得
		IGAUT0201Form form = (IGAUT0201Form)actionForm;
		...
		if( "DISP".equals(mapping.getParameter())){
			...
			//***************************************************************************
			//类型分类(设备)设定
			form = new IGAUT0201Form();
			//form.setEcategory(CommonDefineUtils.ENTIY_CATEGORY_DEVICE);
			req.setAttribute("IGAUT0201Form", form);
		...
		if( "SEARCH".equals(mapping.getParameter())||"SEARCH1".equals(mapping.getParameter())){
			...
			if ("SEARCH1".equals(mapping.getParameter())){
				...
				form = (IGAUT0201Form) req.getSession().getAttribute("IGAUT0201Form");
				if ( form== null){
					form = new IGAUT0201Form();
					//form.setEcategory(CommonDefineUtils.ENTIY_CATEGORY_DEVICE);
				}else {
			...
			//查询记录最大件数取得
			int maxCnt = getMaxDataCount("IGAUT0201");
			
	
			//DTO输入参数设定
			dto.setIgaut0201Form(form);
			...
			//设备检索逻辑调用
			dto = ctlBL.searchEntityItemFor0201Action(dto);
			

			//将设备信息检索结果设定到VO中
			IGAUT02011VO vo = new IGAUT02011VO(dto.getEntityItemVWInfoList());
			vo.setDomainid(dto.getDomainid());
			vo.setDomainversion(dto.getDomainversion());
			vo.setShowRelationMap(dto.getShowRelationMap());
			vo.setCreatetime(dto.getCreatetime());
			
			super.<IGAUT02011VO>setVO(req, vo);
