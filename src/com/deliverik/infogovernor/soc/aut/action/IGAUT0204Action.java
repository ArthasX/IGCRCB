...
import com.deliverik.infogovernor.soc.aut.vo.IGAUT02041VO;
 ...
 *
 */
public class IGAUT0204Action extends BaseAction {

	static Log log = LogFactory.getLog(IGAUT0204Action.class);
	...
	public ActionForward doProcess(ActionMapping mapping, ActionForm actionForm, HttpServletRequest req, HttpServletResponse res) throws Exception {
		//FORM取得
		IGAUT0204Form form = (IGAUT0204Form)actionForm;
		...
		if( "DISP".equals(mapping.getParameter())){
			...
			//***************************************************************************
			//类型分类(设备)设定
			form = new IGAUT0204Form();
			//form.setEcategory(CommonDefineUtils.ENTIY_CATEGORY_DEVICE);
			req.setAttribute("IGAUT0204Form", form);
		...
		if( "SEARCH".equals(mapping.getParameter())||"SEARCH1".equals(mapping.getParameter())){
			...
			if ("SEARCH1".equals(mapping.getParameter())){
				...
				form = (IGAUT0204Form) req.getSession().getAttribute("IGAUT0204Form");
				if ( form== null){
					form = new IGAUT0204Form();
					//form.setEcategory(CommonDefineUtils.ENTIY_CATEGORY_DEVICE);
				}else {
			...
			//查询记录最大件数取得
			int maxCnt = getMaxDataCount("IGAUT0204");
...
//			}
			...
			//设备检索逻辑调用
			dto = ctlBL.searchEntityItemFor0204Action(dto);
			

			//将设备信息检索结果设定到VO中
			IGAUT02041VO vo = new IGAUT02041VO(dto.getEntityItemVWInfoList());
			vo.setDomainid(dto.getDomainid());
			vo.setDomainversion(dto.getDomainversion());
			vo.setShowRelationMap(dto.getShowRelationMap());
			vo.setCreatetime(dto.getCreatetime());
			
			super.<IGAUT02041VO>setVO(req, vo);
