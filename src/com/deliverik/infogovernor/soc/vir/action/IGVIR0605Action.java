...
import com.deliverik.infogovernor.soc.vir.vo.IGVIR06051VO;
 ...
 *
 */
public class IGVIR0605Action extends BaseAction {

	static Log log = LogFactory.getLog(IGVIR0605Action.class);
	...
	public ActionForward doProcess(ActionMapping mapping, ActionForm actionForm, HttpServletRequest req, HttpServletResponse res) throws Exception {
		//FORM取得
		IGVIR0605Form form = (IGVIR0605Form)actionForm;
		...
		if( "DISP".equals(mapping.getParameter())){
			...
			//***************************************************************************
			//类型分类(设备)设定
			form = new IGVIR0605Form();
			//form.setEcategory(CommonDefineUtils.ENTIY_CATEGORY_DEVICE);
			req.setAttribute("IGVIR0605Form", form);
		...
		if( "SEARCH".equals(mapping.getParameter())||"SEARCH1".equals(mapping.getParameter())){
			...
			if ("SEARCH1".equals(mapping.getParameter())){
				...
				form = (IGVIR0605Form) req.getSession().getAttribute("IGVIR0605Form");
				if ( form== null){
					form = new IGVIR0605Form();
					//form.setEcategory(CommonDefineUtils.ENTIY_CATEGORY_DEVICE);
				}else {
			...
			//查询记录最大件数取得
			int maxCnt = getMaxDataCount("IGVIR0605");
			...
			//设备检索逻辑调用
			dto = ctlBL.searchEntityItem0605Action(dto);
			

			//将设备信息检索结果设定到VO中
			IGVIR06051VO vo = new IGVIR06051VO(dto.getEntityItemVWInfoList());
			vo.setDomainid(dto.getDomainid());
			vo.setDomainversion(dto.getDomainversion());
			vo.setShowRelationMap(dto.getShowRelationMap());
			vo.setCreatetime(dto.getCreatetime());
			
			super.<IGVIR06051VO>setVO(req, vo);
