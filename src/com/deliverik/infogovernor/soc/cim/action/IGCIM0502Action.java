...
import com.deliverik.infogovernor.soc.cim.form.IGCIM0502Form;
import com.deliverik.infogovernor.soc.cim.vo.IGCIM05021VO;
import com.deliverik.infogovernor.sym.bl.IGSYM03BL;
import com.deliverik.infogovernor.sym.dto.IGSYM03DTO;

/**
 * 概述: 交换机配置分析查询画面Action处理
 * 功能描述: 交换机配置分析查询画面Action处理
 * 创建记录: yanglongquan 2013/05/22
 * 修改记录: 
 */
public class IGCIM0502Action extends BaseAction {

	static Log log = LogFactory.getLog(IGCIM0502Action.class);
	...
	public ActionForward doProcess(ActionMapping mapping, ActionForm actionForm, HttpServletRequest req, HttpServletResponse res) throws Exception {
		//FORM取得
		IGCIM0502Form form = (IGCIM0502Form)actionForm;
		...
		if( "DISP".equals(mapping.getParameter())){
			//交换机配置查询画面初期显示处理
			log.debug("========交换机配置查询画面初期显示处理开始========");
			
			//***************************************************************************
			//类型分类(设备)设定
			form = new IGCIM0502Form();
			form.setEcategory(CommonDefineUtils.ENTIY_CATEGORY_DEVICE);
			req.setAttribute("IGCIM0502Form", form);
			...
			log.debug("========交换机配置查询画面初期显示处理终了========");
			return mapping.findForward("DISP");
		} else if("REDISP".equals(mapping.getParameter())){
			//交换机配置查询画面再显示处理
			log.debug("========交换机配置查询画面再显示处理开始========");
			
			addMessage(req, new ActionMessage("IGCO10000.I003"));
			log.debug("========交换机配置查询画面再显示处理终了========");
			return mapping.findForward("DISP");
		}	
		...
		if( "SEARCH".equals(mapping.getParameter())||"SEARCH1".equals(mapping.getParameter())){
			...
			//交换机配置查询处理
			log.debug("========交换机配置查询处理开始========");
			if ("SEARCH1".equals(mapping.getParameter())){
				//由详细画面，编辑画面返回查询画面时的再检索处理
				form = (IGCIM0502Form) req.getSession().getAttribute("IGCIM0502Form");
				if ( form== null){
					form = new IGCIM0502Form();
					form.setEcategory(CommonDefineUtils.ENTIY_CATEGORY_DEVICE);
				}else {
			...
			//查询记录最大件数取得
			int maxCnt = getMaxDataCount("IGCIM0502");
...
//			}
			...
			if(esyscoding!=null&&!"".equals(esyscoding)&&esyscoding.length()==6){
				...
				form.setEsyscoding(CommonDefineUtils.SWITCH_ESYSCODING);
			}
			//DTO输入参数设定
			...
			//将交换机配置信息检索结果设定到VO中
			IGCIM05021VO vo = new IGCIM05021VO(dto.getEntityItemVWInfoList());
			...
			super.<IGCIM05021VO>setVO(req, vo);
			
			log.debug("========交换机配置查询处理终了========");
		}
