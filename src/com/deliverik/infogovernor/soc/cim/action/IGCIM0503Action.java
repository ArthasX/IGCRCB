...
import com.deliverik.infogovernor.soc.cim.form.IGCIM0503Form;
import com.deliverik.infogovernor.soc.cim.vo.IGCIM05031VO;
 ...
 * 修改记录: 
 */
public class IGCIM0503Action extends BaseAction {

	static Log log = LogFactory.getLog(IGCIM0503Action.class);
	...
	public ActionForward doProcess(ActionMapping mapping, ActionForm actionForm, HttpServletRequest req, HttpServletResponse res) throws Exception {
		//FORM取得
		IGCIM0503Form form = (IGCIM0503Form)actionForm;
		...
		if( "DISP".equals(mapping.getParameter())){
			//存储配置查询画面初期显示处理
			log.debug("========存储配置查询画面初期显示处理开始========");
			
			//***************************************************************************
			//类型分类(设备)设定
			form = new IGCIM0503Form();
			form.setEcategory(CommonDefineUtils.ENTIY_CATEGORY_DEVICE);
			req.setAttribute("IGCIM0503Form", form);
			...
			log.debug("========存储配置查询画面初期显示处理终了========");
			return mapping.findForward("DISP");
		} else if("REDISP".equals(mapping.getParameter())){
			//存储配置查询画面再显示处理
			log.debug("========存储配置查询画面再显示处理开始========");
			
			addMessage(req, new ActionMessage("IGCO10000.I003"));
			log.debug("========存储配置查询画面再显示处理终了========");
			return mapping.findForward("DISP");
		}	
		...
		if( "SEARCH".equals(mapping.getParameter())||"SEARCH1".equals(mapping.getParameter())){
			...
			//存储配置查询处理
			log.debug("========存储配置查询处理开始========");
			if ("SEARCH1".equals(mapping.getParameter())){
				//由详细画面，编辑画面返回查询画面时的再检索处理
				form = (IGCIM0503Form) req.getSession().getAttribute("IGCIM0503Form");
				if ( form== null){
					form = new IGCIM0503Form();
					form.setEcategory(CommonDefineUtils.ENTIY_CATEGORY_DEVICE);
				}else {
			...
			//查询记录最大件数取得
			int maxCnt = getMaxDataCount("IGCIM0503");
...
//			}
			...
			if(esyscoding!=null&&!"".equals(esyscoding)&&esyscoding.length()==6){
				...
				form.setEsyscoding(CommonDefineUtils.STORAGE_ESYSCODING);
			}
			//DTO输入参数设定
			...
			//将存储配置信息检索结果设定到VO中
			IGCIM05031VO vo = new IGCIM05031VO(dto.getEntityItemVWInfoList());
			...
			super.<IGCIM05031VO>setVO(req, vo);
			
			log.debug("========存储配置查询处理终了========");
		}
