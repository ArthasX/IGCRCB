...
import com.deliverik.infogovernor.soc.cim.vo.IGCIM16311VO;
 ...
 *
 */
public class IGCIM1631Action extends BaseAction {

	static Log log = LogFactory.getLog(IGCIM1631Action.class);
	...
	public ActionForward doProcess(ActionMapping mapping, ActionForm actionForm, HttpServletRequest req, HttpServletResponse res) throws Exception {
		//FORM取得
		IGCIM1631Form form = (IGCIM1631Form)actionForm;
		
		//BL取得
		IGCIM01BL ctlBL = (IGCIM01BL) getBean("igcim01BL");
		IGCIM16BL cim16BL = (IGCIM16BL) getBean("igcim16BL");
		...
		if( "DISP".equals(mapping.getParameter())){
			...
			//***************************************************************************
			//类型分类(设备)设定
			form = new IGCIM1631Form();
			//form.setEcategory(CommonDefineUtils.ENTIY_CATEGORY_DEVICE);
			req.setAttribute("IGCIM1631Form", form);
		...
		if( "SEARCH".equals(mapping.getParameter())||"SEARCH1".equals(mapping.getParameter())){
			IGCIM16DTO cim16Dto = new IGCIM16DTO();
			
			//分页用DTO取得
			getPaginDTO(req,"IGCIM1631");
			...
			if ("SEARCH1".equals(mapping.getParameter())){
				...
				form = (IGCIM1631Form) req.getSession().getAttribute("IGCIM1631Form");
				if ( form== null){
					form = new IGCIM1631Form();
				}else {
			...
			//查询记录最大件数取得
			int maxCnt = getMaxDataCount("IGCIM1631");
			
			//DTO输入参数设定
			cim16Dto.setIgcim1631Form(form);
			
			cim16Dto.setMaxSearchCount(maxCnt);
			
			cim16Dto.setPagingDto(pDto);
			
			//设备检索逻辑调用
			cim16Dto = cim16BL.searchSoc0163List(cim16Dto);
			

			//将设备信息检索结果设定到VO中
			IGCIM16311VO vo = new IGCIM16311VO();
			vo.setSoc0163List(cim16Dto.getSoc0163List());
			super.<IGCIM16311VO>setVO(req, vo);
