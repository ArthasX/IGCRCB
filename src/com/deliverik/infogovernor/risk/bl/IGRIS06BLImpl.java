 ...
 * 功能描述： 1.
 *           2.
 * 创建记录：yukexin    2014-9-11 下午3:44:46	
 * 修改记录：null
 */	
public class IGRIS06BLImpl extends BaseBLImpl implements IGRIS06BL {

	static Log log = LogFactory.getLog(IGRIS06BLImpl.class);
	...
	protected static final String SYSCODE = "999021";	
...
//	protected IG225BL soc0109BL;
	...
	public IGRIS06DTO insertEntityItemAction(IGRIS06DTO dto) throws BLException {
		...
		//输入Form取得
		IGRIS0602Form form = (IGRIS0602Form)dto.getAttachFile();
		if(form.getFileList(0).getFile()!=null && form.getFileList(0).getFile()!=null && !form.getFileList(0).getFile().getFileName().equals("")){
			String attkey = insertFileListAction(dto);
			form.setNattkey(attkey);
			form.getCivalue()[3] = attkey;
		}
		...
		//登记日期
		entityItem1.setEiinsdate(form.getEiinsdate());
		//检查项状态默认1 启用
		entityItem1.setEistatus("1");
	...
	private String insertFileListAction(IGRIS06DTO dto) throws BLException {
		IGRIS0602Form form = dto.getAttachFile();
	...
	private String updateFileListAction(IGRIS06DTO dto) throws BLException {
		IGRIS0604Form form = dto.getIgris0604Form();
		
		// 上传文件检测
		AttachmentTB attachmentOld = null;
		if(form.getAttachmentList() !=null && form.getAttachmentList().size() != 0){
			attachmentOld = (AttachmentTB) form.getAttachmentList().get(0);
		}else{
			attachmentOld = new AttachmentTB();
		}
		AttachmentTB attachment = new AttachmentTB();
		String attkey = null;
		if(attachmentOld.getAttkey()==null ||attachmentOld.getAttkey().equals("")){
			attkey = IGStringUtils.getAttKey();
			attachmentOld.setAttkey(attkey);
		}else{
			attkey = attachmentOld.getAttkey();
		}
		List<FormFileInfo> astFormFileInfo = form.getFileListInfo();
		for (int i = 0; i < astFormFileInfo.size(); i++) {
			FormFile file = astFormFileInfo.get(i).getFile();
...
//		form.setEistatus(entityItemVWInfo.getEistatus());
	 ...
	 */
	public IGRIS06DTO editEntityItemAction(IGRIS06DTO dto) throws BLException, FileNotFoundException, IOException {
		...
		if(StringUtils.isEmpty(ident)){
			ident = IGRIS06BLImpl.SAVE_VERSION;
		}
		// 版本号初始化
		int version = 1;
		...
		// 判断是否升级版本号
		// =====================>> 修改 start
		if (IGRIS06BLImpl.SAVE_VERSION_UP.equals(ident)) { // 升级版本号
			version = form.getEiversion() + 1;
			form.setEiversion(version);
		...
		} else if (IGRIS06BLImpl.SAVE_VERSION.equals(ident)) { // 不升级版本号
		...
		//更新检查说明
		entityItem.setEidesc(form.getCivalue()[1]);
		//更新检查项分类
		entityItem.setEsyscoding(form.getCivalue()[4]);
		//更新登记日期
		entityItem.setEiinsdate(form.getCivalue()[5]);
		...
		if(version ==1&&versonFlag){
			...
			//与该资产相关的关系码
			eircond1.setEirrelationcode(IGRIS06BLImpl.EIRRELATIONCODE);
	...
	public IGRIS06DTO initIGRIS0605Action(IGRIS06DTO dto) throws BLException {
		...
		if (StringUtils.isEmpty(dto.getIgris0605Form().getCiversion()+"")) {
			dto.getIgris0605Form().setCiversion(String.valueOf(entityItemVWInfo.getEiversion()));
		}
		//获取历史版本更新时间
		dto.setConfigItemVersionTime(this.soc0118BL.searchVersionUpdateTime(dto.getIgris0605Form().getEiid()));
	...
	public IGRIS06DTO initIGRIS0606Action(IGRIS06DTO dto) throws BLException {
		...
		//检查项信息检索
		SOC0124Info entityItemVWInfo = this.soc0118BL.searchEntityItemVWByKey(
				Integer.parseInt(dto.getIgris0606Form().getEiid()));
		...
		if((dto.getIgris0606Form().getEiid())!=null)
		{parcond.setPareiid(Integer.parseInt(dto.getIgris0606Form().getEiid()));}
...
//		
//		cldcond.setCldeiid(dto.getIgris0606Form().getEiid());
...
//		}
	...
	public IGRIS06DTO insertEntityItemRelationAction(IGRIS06DTO dto) throws BLException {
		...
		IGRIS0607Form form = dto.getIgris0607Form();
		...
		//资产关系登记
		SOC0119Info ret = 
			this.soc0119BL.registEntityItemRelation(dto.getIgris0607Form());
	...
	public IGRIS06DTO searchRelationListAction(IGRIS06DTO dto) throws BLException {
		...
		List<LabelValueBean> list = this.codeDetailBL.searchLabelValueBeanList(CommonDefineUtils.ENTIY_CATEGORY_SAFETY_CHECKITEMS,CodeDefine.getCodeDefine("ENTITY_ITEM_RELATION_CODE").getCcid());
	...
	public IGRIS06DTO initEntityItem(IGRIS06DTO dto) throws BLException {
		...
		IGRIS0612Form form = dto.getIgris0612Form();
...
//		ercond.setCldeid(String.valueOf(entityItem.getEid()));
	...
	public IGRIS06DTO searchEntityItemLableAction(IGRIS06DTO dto) throws BLException {
		...
		IGRIS0602Form form = dto.getIgris0602Form();
	...
	public IGRIS06DTO checkEntityItemDomain(IGRIS06DTO dto) throws BLException{
		...
		if(isRoleManager) {
			...
			SOC0117SearchCondImpl entityCond = new SOC0117SearchCondImpl();
			entityCond.setEsyscoding(CommonDefineUtils.ENTIY_CATEGORY_SAFETY_CHECKITEMS_SYSCODING);
	...
	public IGRIS06DTO searchCheckItemListAction(IGRIS06DTO dto)
			throws BLException {
		
		IGRIS0614Form form = dto.getIgris0614form();
		/**检查项查询条件设定*/
		CheckItemVWSearchCondImpl cond = new CheckItemVWSearchCondImpl();
		cond.setEiname(form.getEiname() != null ?form.getEiname().trim() : form.getEiname());
		...
		/**分页条数*/
		int totalCount = this.checkItemVWBL.searchSafeInfoCheckItemVW(cond, 0, 0).size();
		...
		/**检查项资产查询*/
		List<CheckItemVWInfo> itemList = this.checkItemVWBL.searchSafeInfoCheckItemVW(cond, pDto.getFromCount(),
				pDto.getPageDispCount());
	...
	public IGRIS06DTO searchCheckItemListNotPageAction(IGRIS06DTO dto)
			throws BLException {
		...
		/**检查项资产查询*/
		List<CheckItemVWInfo> itemList = this.checkItemVWBL.searchSafeInfoCheckItemVW(cond, 0, 0);
	...
	public IGRIS06DTO searchCheckItemWindowAction(IGRIS06DTO dto)
			throws BLException {
		...
		cond.setManagername(form.getMguserid());
		cond.setEiid_NotIn(form.getEiids());
		cond.setBh(form.getEilabel());
		cond.setCheckfrequency(form.getCheckfrequency());
		cond.setStartdate(form.getStartdate());
		cond.setEnddate(form.getEnddate());
...
//		pDto.setTotalCount(totalCount);
	 ...
	 * @return IGASM18DTO 
	 */
	public IGRIS06DTO searchRiskPropertyActioin(IGRIS06DTO dto)
			throws BLException {
	 ...
	 * @throws BLException
	 */
	public IGRIS06DTO createConfigItem(Integer eiid, List<EntityItemConfigValueInfo> lst_EntityItemConfigValueInfo)throws BLException {
	...
	public Integer createEntityItem(EntityItemInfo entityItem) 	throws BLException {
		...
		instance.setEiinsdate(entityItem.getEiinsdate());//登记日期
	 ...
	 * 修改记录：null
	 */	
	public IGRIS06DTO serachCheckItemInfoListAction(IGRIS06DTO dto)
			throws BLException {
		List<CheckItemVWInfo> list = dto.getItemList();
		List<CheckItemVWInfo> newList = new ArrayList<CheckItemVWInfo>();
		if(list != null && list.size() != 0){
			for (CheckItemVWInfo checkItemVWInfo : list) {
				IGRIS0604Form form = new IGRIS0604Form();
				form.setEiid(checkItemVWInfo.getEiid().toString());
				SOC0118Info entityItem = this.soc0118BL.searchEntityItemByKey(Integer.parseInt(form.getEiid()));
				CheckItemVW bean = (CheckItemVW)SerializationUtils.clone(checkItemVWInfo);
				bean.setEiname(entityItem.getEiname());
				bean.setEidesc(entityItem.getEidesc());
				newList.add(bean);
			}
			dto.setItemList(newList);
		}
	...
	public Integer updateCheckItemStatus(IGRIS06DTO dto) throws BLException {
		int success = 0;
		//获取检查项id
		String eiid = dto.getIgris0604Form().getEiid();
		if(null != eiid && !"".equals(eiid)){
			//查询该检查项是否存在
			SOC0118Info entityItem = this.soc0118BL.searchEntityItemByKey(Integer.parseInt(eiid));
			if(entityItem != null){
				//更新检查项状态,若启用则变停止,若停止则变启用 (status是将要改变的状态值)
				Integer status = "1".equals(entityItem.getEistatus()) ? 0 : 1;
				String sql = "update soc0118 set EISTATUS ='" + status + "' where eiid = '" + eiid +"'";
				success = soc0118BL.executeSQL(sql);
				//如果检查项状态更改成功
				if(success > 0){
					RiskCheckSearchCondImpl cond = new RiskCheckSearchCondImpl();
					cond.setRcasset(Integer.parseInt(eiid));
					//更新使用指定eiid检查项
					List<RiskCheck> riskCheckList =riskCheckBL.searchRiskCheck(cond, 0, 0);
					if(null != riskCheckList && riskCheckList.size() > 0){
						//遍历如果停用则启用,启用则停用
						for (RiskCheck riskCheck : riskCheckList) {
							//DTO生成
							IGRIS02DTO igris02dto = new IGRIS02DTO();
							//form生成
							IGRIS0202Form igris0202Form = new IGRIS0202Form();
							//复制对象
							RiskCheckTB riskCheckTB = (RiskCheckTB)SerializationUtils.clone(riskCheck);
							//赋值状态
							if(status == 0){
								riskCheckTB.setRcstatus(status.toString());
								igris0202Form.setRcstatus(status.toString());
							}else if(status == 1){
								riskCheckTB.setRcstatus(status.toString());
								igris0202Form.setRcstatus(status.toString());
							}
							
							igris0202Form.setRcid(riskCheckTB.getRcid());
							igris02dto.setUser(dto.getUser());
							igris02dto.setIgRIS0202Form(igris0202Form);
							igris02dto.setRiskCheck(riskCheckTB);
							//检查策略检查项调整状态 
							igris02dto = igris02BL.changeRiskCheckStatusAction(igris02dto);
							
							//更新检查策略中检查项状态
							riskCheckBL.updateRiskCheck(riskCheckTB);
						}
					}
				}
			}
