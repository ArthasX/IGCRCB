 ...
 * </p>
 */
public class IGDRMBTN0102BLImpl extends BaseBLImpl implements
		WorkFlowStatusButtonHandlerBL {
	
	static Log log = LogFactory.getLog(IGDRMBTN0102BLImpl.class);
	...
	public void statusButtonTreatmentExecute(WorkFlowStatusEventBeanInfo bean) throws BLException {
		...
		if(StringUtils.isNotEmpty(opdid)){
			if("单一场景".equals(sceneType)){
				...
				// ig560tb =
				// (IG560TB)SerializationUtils.clone(ig560List.get(i));
				...
				//升级原流程定义ID
				workFlowDefinitionBL.upgradeGroupProcessDefVersion(opdid);
				//流程版本号
				String version = StringUtils.leftPad(String.valueOf(Integer.parseInt(opdid.substring(5, 7)) + 1), 2, "0");
				//新版本流程pdid
				String pgdid = opdid.substring(0, 5) + version;
				// 获取流程组定义
				IG480Info cptInfo = workFlowDefinitionBL.searchGroupProcessDefinitionByKey(pgdid);
//				IG380Info cptInfo = workFlowDefinitionBL.searchProcessDefinitionByKey(pdid);
				// 更新流程定义基本信息-原流程开启
				IG480TB cptTB = (IG480TB) SerializationUtils.clone(cptInfo);
				cptTB.setPgdstatus(IGPRDCONSTANTS.PD_STATUS_ENABLE);
				// 更新流程定义
				workFlowDefinitionBL.updateGroupProcessDefinition(cptTB);
				// 获取新版本流程信息
				IG480Info ptInfo = workFlowDefinitionBL.searchGroupProcessDefinitionByKey(pgdid);
				// 更新流程定义基本信息
				new480TB = (IG480TB) SerializationUtils.clone(ptInfo);
				new480TB.setPgdname(prtitle); // 场景名称
				new480TB.setPgddesc(pddesc); // 场景描述
				// 更新流程定义基本信息-修订流程关闭
				new480TB.setPgdcrtime(IGStringUtils.getCurrentDate()); // 创建时间
				new480TB.setPgdstatus(IGPRDCONSTANTS.PD_STATUS_DISABLE);
				
				// 更新流程定义
				workFlowDefinitionBL.updateGroupProcessDefinition(new480TB);
				// 清空其他表数据
				//clearRelatedData(pdid);
				// 将流程定义ID设定到流程中
				PublicProcessInfoValue ppivInfo = new PublicProcessInfoValue(bean.getLogInfo());
